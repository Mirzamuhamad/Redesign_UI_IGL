...
Partial Class Transaction_TrWODownPriority
    ...
    Sub BindData()
        ...
        Dim SQLString, tes As String

        Try

            StrFilter1 = " Where YEAR(EndDate) = " + QuotedStr(ddlYear.SelectedValue) + " AND DoneClosing = 'Y'"
            ...
            '    SQLString = "EXEC S_PLWOPlanOutDown ''," + QuotedStr(StrFilter)
            '    tempDS = SQLExecuteQuery(SQLString, ViewState("DBConnection").ToString)
            '    'tempDS = SQLExecuteQuery("SELECT * FROM V_PLWOPlanForDown " + StrFilter, ViewState("DBConnection").ToString)
            'Else
            '    SQLString = "EXEC S_PLWOPlanOutDown " + QuotedStr(StrFilter1)
            '    tempDS = SQLExecuteQuery(SQLString, ViewState("DBConnection").ToString)
            '    'tempDS = SQLExecuteQuery("SELECT * FROM V_PLWOPlanForDown " + StrFilter1, ViewState("DBConnection").ToString)
            'End If
            ...
            If StrFilter.Length > 5 Then
                ...
                tempDS = SQLExecuteQuery("SELECT * FROM V_PLWOPlanForDown " + StrFilter, ViewState("DBConnection").ToString)
            Else
                SQLString = "SELECT * FROM V_PLWOPlanForDown "
                tempDS = SQLExecuteQuery(SQLString, ViewState("DBConnection").ToString)
            End If
    ...
    Private Sub bindDataProcess()
        Try
            ...
            Dim CB As CheckBox
            Dim lbWeek, lbReference As Label
            ...
            For Each GVR In DataGrid.Rows
                ...
                lbWeek = GVR.FindControl("BtmWeek")
                lbReference = GVR.FindControl("Reference")

                If CB.Checked Then
                    HaveSelect = True
                    SQLString = "DECLARE @A VARCHAR(255) EXEC S_PLWOPlanDown " + QuotedStr(lbReference.Text) + ", '',  " + QuotedStr(ViewState("GLYear").ToString) + "," + QuotedStr(lbWeek.Text) + _
                    ", " + QuotedStr(ViewState("GLPeriod").ToString) + _
    ...
    Protected Sub ddlYear_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlYear.SelectedIndexChanged
        Try
            BindData()
        Catch ex As Exception
            Throw New Exception("ddlYear_SelectedIndexChanged Error : " + ex.ToString)
        End Try
    End Sub
End Class