...
Partial Class Transaction_TrRDFormulaTrial_TrRDFormulaTrial
    Inherits System.Web.UI.Page
    Protected con As New SqlConnection
    Protected da As New SqlDataAdapter
    Protected GetStringHd As String = "Select * From V_RDFormulaTrialHd"

    Private Function GetStringDt(ByVal Nmbr As String) As String
        Return "SELECT * From V_RDFormulaTrialDt WHERE TransNmbr = " + QuotedStr(Nmbr)
    End Function
    ...
    Protected Sub BtnGo_Click(ByVal sender As Object, ByVal e As EventArgs) Handles BtnGo.Click, btnGo2.Click
        ...
        Try
            ...
            If ActionValue = "Print" Then
                ...
                Session("SelectCommand") = "EXEC S_RDFormulaTrialForm " + Result
                Session("ReportFile") = ".../../../Rpt/RDFormulaTrialForm.frx"
            ...
            Else
                ...
                For j = 0 To (Nmbr.Length - 1)
                    If Nmbr(j) = "" Then
                        Exit For
                    Else
                        Result = ExecSPCommandGo(ActionValue, "S_RDFormulaTrial", Nmbr(j), CInt(Session(Request.QueryString("KeyId"))("Year")), CInt(Session(Request.QueryString("KeyId"))("Period")), ViewState("UserId").ToString, ViewState("DBConnection").ToString)
                        If Trim(Result) <> "" Then
        ...
        Try
            If Not Dr Is Nothing Then
                If Dr.RowState = DataRowState.Deleted Then
                    Return True
                End If
                If Dr("CriteriaSample").ToString = "" Then
    ...
    Protected Sub FillTextBoxHd(ByVal Nmbr As String)
        ...
        Try
            ...
            BindToText(tbComment, Dt.Rows(0)("Comment").ToString)
    ...
    Protected Sub FillTextBoxDt(ByVal RRNo As String)
        Dim Dr As DataRow()
        Try
            Dr = ViewState("Dt").select("CriteriaSample = " + QuotedStr(RRNo))
            If Dr.Length > 0 Then
                BindToText(tbCriteria, Dr(0)("CriteriaSample").ToString)
                BindToText(tbCriteriaName, Dr(0)("CriteriaSampleName").ToString)
                'BindToText(tbStandard, Dr(0)("Standard").ToString)
                BindToDropList(ddlCriteriaValue, Dr(0)("CriteriaValue").ToString)
    ...
    Protected Sub btnSaveDt_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnSaveDt.Click
        Try
            ...
            If ViewState("StateDt") = "Edit" Then
                If ViewState("DtValue") <> tbCriteria.Text Then
                    If CekExistData(ViewState("Dt"), "CriteriaSample", tbCriteria.Text) Then
                        lbStatus.Text = "Criteria Sample " + tbCriteriaName.Text + " has already exists"
                        Exit Sub
                    End If
                End If

                Dim Row As DataRow
                Row = ViewState("Dt").Select("CriteriaSample = " + QuotedStr(ViewState("DtValue")))(0)
                'Row = ViewState("Dt").Select("CriteriaSample = " + QuotedStr(tbCriteria.Text))(0)

                Row.BeginEdit()
                Row("CriteriaSample") = tbCriteria.Text
                Row("CriteriaSampleName") = tbCriteriaName.Text
                'Row("Standard") = tbStandard.Text
                Row("CriteriaValue") = ddlCriteriaValue.Text
            ...
            Else
                ...
                dr("CriteriaSample") = tbCriteria.Text
                dr("CriteriaSampleName") = tbCriteriaName.Text
                'dr("Standard") = tbStandard.Text
                dr("CriteriaValue") = ddlCriteriaValue.Text
    ...
    Private Sub SaveAll()
        ...
        Try
            ...
            If ViewState("StateSave") = "Insert" Then
                tbCode.Text = GetAutoNmbr("RDFRT", "Y", Year(tbDate.SelectedValue), Month(tbDate.SelectedValue), "", ViewState("DBConnection").ToString)
                SQLString = "INSERT INTO RDFormulaTrialHd (TransNmbr, TransDate, STATUS, Sample, Customer, DeliveryDate, Comment, Remark, UserPrep, DatePrep, UserType ) " + _
                "SELECT " + QuotedStr(tbCode.Text) + ", " + QuotedStr(Format(tbDate.SelectedValue, "yyyy-MM-dd")) + ", 'H', " + QuotedStr(tbSample.Text) + "," + QuotedStr(tbCustomer.Text) + ", " + QuotedStr(Format(tbDeliveryDate.SelectedValue, "yyyy-MM-dd")) + ", " + QuotedStr(tbComment.Text) + ", " + QuotedStr(tbRemark.Text) + ", " + QuotedStr(ViewState("UserId").ToString) + ", GetDate()," + QuotedStr(ddlUserType.SelectedValue)
                ViewState("TransNmbr") = tbCode.Text
            Else
                Dim cekStatus As String
                cekStatus = SQLExecuteScalar("Select Status FROM RDFormulaTrialHd WHERE TransNmbr = " + QuotedStr(tbCode.Text), ViewState("DBConnection").ToString)
                If cekStatus = "P" Then
                    lbStatus.Text = MessageDlg("Save failed. Data has already posted by another user")
                    Exit Sub
                End If
                SQLString = "UPDATE RDFormulaTrialHd SET TransDate = " + QuotedStr(Format(tbDate.SelectedValue, "yyyy-MM-dd")) + _
                            ", Sample = " + QuotedStr(tbSample.Text) + _
                            ", UserType = " + QuotedStr(ddlUserType.SelectedValue) + _
                            ", Customer = " + QuotedStr(tbCustomer.Text) + _
                            ", DeliveryDate = '" + Format(tbDeliveryDate.SelectedValue, "yyyy-MM-dd") + _
                            "', Comment = " + QuotedStr(tbComment.Text) + _
            ...
            con.Open()
            'Dim cmdSql As New SqlCommand("SELECT  TransNmbr, CriteriaSample, Standard, CriteriaValue, Remark FROM RDFormulaTrialDt WHERE TransNmbr = " + QuotedStr(ViewState("TransNmbr")), con)
            Dim cmdSql As New SqlCommand("SELECT  TransNmbr, CriteriaSample, CriteriaValue, Remark FROM RDFormulaTrialDt WHERE TransNmbr = " + QuotedStr(ViewState("TransNmbr")), con)
            ...
            Dim Dt As New DataTable("RDFormulaTrialDt")
    ...
    Protected Sub LbAdvSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles LbAdvSearch.Click
        ...
        Try
            ...
            FilterName = "Formula Trial No, Formula Trial Date, Sample, Customer, Delivery Date, Comment, Remark"
            FilterValue = "TransNmbr, dbo.FormatDate(TransDate), SampleName, CustomerName, dbo.FormatDate(DeliveryDate), Comment, Remark"
                ...
                ElseIf DDL.SelectedValue = "Print" Then
                    Try
                        ...
                        If CekMenu <> "" Then
                            ...
                            Exit Sub
                        End If
                        Session("SelectCommand") = "EXEC S_RDFormulaTrialForm '''" + GVR.Cells(2).Text + "'''"
    ...
    Protected Sub GridDt_RowDeleting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewDeleteEventArgs) Handles GridDt.RowDeleting
        Try
            ...
            dr = ViewState("Dt").Select("CriteriaSample = " + QuotedStr(GVR.Cells(1).Text))
            For Each r In dr
                r.Delete()
            Next
            BindGridDt(ViewState("Dt"), GridDt)
    ...
    Protected Sub BtnGo_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnGo.Init
