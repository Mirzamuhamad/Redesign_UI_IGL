...
Partial Class Transaction_TrJamsostekRegChange_TrJamsostekRegChange
    Inherits System.Web.UI.Page
    Protected con, con2 As New SqlConnection
    Protected da, da2 As New SqlDataAdapter
    Protected GetStringHd As String = "Select * From V_PEJamsostekRegChangeHd"
        ...
        Try
            ...
            If Not Session("Result") Is Nothing Then
                ...
                If ViewState("Sender") = "btnEmp" Then
                    ...
                    BindToText(tbOldSalaryGP, Session("Result")(5).ToString)
    ...
    Private Sub SetInit()
        ...
        tbOldSalaryGP.Attributes.Add("OnBlur", "setformat();")
        tbSalaryGP.Attributes.Add("OnBlur", "setformat();")
        tbSalaryGP.Attributes.Add("OnKeyDown", "return PressNumeric();")
    End Sub
    ...
    Private Function GetStringDt(ByVal Nmbr As String) As String
        Return "SELECT * From V_PEJamsostekRegChangeDt WHERE TransNmbr = " + QuotedStr(Nmbr) + " "
    End Function
        ...
        Try
            ...
            For j = 0 To (Nmbr.Length - 1)
                If Nmbr(j) = "" Then
                    Exit For
                Else
                    Result = ExecSPCommandGo(ActionValue, "S_PEJamsostekRegChange", Nmbr(j), CInt(Session(Request.QueryString("KeyId"))("Year")), CInt(Session(Request.QueryString("KeyId"))("Period")), ViewState("UserId").ToString, ViewState("DBConnection").ToString)
                    If Trim(Result) <> "" Then
    ...
    Private Sub SaveAll()
        ...
        Try
            ...
            If ViewState("StateSave") = "Insert" Then
                tbRef.Text = GetAutoNmbr("JC", "Y", Year(tbDate.SelectedValue), Month(tbDate.SelectedValue), "", ViewState("DBConnection").ToString)
                SQLString = "INSERT INTO PEJamsostekRegChangeHd (TransNmbr, Status, TransDate, Tahun, Bulan, EmpAppr, JobTtlAppr, EmpAcknow, JobTtlAcknow, Remark, UserPrep, DatePrep) " + _
            ...
            Else
                ...
                cekStatus = SQLExecuteScalar("Select Status FROM PEJamsostekRegChangeHd WHERE TransNmbr = " + QuotedStr(tbRef.Text), ViewState("DBConnection").ToString)
                If cekStatus = "P" Then
                    lbStatus.Text = MessageDlg("Save failed... Data has already posted by another user")
                    Exit Sub
                End If
                SQLString = "UPDATE PEJamsostekRegChangeHd SET Tahun = " + ddlYear.SelectedValue + _
            ...
            con.Open()
            Dim cmdSql As New SqlCommand("SELECT TransNmbr, EmpNumb, JamSosTekNo, OldSalaryGP, SalaryGP, Remark FROM PEJamsostekRegChangeDt WHERE TransNmbr = '" & ViewState("Reference") & "'", con)
            ...
            Dim Dt As New DataTable("PEJamsostekRegChangeDt")
        ...
        Try
            If Not (Dr Is Nothing) Then
                ...
                If Dr("Emp_Name").ToString.Trim = "" Then
                    ...
                    Return False
                End If
                If Dr("JamSosTekNo").ToString.Trim = "" Then
                    lbStatus.Text = MessageDlg("JamSosTek No Must Have Value")
                    Return False
                End If
                If CFloat(Dr("SalaryGP").ToString) <= 0 Then
                    lbStatus.Text = MessageDlg("Salary Must Have Value")
                    Return False
                End If
            Else
                ...
                If tbEmpName.Text.Trim = "" Then
                    ...
                    Return False
                End If
                If tbJamSosTekNo.Text.Trim = "" Then
                    lbStatus.Text = MessageDlg("JamSosTek No must have value")
                    tbJamSosTekNo.Focus()
                    Return False
                End If
                If CFloat(tbSalaryGP.Text.Trim) <= 0 Then
                    lbStatus.Text = MessageDlg("Salary must have value")
                    tbSalaryGP.Focus()
                    Return False
                End If
            ...
            If e.CommandName = "Go" Then
                ...
                ElseIf DDL.SelectedValue = "Print" Then
                    ...
                    If CekMenu <> "" Then
                        ...
                        Exit Sub
                    End If
                    Session("SelectCommand") = "EXEC S_PEFormJamsostekChange " + QuotedStr(GVR.Cells(2).Text) + "," + QuotedStr(ViewState("UserId"))
                    Session("ReportFile") = ".../../../Rpt/FormJamsostekChange.frx"
    ...
    Protected Sub FillTextBoxDt(ByVal ItemNo As String)
        ...
        Try
            ...
            If Dr.Length > 0 Then
                ...
                BindToText(tbOldSalaryGP, Dr(0)("OldSalaryGP").ToString, ViewState("DigitHome"))
    ...
    Protected Sub btnSaveDt_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSaveDt.Click
        ...
        Try
            If ViewState("StateDt") = "Edit" Then
                ...
                Row("OldSalaryGP") = tbOldSalaryGP.Text
            ...
            Else
                ...
                dr("OldSalaryGP") = tbOldSalaryGP.Text
    ...
    Protected Sub btnEmpAppr_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnEmpAppr.Click
        ...
        Try
            Session("filter") = "SELECT Emp_No, Emp_Name, Job_Title, Job_Title_Name FROM V_MsEmployee WHERE COALESCE(Fg_Active, 'N') = 'Y' AND COALESCE(Fg_Approval, 'N') = 'Y' AND Emp_No <> " + QuotedStr(tbEmpAcknow.Text)
            Session("DBConnection") = ViewState("DBConnection")
    ...
    Protected Sub btnEmpAcknow_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnEmpAcknow.Click
        Dim ResultField As String
        Try
            Session("filter") = "SELECT Emp_No, Emp_Name, Job_Title, Job_Title_Name FROM V_MsEmployee WHERE COALESCE(Fg_Active,'N') = 'Y' AND COALESCE(Fg_Acknowledged,'N') = 'Y' AND Emp_No <> " + QuotedStr(tbEmpAppr.Text)
            Session("DBConnection") = ViewState("DBConnection")
            ResultField = "Emp_No, Emp_Name, Job_Title"
    ...
    Protected Sub btnEmp_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnEmp.Click
        Dim ResultField As String
        Try
            'Session("filter") = "SELECT Emp_No, Emp_Name, Gender, Birth_Date, JamsostekNo FROM V_MsEmployee WHERE COALESCE(Fg_Active, 'N') = 'Y' AND COALESCE(FgJamsostek, 'N') = 'Y' AND Emp_No NOT IN (" + QuotedStr(tbEmpAppr.Text) + "," + QuotedStr(tbEmpAcknow.Text) + ")"
            Session("filter") = "EXEC S_PEJamsostekRegChangeFindEmp ''," + ddlYear.SelectedItem.Text + "," + (ddlPeriod.SelectedIndex + 1).ToString + "," + QuotedStr(tbEmpAppr.Text) + "," + QuotedStr(tbEmpAcknow.Text)
            Session("DBConnection") = ViewState("DBConnection")
            ResultField = "EmpNumb, EmpName, Gender, JamsostekNo, BirthDate, OldSalary"
    ...
    Protected Sub tbEmpCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbEmpCode.TextChanged
        ...
        Dim DT As DataTable
        Try
            DT = SQLExecuteQuery("EXEC S_PEJamsostekRegChangeFindEmp " + QuotedStr(tbEmpCode.Text) + "," + ddlYear.SelectedItem.Text + "," + (ddlPeriod.SelectedIndex + 1).ToString + "," + QuotedStr(tbEmpAppr.Text) + "," + QuotedStr(tbEmpAcknow.Text), ViewState("DBConnection").ToString).Tables(0)
            If DT.Rows.Count > 0 Then
                Dr = DT.Rows(0)
            Else
                Dr = Nothing
            ...
            If Not Dr Is Nothing Then
                ...
                tbOldSalaryGP.Text = Dr("OldSalary")
