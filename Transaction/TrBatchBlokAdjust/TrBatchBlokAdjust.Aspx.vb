...
Partial Class TrBatchBlokAdjust
    Inherits System.Web.UI.Page


    Protected con As New SqlConnection
    Protected da As New SqlDataAdapter
    Protected GetStringHd As String = "Select * From V_PLBatchBlockAdjustHd"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            If Not IsPostBack Then
                ...
                tbTanamDt.Attributes.Add("OnKeyDown", "return PressNumeric();")
            ...
            If Not Session("Result") Is Nothing Then
                If ViewState("Sender") = "btnBatchHD" Then
                    ...
                    ddlDivision.SelectedValue = Session("Result")(1).ToString
                    tbGood.Text = Session("Result")(2).ToString
                    tbTanamHidden.Text = Session("Result")(3).ToString
                    'tbTbm.Text = CInt(CFloat(tbTanam.Text))
                    tbTbm.Text = CInt(CFloat(tbTanamHidden.Text))
                ...
                If ViewState("Sender") = "btnBatch" Then
                    ...
                    tbBlockName.Text = Session("Result")(1).ToString
                    tbUse.Text = Session("Result")(2).ToString
                    tbMaxCap.Text = Session("Result")(3).ToString
                    tbTbm.Text = CInt(CFloat(tbTanamHidden.Text))
                    tbSaldoCap.Text = CInt(CFloat(tbMaxCap.Text) - CFloat(tbUse.Text) - CFloat(tbTanamDt.Text))
                    tbTanamDt.Text = "0"
    ...
    Private Sub SetInit()
        FillRange(ddlRange)
        FillCombo(ddlDivision, "EXEC S_GetDivision", True, "Division_Code", "Division_Name", ViewState("DBConnection"))
        'FillCombo(ddlDivision, "EXEC S_GetTeamNew", True, "Division", "DivisionName", ViewState("DBConnection"))
        'FillCombo(tbBatch, "SELECT * FROM V_PLTransferModuleGetBatch", False, "Batch_No", "BatchName", ViewState("DBConnection"))
    ...
    Private Function GetStringDt(ByVal Nmbr As String) As String
        Return "SELECT TransNmbr, Block, BlockName, QtyMax, QtyUse, QtyTanam, QtySaldo, QtyTBM, Remark FROM V_PLBatchBlockAdjustDt WHERE TransNmbr = " + QuotedStr(Nmbr)
    End Function
            ...
            Else
                ...
                For j = 0 To (Nmbr.Length - 1)
                    If Nmbr(j) = "" Then
                        Exit For
                    Else
                        Result = ExecSPCommandGo(ActionValue, "S_PLBatchBlockAdjust", Nmbr(j), CInt(Session(Request.QueryString("KeyId"))("Year")), CInt(Session(Request.QueryString("KeyId"))("Period")), ViewState("UserId").ToString, ViewState("DBConnection").ToString)
                        If Trim(Result) <> "" Then
    ...
    Protected Sub btnSaveDt_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnSaveDt.Click
        Try
            If ViewState("StateDt") = "Edit" Then
                ...
                Row("QtyTanam") = tbTanamDt.Text
                Row("QtySaldo") = tbSaldoCap.Text
                Row("QtyTBM") = tbTbm.Text
            ...
            Else
                ...
                dr("QtyTanam") = tbTanamDt.Text
                dr("QtySaldo") = tbSaldoCap.Text
                dr("QtyTBM") = tbTbm.Text
    ...
    Private Sub SaveAll()
        ...
        Try
            ...
            If ViewState("StateSave") = "Insert" Then
                tbRef.Text = GetAutoNmbr("BBA", "Y", Year(tbDate.SelectedDate), Month(tbDate.SelectedDate), "", ViewState("DBConnection").ToString)

                SQLString = "INSERT INTO PLBatchBlockAdjustHd (TransNmbr, Status, BatchNo, QtyTanam, QtyGood, TransDate, Remark, UserPrep, DatePrep, Division) " + _
                "SELECT " + QuotedStr(tbRef.Text) + " , 'H', " + QuotedStr(tbBatch.Text) + " , " + _
                 tbTanam.Text.Replace(",", "") + ", " + tbGood.Text.Replace(",", "") + ", '" + Format(tbDate.SelectedDate, "yyyy-MM-dd") + "' ," + QuotedStr(tbRemark.Text) + ",  " + _
                 QuotedStr(ViewState("UserId").ToString) + " , getDate(), " + QuotedStr(ddlDivision.SelectedValue)

            Else
                Dim cekStatus As String
                cekStatus = SQLExecuteScalar("Select Status FROM PLBatchBlockAdjustHd WHERE TransNmbr = " + QuotedStr(tbRef.Text), ViewState("DBConnection").ToString)
                If cekStatus = "P" Then
                    lbStatus.Text = MessageDlg("Save failed... Data has already posted by another user")
                    Exit Sub
                End If
                SQLString = "UPDATE PLBatchBlockAdjustHd SET BatchNo = " + QuotedStr(tbBatch.Text) + " , Remark = " + QuotedStr(tbRemark.Text) + "," + _
                " QtyGood = " + tbGood.Text.Replace(",", "") + ",  QtyTanam = " + tbTanam.Text.Replace(",", "") + ", Division = " + QuotedStr(ddlDivision.SelectedValue) + ", " + _
            ...
            con.Open()
            Dim cmdSql As New SqlCommand("SELECT TransNmbr, Block, QtyMax, QtyUse, QtyTanam, QtySaldo, QtyTBM, Remark FROM PLBatchBlockAdjustDt WHERE TransNmbr = '" & ViewState("TransNmbr") & "'", con)
            ...
            Dim Update_Command = New SqlCommand("UPDATE PLBatchBlockAdjustDt SET Block = @Block, QtyMax = @QtyMax, QtyUse = @QtyUse, " + _
            "QtyTanam = @QtyTanam, QtySaldo = @QtySaldo, QtyTBM = @QtyTBM, Remark = @Remark WHERE TransNmbr = '" & ViewState("TransNmbr") & "' AND Block = @OldBlock", con)
            ' Define output parameters.
            Update_Command.Parameters.Add("@Block", SqlDbType.VarChar, 20, "Block")
            Update_Command.Parameters.Add("@QtyMax", SqlDbType.Int, 4, "QtyMax")
            Update_Command.Parameters.Add("@QtyUse", SqlDbType.Int, 4, "QtyUse")
            Update_Command.Parameters.Add("@QtyTanam", SqlDbType.Int, 4, "QtyTanam")
            Update_Command.Parameters.Add("@QtySaldo", SqlDbType.Int, 4, "QtySaldo")
            Update_Command.Parameters.Add("@QtyTBM", SqlDbType.Int, 4, "QtyTBM")
            Update_Command.Parameters.Add("@Remark", SqlDbType.VarChar, 60, "Remark")

            ' Define intput (WHERE) parameters.
            param = Update_Command.Parameters.Add("@OldBlock", SqlDbType.VarChar, 20, "Block")
            param.SourceVersion = DataRowVersion.Original
            da.UpdateCommand = Update_Command

            ' Create the DeleteCommand.
            Dim Delete_Command = New SqlCommand( _
            "DELETE FROM PLBatchBlockAdjustDt WHERE TransNmbr = '" & ViewState("TransNmbr") & "' AND Block = @Block ", con)
            ' Add the parameters for the DeleteCommand.
            param = Delete_Command.Parameters.Add("@Block", SqlDbType.VarChar, 20, "Block")
            param.SourceVersion = DataRowVersion.Original
            da.DeleteCommand = Delete_Command

            Dim Dt As New DataTable("PLBatchBlockAdjustDt")
    ...
    Protected Sub btnBatchHD_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnBatchHD.Click
        Dim ResultField As String
        Try
            Session("filter") = " SELECT DISTINCT BatchNo, BatchDate, Varietas, Division, QtyPN, Qty_TBM, Type FROM V_PLBatchBlockAdjustGetBatch"
            ResultField = "BatchNo, Division, QtyPN, Qty_TBM "
    ...
    Function CekDt(Optional ByVal Dr As DataRow = Nothing) As Boolean
        Try
            If Not (Dr Is Nothing) Then
                ...
                If (Dr("QtyTBM").ToString = "0") Then
                    lbStatus.Text = MessageDlg("QtyTBM " + Dr("QtyTBM").ToString + " must have value Qty TBM")
                    Return False
                End If

            Else
                If tbBlockName.Text.Trim = "0" Then
                    lbStatus.Text = MessageDlg("Block Name Must Have Value")
                    tbBlockName.Focus()
                    Return False
                End If
                ...
                If ((tbTanamDt.Text.Trim) = "") Then
                    lbStatus.Text = MessageDlg("Tanam Tone must have value")
                    tbTanamDt.Focus()
                    Return False
                End If
                ...
                If (tbTbm.Text.Trim) = "" Then
                    lbStatus.Text = MessageDlg("TBM Adjust must have value")
                    tbTbm.Focus()
                    Return False
                End If
    ...
    Protected Sub LbAdvSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles LbAdvSearch.Click
        ...
        Try
            ...
            FilterName = " Date, Division, BatchNo, Remark"
            FilterValue = " Division, BatchNo, QtGood, QtyTanam, TransNmbr, Status, Format_Name, Remark"
    ...
    Protected Sub tbBatch_TextChanged(ByVal sender As Object, ByVal e As EventArgs) Handles tbBatch.TextChanged
        ...
        Dim Dr As DataRow
        Try
            Dt = SQLExecuteQuery("SELECT Block_Code, Block_Name, Division, QtyUse, MaxCap FROM V_PLTransferBatchGetBlock = Block_Code " + QuotedStr(tbBlock.Text), ViewState("DBConnection").ToString).Tables(0)
            If Dt.Rows.Count > 0 Then
                Dr = Dt.Rows(0)
                'tbLastSensus.Text = FormatFloat(Dr("QtySaldo").ToString, ViewState("DigitQty"))
                tbBlock.Text = CInt(Dr("Block_Code").ToString)
                tbBlockName.Text = CInt(Dr("Block_Name").ToString)
                tbMaxCap.Text = CInt(Dr("MaxCap").ToString)
                tbUse.Text = CInt(Dr("QtyUse").ToString)
                ddlDivision.SelectedValue = CInt(Dr("Division").ToString)
                'tbSaldoCap.Text = "0"
                'tbSaldo.Text = CInt(Dr("QtySaldo").ToString)
            End If
        Catch ex As Exception
    ...
    Protected Sub FillTextBoxHd(ByVal Nmbr As String)
        ...
        Try
            ...
            BindToText(tbGood, Dt.Rows(0)("QtyGood").ToString)
            BindToText(tbTanam, Dt.Rows(0)("QtyTanam").ToString)
            BindToText(tbRemark, Dt.Rows(0)("Remark").ToString)
            Type = SQLExecuteScalar("Select Block_Name From V_PLTransferBatchGetBlock WHERE Block_Code = " + QuotedStr(tbBlock.Text), ViewState("DBConnection").ToString)
    ...
    Protected Sub FillTextBoxDt(ByVal Modul As String)
        Dim Dr As DataRow()
        Try
            Dr = ViewState("Dt").select("Block = " + QuotedStr(Modul))
            If Dr.Length > 0 Then
                BindToText(tbBlock, Dr(0)("Block").ToString)
                BindToText(tbBlockName, Dr(0)("BlockName").ToString)
                BindToText(tbMaxCap, Dr(0)("QtyMax").ToString)
                BindToText(tbUse, Dr(0)("QtyUse").ToString)
                BindToText(tbTanamDt, Dr(0)("QtyTanam").ToString)
                BindToText(tbSaldoCap, Dr(0)("QtySaldo").ToString)
                BindToText(tbTbm, Dr(0)("QtyTBM").ToString)
    ...
    Protected Sub tbTanamDt_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbTanamDt.TextChanged, tbMaxCap.TextChanged, tbUse.TextChanged, tbSaldoCap.TextChanged, tbTbm.TextChanged
        Try

            tbTanam.Text = CInt(CFloat(tbTanamDt.Text))
            tbSaldoCap.Text = CFloat(tbMaxCap.Text) - CFloat(tbUse.Text) - CFloat(tbTanamDt.Text)
