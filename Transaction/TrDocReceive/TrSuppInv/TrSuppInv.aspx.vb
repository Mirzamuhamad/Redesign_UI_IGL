...
Partial Class SuppInv
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            If Not IsPostBack Then
                ...
                lbCount.Text = SQLExecuteScalar("SELECT COUNT(PO_No) FROM V_FNSuppInvGetRR ", ViewState("DBConnection").ToString)
            ...
            If Not Session("Result") Is Nothing Then
                ...
                If ViewState("Sender") = "btnDPNo" Then
                    ...
                    ViewState("DigitCurr") = SQLExecuteScalar("Select Digit FROM VMsCurrency WHERE Currency = " + QuotedStr(tbDPCurrency.Text), ViewState("DBConnection"))
                    ...
                    tbDPBase.Text = FormatFloat(Session("Result")(5).ToString, ViewState("DigitCurr"))
                    tbDPPPn.Text = FormatFloat(Session("Result")(6).ToString, ViewState("DigitCurr"))
                    tbDPTotal.Text = FormatFloat(Session("Result")(7).ToString, ViewState("DigitCurr"))
                    tbPaidBase.Text = FormatFloat(Session("Result")(8).ToString, ViewState("DigitCurr"))
                    tbPaidPPN.Text = FormatFloat(Session("Result")(9).ToString, ViewState("DigitCurr"))
                    tbPaidTotal.Text = FormatFloat(Session("Result")(10).ToString, ViewState("DigitCurr"))
                    tbDPBaseForex.Text = FormatFloat(CFloat(tbDPBase.Text) - CFloat(tbPaidBase.Text), ViewState("DigitCurr"))
                    tbDPPPnForex.Text = FormatFloat(CFloat(tbDPPPn.Text) - CFloat(tbPaidPPN.Text), ViewState("DigitCurr"))
                    tbDPTotalForex.Text = FormatFloat(CFloat(tbDPBaseForex.Text) + CFloat(tbDPPPnForex.Text), ViewState("DigitCurr"))
                End If
    ...
    Private Sub SetInit()
        ...
        ' ''Me.tbDisc.Attributes.Add("OnBlur", "BaseDiscPPnPPhOtherTotalHd(" + Me.tbBaseForex.ClientID + "," + tbDiscForex.ClientID + "," + Me.tbPPN.ClientID + "," + Me.tbPPNForex.ClientID + "," + tbPPhForex.ClientID + "," + tbOtherForex.ClientID + "," + Me.tbTotalForex.ClientID + ",'%'); setformat();")
        ...
        Try
            ...
            If StringRR.Length >= 4 Then
                ...
                For Each drresult In dt.Rows
                    ...
                    ViewState("DigitCurr") = SQLExecuteScalar("Select Digit FROM VMsCurrency WHERE Currency = " + QuotedStr(drresult("Currency")), ViewState("DBConnection"))
                    ...
                    drNew("Remark") = ""
                    drNew("DPBase") = FormatFloat(CFloat(drresult("BaseForex")), ViewState("DigitCurr"))
                    drNew("DPPPn") = FormatFloat(CFloat(drresult("PPnForex")), ViewState("DigitCurr"))
                    drNew("DPTotal") = FormatFloat(CFloat(drresult("BaseForex")) + CFloat(drresult("PPnForex")), ViewState("DigitCurr"))
                    drNew("PaidBase") = FormatFloat(CFloat(drresult("BasePaid")), ViewState("DigitCurr"))
                    drNew("PaidPPn") = FormatFloat(CFloat(drresult("PPnPaid")), ViewState("DigitCurr"))
                    drNew("PaidTotal") = FormatFloat(CFloat(drresult("BasePaid")) + CFloat(drresult("PPnPaid")), ViewState("DigitCurr"))
                    drNew("BaseForex") = FormatFloat(CFloat(drresult("BaseToPaid")), ViewState("DigitCurr"))
                    drNew("PPnForex") = FormatFloat(CFloat(drresult("PPnToPaid")), ViewState("DigitCurr"))
                    drNew("TotalForex") = FormatFloat(CFloat(drresult("BaseToPaid")) + CFloat(drresult("PPnToPaid")), ViewState("DigitCurr"))
        ...
        Try
            ...
            For j = 0 To (Nmbr.Length - 1)
                If Nmbr(j) = "" Then
                    Exit For
                Else
                    Result = ExecSPCommandGo(ActionValue, "S_FNSuppInv", Nmbr(j), CInt(Session(Request.QueryString("KeyId"))("Year")), CInt(Session(Request.QueryString("KeyId"))("Period")), ViewState("UserId").ToString, ViewState("DBConnection").ToString)
                    If Trim(Result) <> "" Then
    ...
    Private Sub SaveAll()
        ...
        Try
            ...
            If ViewState("StateSave") = "Insert" Then
                ...
                tbRef.Text = GetAutoNmbr("SIN", "Y", Year(tbDate.SelectedValue), Month(tbDate.SelectedValue), "", ViewState("DBConnection").ToString)

                SQLString = "INSERT INTO FINSuppInvHd (TransNmbr, FgReport, Status, TransDate, Supplier, Attn, PONo, " + _
            ...
            Else
                ...
                cekStatus = SQLExecuteScalar("Select Status FROM FINSuppInvHd WHERE TransNmbr = " + QuotedStr(tbRef.Text), ViewState("DBConnection").ToString)
                If cekStatus = "P" Then
                    lbStatus.Text = MessageDlg("Save failed... Data has already posted by another user")
                    Exit Sub
                End If
                SQLString = "UPDATE FINSuppInvHd SET FgReport = " + QuotedStr(ViewState("FgReport").ToString) + ", Supplier = " + QuotedStr(tbSuppCode.Text) + ", Attn = " + QuotedStr(tbAttn.Text) + _
            ...
            con.Open()
            Dim cmdSql As New SqlCommand("SELECT Transnmbr, Product, ReffType, ReffNmbr, CostCtr, QtyOrder, UnitOrder, Qty, Unit, PriceForex, BrutoForex, Disc, DiscForex, NettoForex, PPh, PPhForex, Remark FROM FINSuppInvDt WHERE TransNmbr = '" & ViewState("TransNmbr") & "'", con)
            ...
            "UPDATE FINSuppInvDt SET ReffType = @ReffType, ReffNmbr = @ReffNmbr, " + _
            ...
            Dim Delete_Command = New SqlCommand( _
                "DELETE FROM FINSuppInvDt WHERE TransNmbr = '" & ViewState("TransNmbr") & "' AND ReffType = @ReffType AND ReffNmbr = @ReffNmbr AND CostCtr = @CostCtr AND Product = @Product ", con)
            ...
            Dim Dt As New DataTable("FINSuppInvDt")
            ...
            'save dt2
            Dim cmdSql2 As New SqlCommand("SELECT  TransNmbr, DPNo, Currency, ForexRate, PPn, PPnRate, DPBase, DPPPn, DPTotal, PaidBase, PaidPPN, PaidTotal, BaseForex, PPnForex, TotalForex, Remark FROM FINSuppInvDP WHERE TransNmbr = '" & ViewState("TransNmbr") & "'", con)
            ...
            "UPDATE FINSuppInvDP SET DPNo = @DPNo, Currency = @Currency, " + _
            ...
            Dim Delete_Command2 = New SqlCommand( _
                "DELETE FROM FINSuppInvDP WHERE TransNmbr = '" & ViewState("TransNmbr") & "' AND DPNo = @DPNo ", con)
            ' Add the parameters for the DeleteCommand.
            param2 = Delete_Command2.Parameters.Add("@DPNo", SqlDbType.VarChar, 20, "DPNo")
            param.SourceVersion = DataRowVersion.Original
            da.DeleteCommand = Delete_Command2

            Dim Dt2 As New DataTable("FINSuppInvDP")
            Dt2 = ViewState("Dt2")
            da.Update(Dt2)
            Dt2.AcceptChanges()
            ViewState("Dt2") = Dt2
    ...
    Private Sub ClearDP()
        Try
            ...
            tbDPPPnPercent.Text = FormatFloat("2", ViewState("DigitPercent"))
            tbDPBase.Text = FormatFloat("2", ViewState("DigitHome"))
            tbDPPPn.Text = FormatFloat("2", ViewState("DigitHome"))
            tbDPTotal.Text = FormatFloat("2", ViewState("DigitHome"))
            tbPaidBase.Text = FormatFloat("2", ViewState("DigitHome"))
            tbPaidPPN.Text = FormatFloat("2", ViewState("DigitHome"))
            tbPaidTotal.Text = FormatFloat("2", ViewState("DigitHome"))
            tbDPBaseForex.Text = FormatFloat("2", ViewState("DigitHome"))
            tbDPPPnForex.Text = FormatFloat("2", ViewState("DigitHome"))
            tbDPTotalForex.Text = FormatFloat("2", ViewState("DigitHome"))
            tbDPRemarkDt.Text = ""
        Catch ex As Exception
            ...
            If e.CommandName = "Go" Then
                ...
                ElseIf DDL.SelectedValue = "Print" Then
                    Try
                        ...
                        End If
                        Session("DBConnection") = ViewState("DBConnection")
                        Session("SelectCommand") = "EXEC S_FNFormSuppInv " + QuotedStr(GVR.Cells(2).Text)
                        Session("ReportFile") = ".../../../Rpt/FormSuppInv.frx"
    ...
    Protected Sub btnGetData_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnGetData.Click
        ...
        Try
            ...
            Session("Filter") = "EXEC S_FNSuppInvGetRR " + QuotedStr(Filter)
            ResultField = "RR_Type, RR_No, Report, Supplier_Code, Supplier_Name, Attn, PO_No, Term, Currency, Product_Code, Product_Name, Cost_Ctr, Qty_Order, Unit_Order, Qty, Unit, Price_Forex, Bruto_Forex, Disc, Disc_Forex, PPn, PPn_Forex, PPh, PPh_Forex, Netto_Forex, FgPriceIncludePPN, RemarkHd"
            CriteriaField = "RR_Type, RR_No, RR_Date, Report, Supplier_Code, Supplier_Name,	Attn,	PO_No,	Term, Currency, Supplier_SJ_No,	Supplier_SJ_Date "
    ...
    Protected Sub tbDate_SelectionChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbDate.SelectionChanged
        Try
            ...
            Dim cPPN As String = SQLExecuteScalar(sql, ViewState("DBConnection").ToString)
																				...
																				if ddlFgPriceInclude.selectedValue = "Y" Then
    ...
    Protected Sub btnSaveDt2_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSaveDt2.Click
        Try
            ...
            If ViewState("StateDt2") = "Edit" Then
                ...
                Row("DPBase") = FormatFloat(tbDPBase.Text, ViewState("DigitCurr"))
                Row("DPPPN") = FormatFloat(tbDPPPn.Text, ViewState("DigitCurr"))
                Row("DPTotal") = FormatFloat(tbDPTotal.Text, ViewState("DigitCurr"))
                Row("PaidBase") = FormatFloat(tbPaidBase.Text, ViewState("DigitCurr"))
                Row("PaidPPN") = FormatFloat(tbPaidPPN.Text, ViewState("DigitCurr"))
                Row("PaidTotal") = FormatFloat(tbPaidTotal.Text, ViewState("DigitCurr"))
                Row("BaseForex") = FormatFloat(tbDPBaseForex.Text, ViewState("DigitCurr"))
                Row("PPNForex") = FormatFloat(tbDPPPnForex.Text, ViewState("DigitCurr"))
                Row("TotalForex") = FormatFloat(tbDPTotalForex.Text, ViewState("DigitCurr"))
            ...
            Else
                ...
                dr("DPBase") = FormatFloat(tbDPBase.Text, ViewState("DigitCurr"))
                dr("DPPPN") = FormatFloat(tbDPPPn.Text, ViewState("DigitCurr"))
                dr("DPTotal") = FormatFloat(tbDPTotal.Text, ViewState("DigitCurr"))
                dr("PaidBase") = FormatFloat(tbPaidBase.Text, ViewState("DigitCurr"))
                dr("PaidPPN") = FormatFloat(tbPaidPPN.Text, ViewState("DigitCurr"))
                dr("PaidTotal") = FormatFloat(tbPaidTotal.Text, ViewState("DigitCurr"))
                dr("BaseForex") = FormatFloat(tbDPBaseForex.Text, ViewState("DigitCurr"))
                dr("PPNForex") = FormatFloat(tbDPPPnForex.Text, ViewState("DigitCurr"))
                dr("TotalForex") = FormatFloat(tbDPTotalForex.Text, ViewState("DigitCurr"))
    ...
    Protected Sub GridDt2_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridDt2.RowDataBound
        Try
            If Not IsDBNull(DataBinder.Eval(e.Row.DataItem, "DPNo")) Then
                ...
                ElseIf e.Row.RowType = DataControlRowType.Footer Then
                    tbDPForex.Text = FormatNumber(TotalDP, ViewState("DigitCurr"))
                End If
                AttachScript("setformat();", Page, Me.GetType())
            End If
        Catch ex As Exception
