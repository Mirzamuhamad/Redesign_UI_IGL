...
Partial Class Transaction_TrCIPLicenAdmInv_TrCIPLicenAdmInv
    ...
    Protected GetStringHd As String = "SELECT DISTINCT * FROM V_FINCIPInstrJobHd"
    ...
    Private Sub SetInit()
        ...
        Try
            ...
            End If
            lbTitle.Text = "Job Request" 'Request.QueryString("MenuName").ToString
    ...
    Protected Sub BtnGo_Click(ByVal sender As Object, ByVal e As EventArgs) Handles BtnGo.Click, btnGo2.Click
        ...
        Try
            ...
            If (ActionValue = "Print") Or (ActionValue = "Print Full") Then
                ...
                Session("SelectCommand") = "EXEC S_FNFormJobRequest" + Result + "," + QuotedStr(ViewState("UserId"))
                Session("ReportFile") = ".../../../Rpt/FormJobRequest.frx"
            ...
            Else
                ...
                For j = 0 To (Nmbr.Length - 1)
                    If Nmbr(j) = "" Then
                        Exit For
                    Else
                        Result = ExecSPCommandGo(ActionValue, "S_FINCIPInstrJob", Nmbr(j), CInt(Session(Request.QueryString("KeyId"))("Year")), CInt(Session(Request.QueryString("KeyId"))("Period")), ViewState("UserId").ToString, ViewState("DBConnection").ToString)
                        If Trim(Result) <> "" Then
    ...
    Function CekHd() As Boolean
        Try
            ...
            If tbAreaCode.Text.Trim = "" Then
                lbStatus.Text = MessageDlg("Job Area Code must have value")
                tbAreaCode.Focus()
                Return False
            End If
            If tbAreaName.Text.Trim = "" Then
                lbStatus.Text = MessageDlg("Job Area Name must have value")
                tbAreaName.Focus()
                Return False
            End If
            If tbPackJob.Text.Trim = "" Then
                lbStatus.Text = MessageDlg("Paket Pekerjaan must have value")
                tbPackJob.Focus()
                Return False
            End If
    ...
    Protected Sub FillTextBoxHd(ByVal Nmbr As String)
        ...
        Try
            ...
            BindToText(tbPackJob, Dt.Rows(0)("PackJob").ToString)
    ...
    Private Sub SaveAll()
        ...
        Try
            ...
            If ViewState("StateSave") = "Insert" Then
                ...
                tbCode.Text = GetAutoNmbr("IP", "Y", Year(tbDate.SelectedValue), Month(tbDate.SelectedValue), "", ViewState("DBConnection").ToString)

                SQLString = "INSERT INTO FINCIPInstrJobHd(TransNmbr,TransDate,Status,AreaCode,PackJob,BaseForex,TimeJobDate,StartJobDate,FinishJobDate,Durasi,Remark,FgReport,FgActive,UserPrep,DatePrep) " + _
                "SELECT " + QuotedStr(tbCode.Text) + ", " + QuotedStr(Format(tbDate.SelectedValue, "yyyy-MM-dd")) + ", 'H', " + QuotedStr(tbAreaCode.Text) + ", " + _
                QuotedStr(tbPackJob.Text) + ", " + QuotedStr(tbBaseForex.Text.Replace(",", "")) + ", " + QuotedStr(Format(tbStartJobDate.SelectedValue, "yyyy-MM-dd")) + ", " + _
                QuotedStr(Format(tbStartJobDate.SelectedValue, "yyyy-MM-dd")) + ", " + QuotedStr(Format(tbFinishJobDate.SelectedValue, "yyyy-MM-dd")) + ", " + _
                QuotedStr(tbDurasi.Text.Replace(",", "")) + ", " + QuotedStr(tbRemark.Text) + ", " + QuotedStr("Y") + ", " + QuotedStr("Y") + "," + _
                QuotedStr(ViewState("UserId").ToString) + ", GetDate()"
                ViewState("TransNmbr") = tbCode.Text     'QuotedStr(Format(tbTimeJobDate.SelectedValue, "yyyy-MM-dd"))
            Else
                Dim cekStatus As String
                cekStatus = SQLExecuteScalar("SELECT Status FROM FINCIPInstrJobHd WHERE TransNmbr = " + QuotedStr(tbCode.Text), ViewState("DBConnection").ToString)
                If cekStatus = "P" Then
                    lbStatus.Text = MessageDlg("Save failed... Data has already posted by another user")
                    Exit Sub
                End If
                SQLString = "UPDATE FINCIPInstrJobHd SET TransDate = " + QuotedStr(Format(tbDate.SelectedValue, "yyyy-MM-dd")) + ", AreaCode =" + QuotedStr(tbAreaCode.Text) + _
                ", PackJob =" + QuotedStr(tbPackJob.Text) + ", BaseForex =" + QuotedStr(tbBaseForex.Text.Replace(",", "")) + _
                ", TimeJobDate =" + QuotedStr(Format(tbStartJobDate.SelectedValue, "yyyy-MM-dd")) + _
                ", StartJobDate =" + QuotedStr(Format(tbStartJobDate.SelectedValue, "yyyy-MM-dd")) + _
                ", FinishJobDate =" + QuotedStr(Format(tbFinishJobDate.SelectedValue, "yyyy-MM-dd")) + _
                ", Durasi = " + QuotedStr(tbDurasi.Text.Replace(",", "")) + ", Remark = " + QuotedStr(tbRemark.Text) + _
                ...
                ElseIf DDL.SelectedValue = "Print" Then
                    Try
                        ...
                        If CekMenu <> "" Then
                            ...
                            Exit Sub
                        End If
                        Session("SelectCommand") = "EXEC S_FNFormJobRequest '''" + GVR.Cells(2).Text + "'''," + QuotedStr(ViewState("UserId"))
                        Session("ReportFile") = ".../../../Rpt/FormJobRequest.frx"
                        Session("DBConnection") = ViewState("DBConnection")
                        AttachScript("openprintdlg();", Page, Me.GetType)
                    Catch ex As Exception
    ...
    Protected Sub btnArea_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnArea.Click
        ...
        '    CriteriaField = "AreaCode,AreaName"
        '    ViewState("CriteriaField") = CriteriaField.Split(",")
        '    Session("Column") = ResultField.Split(",")
