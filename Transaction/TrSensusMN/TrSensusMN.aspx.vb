...
Partial Class Transaction_TrWOPLan_TrWOPLan
                    ...
                    For Each drResult In Session("Result").Rows
                        ...
                        If ExistRow.Count = 0 Then
                            ...
                            Else
                                ...
                                End If
                            End If
                            GetMachine(tbJob.Text, tbReffNo.Text, ddlDivisi.Text, ddlReffType.SelectedValue)
                            GetMaterial(ddlReffType.SelectedValue, tbReffNo.Text, tbJob.Text, tbBlock.Text, tbRotasi.Text, drResult("Type").ToString, drResult("PlantPeriodCode").ToString, dr("Qty").ToString, dr("Pokok").ToString, ddlDivisi.Text, ddlReffType.SelectedValue)
    ...
    Protected Sub FillTextBoxHd(ByVal Nmbr As String)
        ...
        Try
            ...
            BindToText(tbQty, Dt.Rows(0)("Qty").ToString)
    ...
    Protected Sub FillTextBoxDt(ByVal RRNo As String, ByVal ItemNo As String)
        ...
        Try
            ...
            If Dr.Length > 0 Then
                ...
                BindToText(tbPercentage, Dr(0)("Percentage").ToString)
                BindToText(tbTotal, Dr(0)("QtyTotal").ToString)
                BindToText(tbQtyWO, Dr(0)("Qty").ToString)
                BindToText(tbQtyCapacity, Dr(0)("NormaHK").ToString)
                BindToText(tbQtyTarget, Dr(0)("QtyTotal").ToString)
                ...
                ElseIf DDL.SelectedValue = "Print" Then
                    Try
                        ...
                        If CekMenu <> "" Then
                            ...
                            Exit Sub
                        End If
                        Session("SelectCommand") = "EXEC S_PLFormWOPlan " + QuotedStr(GVR.Cells(2).Text) + "," + QuotedStr(ViewState("UserId"))
    ...
    Protected Sub btnReff_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnReff.Click
        Dim ResultField As String
        Try
            Session("filter") = "SELECT  TransNmbr, Team, Person, JobPlant, JobPlant_Name, Rotasi, StartDate, EndDate, Qty, Section, RK_Year, RK_Month, Supplier, SuppName From V_PLWOPlanGetDivBlock where Division =" + QuotedStr(ddlDivisi.SelectedValue)
    ...
    Protected Sub btnGetDt_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnGetDt.Click
        ...
        Try
            If ddlReffType.SelectedValue = "Schedule" Then
                Session("filter") = "EXEC S_PLWOPlanReff " + QuotedStr(ddlReffType.SelectedValue) + ", ' And A.TransNmbr = '" + QuotedStr(tbReffNo.Text) + "' And A.JobPlant = '" + QuotedStr(tbJob.Text) + "' And A.Division = '" + QuotedStr(ddlDivisi.SelectedValue) + "' And A.Rotasi = " + tbRotasi.Text + "'"
                ResultField = "Type, DivisiBlokCode, DivisiBlokName, PlantPeriodCode, PlantPeriodName, StartDate, EndDate, Qty, Area, MaxCap"
            Else
    ...
    Protected Sub tbPercentage_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbPercentage.TextChanged
        Try
            ...
            tbQtyWO.Text = (Val(tbPercentage.Text) / 100) * Val(tbQtyTotal.Text)
        Catch ex As Exception
    ...
    Private Sub GetMaterial(ByVal type As String, ByVal reffNo As String, ByVal job As String, ByVal block As String, ByVal rotasi As Integer, ByVal divBlockType As String, ByVal statusTanam As String, ByVal qty As Double, ByVal pkk As Double, ByVal Divisi As String, ByVal ReffType As String)
        ...
        Try
            ...
            For Each drResult In Session("Result").Rows
                ...
                If ExistRow.Count = 0 Then
                    dtUnit = SQLExecuteQuery("EXEC S_PLWOPlanFindRMReff " + QuotedStr(ReffType) + " , " + QuotedStr(reffNo) + " , " + QuotedStr(job) + " , " + QuotedStr(block) + " , " + rotasi + " , " + QuotedStr(divBlockType) + " , " + QuotedStr(statusTanam) + " , " + QuotedStr(qty) + " , " + QuotedStr(pkk), ViewState("DBConnection")).Tables(0)
                    drUnit = dtUnit.Rows(0)
                    Dim dr As DataRow
                    dr = ViewState("Dt4").NewRow
                    dr("Type") = type
