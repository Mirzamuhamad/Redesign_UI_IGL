...
Partial Class Transaction_TrSPTBS_TrSPTBS
    ...
    Private Sub SaveAll()
        ...
        Try
            ...
            If ViewState("StateSave") = "Insert" Then
                ...
                "DateAngkut, JamAngkut, FgTransit, CarNoTransit, OperatorTransit, Remark, UserPrep, DatePrep,  " + _
                "BeratMasuk,BeratKeluar,BeratGross,Grading,GradingKg,BeratNet) " + _
                ...
                QuotedStr(tbCarT.Text) + "," + QuotedStr(tbCarNameT.Text) + "," + QuotedStr(tbRemark.Text) + "," + _
                QuotedStr(ViewState("UserId").ToString) + ", getDate() , " + _
                QuotedStr(tbBrtMasuk.Text) + "," + QuotedStr(tbBrtKeluar.Text) + "," + QuotedStr(tbBrtGross.Text) + "," + QuotedStr(tbGrading.Text) + ", " + _
                QuotedStr(tbGradingKg.Text) + "," + QuotedStr(tbBrtNet.Text) + ""
            Else
    ...
    Private Sub Disable()
        Try
            tbBrtMasuk.Enabled = False
            tbBrtKeluar.Enabled = False
            tbBrtGross.Enabled = False
            tbGrading.Enabled = False
            tbGradingKg.Enabled = False
            tbBrtNet.Enabled = False
            btnCar.Enabled = False
            btnOP.Enabled = False
        Catch ex As Exception
            Throw New Exception("EnabledHd Dt Error " + ex.ToString)
        End Try
    End Sub

    Private Sub Enable()
        Try
            tbBrtMasuk.Enabled = True
            tbBrtKeluar.Enabled = True
            tbGrading.Enabled = True
            tbGradingKg.Enabled = False
            btnCar.Enabled = True
            btnOP.Enabled = True
        Catch ex As Exception
    ...
    Protected Sub FillTextBoxHd(ByVal Nmbr As String)
        ...
        Try
            ...
            BindToText(tbBrtMasuk, FormatNumber(Dt.Rows(0)("BeratMasuk").ToString), 2)
            BindToText(tbBrtKeluar, FormatNumber(Dt.Rows(0)("BeratKeluar").ToString), 2)
            BindToText(tbBrtGross, FormatNumber(Dt.Rows(0)("BeratGross").ToString), 2)
            BindToText(tbGrading, FormatNumber(Dt.Rows(0)("Grading").ToString), 2)
            BindToText(tbGradingKg, FormatNumber(Dt.Rows(0)("GradingKg").ToString), 2)
            BindToText(tbBrtNet, FormatNumber(Dt.Rows(0)("BeratNet").ToString), 2)
            ddlDivision_SelectedIndexChanged(Nothing, Nothing)
    ...
    Protected Sub tbBrtMasuk_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbBrtMasuk.TextChanged, tbBrtKeluar.TextChanged
        Try
            tbBrtGross.Text = Val(tbBrtMasuk.Text) - Val(tbBrtKeluar.Text)
        Catch ex As Exception
            lbStatus.Text = "tbBrtMasuk_TextChanged Error : " + ex.ToString
        End Try
    End Sub

    Protected Sub tbGrading_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbGrading.TextChanged
        Try
            tbGradingKg.Text = Val(tbBrtGross.Text) * (Val(tbGrading.Text) / 100)
            tbBrtNet.Text = Val(tbBrtGross.Text) - Val(tbGradingKg.Text)
            tbGradingKg.Enabled = False
        Catch ex As Exception
            lbStatus.Text = "tbGrading_TextChanged Error : " + ex.ToString
        End Try
    End Sub
End Class
