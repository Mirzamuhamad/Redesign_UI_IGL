...
Partial Class TrARRate
    Inherits System.Web.UI.Page

    Protected con As New SqlConnection
    Protected da As New SqlDataAdapter
    Protected GetStringHd As String = "Select * From V_FNARRateHd "
        ...
        Try
            ...
            If Not Session("Result") Is Nothing Then
                If ViewState("Sender") = "btnInvNo" Then
                    'ResultField = "Invoice_No, Invoice_Date, Customer, Customer_Name, Currency, Forex_Rate, Invoice_Balance, FgValue"
                    BindToText(tbInvNo, Session("Result")(0).ToString)
                    BindToDate(tbInvDate, Session("Result")(1).ToString)
                    BindToText(tbCustCode, Session("Result")(2).ToString)
                    BindToText(tbCustName, Session("Result")(3).ToString)
                    BindToText(tbForexRate, Session("Result")(5).ToString)
                    BindToText(tbAmountForex, Session("Result")(6).ToString)
                    BindToText(tbFgValue, Session("Result")(7).ToString)
                    tbAmountHome.Text = FormatNumber(CFloat(tbForexRate.Text) * CFloat(tbAmountForex.Text), ViewState("DigitHome"))
                    tbNewAmountHome.Text = FormatNumber(CFloat(tbAmountForex.Text) * CFloat(tbRate.Text), ViewState("DigitHome"))
                    tbAdjust.Text = FormatNumber((CFloat(tbNewAmountHome.Text) - CFloat(tbAmountHome.Text)) * CFloat(tbFgValue.Text), ViewState("DigitHome"))
                End If
                If ViewState("Sender") = "btnGetDt" Then
                    ...
                    For Each drResult In Session("Result").Rows
                        'insert
                        If CekExistData(ViewState("Dt"), "InvoiceNo", drResult("Invoice_No")) Then
                            lbStatus.Text = "InvoiceNo " + QuotedStr(drResult("Invoice_No")) + " has been already exist"
                            Exit Sub
                        End If
                        Dim dr As DataRow
                        ...
                        dr("AmountHome") = FormatNumber(CFloat(drResult("Forex_Rate").ToString) * CFloat(drResult("Invoice_Balance").ToString), ViewState("DigitHome"))
                        dr("NewAmountHome") = FormatNumber(CFloat(drResult("Invoice_Balance").ToString) * CFloat(tbRate.Text), ViewState("DigitHome"))
                        dr("AdjustHome") = FormatNumber((dr("NewAmountHome") - dr("AmountHome")) * drResult("FgValue"), ViewState("DigitHome"))
                        dr("FgValue") = drResult("FgValue")
                        dr("InvoiceDate") = drResult("Invoice_Date")
                        dr("Customer") = drResult("Customer")
                        dr("Customer_Name") = drResult("Customer_Name")
    ...
    Private Function GetStringDt(ByVal Nmbr As String) As String
        Return "SELECT * From V_FNARRateDt WHERE TransNmbr = " + QuotedStr(Nmbr)
    End Function
        ...
        Try
            ...
            For j = 0 To (Nmbr.Length - 1)
                If Nmbr(j) = "" Then
                    Exit For
                Else
                    Result = ExecSPCommandGo(ActionValue, "S_FNARRate", Nmbr(j), CInt(Session(Request.QueryString("KeyId"))("Year")), CInt(Session(Request.QueryString("KeyId"))("Period")), ViewState("UserId").ToString, ViewState("DBConnection").ToString)
                    If Trim(Result) <> "" Then
    ...
    Protected Sub btnSaveDt_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSaveDt.Click
        ...
        Try
            If ViewState("StateDt") = "Edit" Then
                If ViewState("DtValue") <> tbInvNo.Text Then
                    If CekExistData(ViewState("Dt"), "InvoiceNo", tbInvNo.Text) Then
                        lbStatus.Text = "InvoiceNo " + QuotedStr(tbInvNo.Text) + " has been already exist"
                        Exit Sub
                    End If
                End If
                Row = ViewState("Dt").Select("InvoiceNo= " + QuotedStr(ViewState("DtValue")))(0)
                If CekDt() = False Then
                    Exit Sub
                End If
                Row.BeginEdit()
                ...
                Row("Customer_Name") = tbCustName.Text
                Row.EndEdit()
            Else
                'Insert
                If CekDt() = False Then
                ...
                If CekExistData(ViewState("Dt"), "InvoiceNo", tbInvNo.Text) = True Then
                    lbStatus.Text = "InvoiceNo " + QuotedStr(tbInvNo.Text) + " has been already exist"
                    Exit Sub
                End If
                ...
                dr("Customer_Name") = tbCustName.Text
                ViewState("Dt").Rows.Add(dr)
            End If
            MovePanel(pnlEditDt, pnlDt)
            EnableHd(GetCountRecord(ViewState("Dt")) = 0)
    ...
    Private Sub SaveAll()
        ...
        Try
            ...
            If ViewState("StateSave") = "Insert" Then
                tbReference.Text = GetAutoNmbr("ARR", ddlReport.SelectedValue, Year(tbDate.SelectedValue), Month(tbDate.SelectedValue), "", ViewState("DBConnection").ToString)
                SQLString = "INSERT INTO FINARRateHd (TransNmbr, Status, TransDate, FgReport, CostCtr, " + _
            ...
            Else
                ...
                CekStatus = SQLExecuteScalar("Select Status FROM FINARRateHd WHERE TransNmbr = " + QuotedStr(tbReference.Text), ViewState("DBConnection").ToString)
                If CekStatus = "P" Then
                    lbStatus.Text = MessageDlg("Save failed... Data has already posted by another user")
                    Exit Sub
                End If
                SQLString = "UPDATE FINARRateHd SET TransDate = '" + Format(tbDate.SelectedValue, "yyyy-MM-dd") + "' " + _
            ...
            con.Open()
            Dim cmdSql As New SqlCommand("SELECT TransNmbr, InvoiceNo, Customer, ForexRate, AmountForex, AmountHome, NewAmountHome, FgValue FROM FINARRateDt WHERE TransNmbr = '" & ViewState("TransNmbr") & "'", con)
            ...
            Dim Dt As New DataTable("FINARRateDt")
    ...
    Protected Sub LbAdvSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles LbAdvSearch.Click
        ...
        Try
            ...
            FilterName = "Reference, Date, Status, Cost Center, Currency, Remark"
            FilterValue = "TransNmbr, dbo.FormatDate(TransDate), Status, CostCtr, Currency, Remark"
        ...
        Try
            ...
            If e.CommandName = "Go" Then
                ...
                ElseIf DDL.SelectedValue = "Print" Then
                    Try
                        Session("SelectCommand") = "EXEC S_FNFormARRate " + QuotedStr(GVR.Cells(2).Text)
                        Session("ReportFile") = ".../../../Rpt/FormFNARRate.frx"
        ...
        Try
            If Not IsDBNull(DataBinder.Eval(e.Row.DataItem, "InvoiceNo")) Then
                ...
                ElseIf e.Row.RowType = DataControlRowType.Footer Then
                    ...
                    e.Row.Cells(5).Text = FormatNumber(AmountF, ViewState("DigitCurr"))
                    e.Row.Cells(6).Text = FormatNumber(AmountH, ViewState("DigitHome"))
                    e.Row.Cells(7).Text = FormatNumber(NewAmountH, ViewState("DigitHome"))
                    e.Row.Cells(8).Text = FormatNumber(Adjust, ViewState("DigitHome"))
                End If
            End If
        Catch ex As Exception
    ...
    Protected Sub GridDt_RowDeleting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewDeleteEventArgs) Handles GridDt.RowDeleting
        ...
        dr = ViewState("Dt").Select("InvoiceNo=" + QuotedStr(GVR.Cells(1).Text))
        dr(0).Delete()
        BindGridDt(ViewState("Dt"), GridDt)
        EnableHd(GetCountRecord(ViewState("Dt")) = 0)
    End Sub
    ...
    Protected Sub FillTextBoxDt(ByVal InvoiceNo As String)
        Dim Dr As DataRow()
        Try
            Dr = ViewState("Dt").select("InvoiceNo= " + QuotedStr(InvoiceNo))
            If Dr.Length > 0 Then
                ...
                BindToText(tbCustName, Dr(0)("Customer_Name").ToString)
    ...
    Protected Sub btnGetDt_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnGetDt.Click
        ...
        Try
            ...
            End If
            Session("Result") = Nothing
            Session("Filter") = "SELECT Invoice_No, Invoice_Date, Customer, Customer_Name, Customer_PO_No, Term, Due_Date, Currency, Forex_Rate, Invoice_Forex, Invoice_Paid, Invoice_Balance, FgValue FROM V_FNARForRate WHERE Currency = " + QuotedStr(ddlCurr.SelectedValue) + _
    ...
    Protected Sub btnInvNo_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnInvNo.Click
        ...
        Try
            Session("Filter") = "SELECT * FROM V_FNARForRate WHERE Currency = " + QuotedStr(ddlCurr.SelectedValue) + _
