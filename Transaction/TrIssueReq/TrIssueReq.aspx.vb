...
Partial Class Transaction_TrIssueReq_TrIssueReq
    Inherits System.Web.UI.Page
    Protected con As New SqlConnection
    Protected da As New SqlDataAdapter
    Protected GetStringHd As String = "Select * From V_STCIssueReqHD"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            If Not IsPostBack Then
                ...
                FillCombo(ddlPurpose, "EXEC S_GetPurposeUse", True, "Purpose_Code", "Purpose_Name", ViewState("DBConnection"))
                SetInit()
            ...
            If Not ViewState("ProductClose") Is Nothing Then
                If HiddenRemarkClose.Value <> "False Value" Then
                    ...
                    sqlstring = "Declare @A VarChar(255) EXEC S_STIssueReqClosing '" + tbRef.Text + "', '" + ViewState("ProductClose").ToString + "','" + HiddenRemarkClose.Value + "'," + QuotedStr(ViewState("UserId")) + ", @A OUT SELECT @A"
                ...
                If HiddenRemarkDelete.Value = "true" Then
                    ...
                    For j = 0 To (Nmbr.Length - 1)
                        ...
                        Else
                            ...
                            Result = ExecSPCommandGo("Delete", "S_STIssueReq", Nmbr(j), CInt(ViewState("GLYear")), CInt(ViewState("GLPeriod")), ViewState("UserId").ToString, ViewState("DBConnection").ToString)
    ...
    Private Function GetStringDt(ByVal Nmbr As String) As String
        Return "SELECT * From V_STCIssueReqDt WHERE TransNmbr = " + QuotedStr(Nmbr)
    End Function
    ...
    Protected Sub BtnGo_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnGo.Click, btnGo2.Click
        ...
        Try
            ...
            If ActionValue = "Print" Then
                ...
                Next
                Result = Result + "'"
                Session("SelectCommand") = "EXEC S_STFormIssueReq" + Result
                'lbStatus.Text = Session("SelectCommand")
                'Exit Sub
                Session("ReportFile") = ".../../../Rpt/FormIssueReq.frx"
            ...
            Else
                ...
                For j = 0 To (Nmbr.Length - 1)
                    If Nmbr(j) = "" Then
                        Exit For
                    Else
                        Result = ExecSPCommandGo(ActionValue, "S_STIssueReq", Nmbr(j), CInt(Session(Request.QueryString("KeyId"))("Year")), CInt(Session(Request.QueryString("KeyId"))("Period")), ViewState("UserId").ToString, ViewState("DBConnection").ToString)
                        If Trim(Result) <> "" Then
                            lbStatus.Text = lbStatus.Text + Result + " <br/>"
    ...
    Private Sub SaveAll()
        ...
        Try
            ...
            If ViewState("StateSave") = "Insert" Then
                tbRef.Text = GetAutoNmbr("IRQ", "Y", Year(tbDate.SelectedValue), Month(tbDate.SelectedValue), "", ViewState("DBConnection").ToString)
                SQLString = "INSERT INTO STCIssueReqHd (TransNmbr, Status, TransDate, Department, RequestBy, RequireDate, Warehouse, " + _
                "Subled, FgSubled, PurposeUse, Remark, UserPrep, DatePrep) " + _
                "SELECT " + QuotedStr(tbRef.Text) + ", 'H', '" + Format(tbDate.SelectedValue, "yyyy-MM-dd") + "', " + _
                QuotedStr(ddlDept.SelectedValue) + "," + QuotedStr(tbRequestBy.Text) + ", '" + Format(tbRequireDate.SelectedValue, "yyyy-MM-dd") + "', " + _
                QuotedStr(ddlWrhs.SelectedValue) + ", " + QuotedStr(tbSubled.Text) + ", " + QuotedStr(tbFgSubled.Text) + "," + _
                QuotedStr(ddlPurpose.SelectedValue) + "," + QuotedStr(tbRemark.Text) + "," + _
                QuotedStr(ViewState("UserId").ToString) + ", getDate()"
            Else
                Dim cekStatus As String
                cekStatus = SQLExecuteScalar("Select Status FROM STCIssueReqHD WHERE TransNmbr = " + QuotedStr(tbRef.Text), ViewState("DBConnection").ToString)
                If cekStatus = "P" Then
                    lbStatus.Text = MessageDlg("Save failed... Data has already posted by another user")
                    Exit Sub
                End If
                SQLString = "UPDATE STCIssueReqHD SET Department = " + QuotedStr(ddlDept.SelectedValue) + _
                ", Requestby = " + QuotedStr(tbRequestBy.Text) + ", PurposeUse = " + QuotedStr(ddlPurpose.SelectedValue) + _
                ...
                ", RequireDate = " + QuotedStr(Format(tbRequireDate.SelectedValue, "yyyy-MM-dd")) + _
            ...
            con.Open()
            Dim cmdSql As New SqlCommand("SELECT TransNmbr, Product, Qty, Unit, Remark FROM STCIssueReqDt WHERE TransNmbr = '" & ViewState("Reference") & "'", con)
            ...
            Dim Update_Command = New SqlCommand( _
                    "UPDATE STCIssueReqDt SET Product = @Product, Qty = @Qty, Unit = @Unit, Remark = @Remark " + _
            ...
            Dim Delete_Command = New SqlCommand( _
                "DELETE FROM STCIssueReqDt WHERE TransNmbr = '" & ViewState("Reference") & "' AND Product = @Product ", con)
            ' Add the parameters for the DeleteCommand.
            param = Delete_Command.Parameters.Add("@Product", SqlDbType.VarChar, 20, "Product")
            param.SourceVersion = DataRowVersion.Original
            da.DeleteCommand = Delete_Command

            Dim Dt As New DataTable("STCIssueReqDt")
    ...
    Private Sub ClearHd()
        Try
            ...
            tbRequireDate.SelectedValue = Today.Date
            tbFgSubled.Text = "N"
            tbSubled.Text = ""
    ...
    Function CekHd() As Boolean
        Try
            ...
            If tbSubled.Text.Trim = "" And tbFgSubled.Text.Trim <> "N" Then
                ...
                Return False
            End If
            If ddlPurpose.SelectedValue = "" Then
                lbStatus.Text = MessageDlg("Purpose must have value")
                ddlPurpose.Focus()
                Return False
            End If
    ...
    Protected Sub LbAdvSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles LbAdvSearch.Click
        ...
        Try
            ...
            FilterName = "Reference, Date, Department Name, Warehouse Name, Remark"
            FilterValue = "Reference, dbo.FormatDate(TransDate), Department_Name, Warehouse_Name, Remark"
    ...
    Protected Sub GridDt_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GridDt.RowCommand
        Try
            ...
            ElseIf e.CommandName = "Closing" Then
                ...
                If ViewState("Status") <> "P" Then
                    lbStatus.Text = MessageDlg("Status Issue Request is not Post, cannot close product")
                    Exit Sub
                End If
    ...
    Protected Sub FillTextBoxHd(ByVal Nmbr As String)
        ...
        Try
            ...
            BindToDate(tbRequireDate, Dt.Rows(0)("RequireDate").ToString)
            ...
            BindToDropList(ddlPurpose, TrimStr(Dt.Rows(0)("Purpose").ToString))
        Catch ex As Exception
    ...
    Protected Sub btn1_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn1.Click
        ...
        Try
            'Session("filter") = "select Product_Code, Product_Name, Specification, Product_Alias, On_Hand, Unit FROM VMsProductNFGUser WHERE UserId = '" + ViewState("UserId").ToString + "'"
            Session("filter") = "EXEC S_STIssueReqReff " '" + QuotedStr(ViewState("UserId").ToString) + ", " + QuotedStr(ddlDept.SelectedValue)
            CriteriaField = "Product_Code, Product_Name, Specification, Specification2, Unit, Unit_Order "
            ResultField = "Product_Code, Product_Name, Specification, Unit"

            ViewState("Sender") = "btn1"
            Session("Column") = ResultField.Split(",")
            Session("DBConnection") = ViewState("DBConnection")
    ...
    Protected Sub tbCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbCode.TextChanged
        ...
        Dim Dr As DataRow
        Try
            Dt = SQLExecuteQuery("select Product_Code, Product_Name, Specification, Unit FROM VMsProductNFGSimpleUser WHERE Product_Code = '" + tbCode.Text + "'", ViewState("DBConnection").ToString).Tables(0)
    ...
    Protected Sub btnGetDt_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnGetDt.Click
        ...
        Try
            ...
            End If
            Session("Result") = Nothing
            '-- 2015-05-26-0171 waktu get data tdk perlu filter berdasarkan user dan dept.
            'Session("filter") = "EXEC S_STIssueReqReff " + QuotedStr(ViewState("UserId").ToString) + ", " + QuotedStr(ddlDept.SelectedValue)
            Session("filter") = "EXEC S_STIssueReqReff "
            CriteriaField = "Product_Code, Product_Name, Specification, Specification2, Unit, Unit_Order"
            ResultField = "Product_Code, Product_Name, Specification, Unit"
            Session("Column") = ResultField.Split(",")
