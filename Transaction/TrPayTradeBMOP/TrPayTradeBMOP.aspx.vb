...
Partial Class Transaction_TrPayTrade_TrPayTrade
    ...
    Protected GetStringHd As String = "Select distinct TransNmbr, Nmbr, TransDate, Status, FgReport, SupplierCode, SupplierName, Supplier, Attn, Currency, TotalPayForexStr, TotalPayForex, TotalPayment, TotalOthers, TotalInvoice, TotalPPh, TotalCharge, TotalKurs, TotalSelisih, TotalDP, Remark From V_FNPayTradeConHd "
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            If Not IsPostBack Then
                ...
                lbCount.Text = SQLExecuteScalar("SELECT * FROM V_CountSiConPosting", ViewState("DBConnection").ToString)
            End If
            ...
            If Not Session("Result") Is Nothing Then
                ...
                ElseIf ViewState("Sender") = "btnGetInv" Then
                    ...
                    For Each drResult In Session("Result").Rows
                        ...
                        If ExistRow.Count = 0 Then
                            ...
                            'dr("lbItemNodt2") = CInt(lbItemNoDt2.Text)
                ...
                If ViewState("Sender") = "btnOut" Then
                    ...
                    For Each drResult In Session("Result").Rows
                        ...
                        If CekExistData(ViewState("Dt2"), "InvoiceNo", drResult("Invoice_No")) = False Then
                            Dim dr As DataRow
                            dr = ViewState("Dt2").NewRow
                            dr("ItemNo") = GetNewItemNo(ViewState("Dt2"))
                            lbItemNo.Text = GetNewItemNo(ViewState("Dt2"))
    ...
    Private Function GetStringDt(ByVal Nmbr As String) As String
        Return "SELECT * From V_FNPayTradeConDt WHERE TransNmbr = " + QuotedStr(Nmbr)
    End Function
    Private Function GetStringDt2(ByVal Nmbr As String) As String
        Return "SELECT * From V_FNPayTradeConInv WHERE TransNmbr = " + QuotedStr(Nmbr)
    End Function
    ...
    Protected Sub BtnGo_Click(ByVal sender As Object, ByVal e As EventArgs) Handles BtnGo.Click, btnGo2.Click
        ...
        Try
            ...
            If (ActionValue = "Print") Or (ActionValue = "Print Full") Then
                ...
                Session("SelectCommand") = "EXEC S_FNFormVoucherCon" + Result + ",'APBO'," + QuotedStr(ViewState("UserId"))
                Session("ReportFile") = ".../../../Rpt/FormVoucherCon.frx"
            ...
            Else
                ...
                For j = 0 To (Nmbr.Length - 1)
                    If Nmbr(j) = "" Then
                        Exit For
                    Else
                        Result = ExecSPCommandGo(ActionValue, "S_FNPayTradeCon", Nmbr(j), CInt(Session(Request.QueryString("KeyId"))("Year")), CInt(Session(Request.QueryString("KeyId"))("Period")), ViewState("UserId").ToString, ViewState("DBConnection").ToString)
                        If Trim(Result) <> "" Then
    ...
    Private Sub SaveAll()
        ...
        Try
            ...
            If ViewState("StateSave") = "Insert" Then
                ...
                tbCode.Text = GetAutoNmbr("APBO", "Y", Year(tbDate.SelectedValue), Month(tbDate.SelectedValue), "", ViewState("DBConnection").ToString)

                'FORMAT(A.TotalCharge, DigitCurrHome()) AS TotalCharge, A.Remark
                SQLString = "INSERT INTO FINPayTradeConHd (TransNmbr, TransDate, STATUS, FgReport, " + _
                ...
                ", " + tbTotalDPForex.Text.Replace(",", "").Replace(".", "") + ", " + tbTotalPaymentForex.Text.Replace(",", "").Replace(".", "") + _
                ", " + tbTotalPayment.Text.Replace(",", "").Replace(".", "") + ", " + tbTotalOther.Text.Replace(",", "").Replace(".", "") + _
                ", " + tbTotalInvoice.Text.Replace(",", "").Replace(".", "") + ", " + tbTotalPPh.Text.Replace(",", "").Replace(".", "") + ", " + tbTotalCharge.Text.Replace(",", "").Replace(".", "") + _
                ", " + tbTotalKurs.Text.Replace(",", "").Replace(".", "") + _
                ", " + tbTotalSelisih.Text.Replace(",", "").Replace(".", "") + ", " + QuotedStr(tbRemark.Text) + "," + _
            ...
            Else
                ...
                cekStatus = SQLExecuteScalar("Select Status FROM FINPayTradeConHd WHERE TransNmbr = " + QuotedStr(tbCode.Text), ViewState("DBConnection").ToString)
                If cekStatus = "P" Then
                    lbStatus.Text = MessageDlg("Save failed... Data has already posted by another user")
                    Exit Sub
                End If
                SQLString = "UPDATE FINPayTradeConHd SET FgReport =" + QuotedStr("Y") + ", Supplier=" + QuotedStr(tbSuppCode.Text) + _
                ", TransDate = " + QuotedStr(Format(tbDate.SelectedValue, "yyyy-MM-dd")) + ", Attn =" + QuotedStr(tbAttn.Text) + _
                ", TotalDP=" + tbTotalDPForex.Text.Replace(",", "").Replace(".", "") + _
                ", TotalPayForex=" + tbTotalPaymentForex.Text.Replace(",", "").Replace(".", "") + _
                ", TotalPayment=" + tbTotalPayment.Text.Replace(",", "").Replace(".", "") + ", TotalOthers =" + tbTotalOther.Text.Replace(",", "").Replace(".", "") + _
                ", TotalInvoice= " + tbTotalInvoice.Text.Replace(",", "").Replace(".", "") + ", TotalPPh= " + tbTotalPPh.Text.Replace(",", "").Replace(".", "") + _
                ", TotalKurs = " + tbTotalKurs.Text.Replace(",", "").Replace(".", "") + _
                ", TotalCharge= " + tbTotalCharge.Text.Replace(",", "").Replace(".", "") + ", TotalSelisih= " + tbTotalSelisih.Text.Replace(",", "").Replace(".", "") + ", Remark = " + QuotedStr(tbRemark.Text) + _
                ", DatePrep = GetDate()" + _
            ...
            Dim cmdSql As New SqlCommand("SELECT TransNmbr, ItemNo, PaymentType, PaymentDate, DocumentNo, Reference, GiroDate, DueDate, BankPayment, Currency, ForexRate, PaymentForex, PaymentHome, Remark, FgMode, FgValue, DPRate FROM FINPayTradeConDt WHERE TransNmbr = " + QuotedStr(ViewState("TransNmbr")), con)
            ...
            "UPDATE FINPayTradeConDt SET ItemNo = @ItemNo, PaymentType = @PaymentType, " + _
            ...
            Dim Delete_Command = New SqlCommand( _
                "DELETE FROM FINPayTradeConDt WHERE TransNmbr = '" & ViewState("TransNmbr") & "' AND ItemNo = @ItemNo ", con)
            ' Add the parameters for the DeleteCommand.
            param = Delete_Command.Parameters.Add("@ItemNo", SqlDbType.Int, 4, "ItemNo")
            param.SourceVersion = DataRowVersion.Original
            da.DeleteCommand = Delete_Command

            Dim Dt As New DataTable("FINPayTradeConDt")
            ...
            If Not ViewState("Dt2") Is Nothing Then
                ...
                Dim Dt2 As New DataTable("FINPayTradeConDtInv")
            ...
            If e.CommandName = "Go" Then
                ...
                ElseIf (DDL.SelectedValue = "Print") Or DDL.SelectedValue = "Print Full" Then
                    Try
                        ...
                        End If
                        Session("DBConnection") = ViewState("DBConnection")
                        Session("SelectCommand") = "EXEC S_FNFormVoucherCon '''" + GVR.Cells(2).Text + "''','APBO'," + QuotedStr(ViewState("UserId"))
                        Session("ReportFile") = ".../../../Rpt/FormVoucherCon.frx"
                        AttachScript("openprintdlg();", Page, Me.GetType)
                    Catch ex As Exception
    ...
    Protected Sub FillTextBoxHd(ByVal Nmbr As String)
        ...
        Try
            ...
            'If ViewState("DigitCurr") = "IDR" Then
    ...
    Protected Sub btnSupp_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnSupp.Click
        ...
        Try
            Session("DBConnection") = ViewState("DBConnection")
            Session("filter") = "SELECT * FROM VMsSupplierAP"
    ...
    Protected Sub btnGetInv_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnGetInv.Click
        ...
        Try
            sqlstring = "EXEC S_FNPayTradeConGetInv '" + tbSuppCode.Text.Trim + "','Y'," + QuotedStr(ViewState("TransNmbr")) '" + ddlReport.SelectedValue.ToString + "
    ...
    Protected Sub lbCount_Click(ByVal sender As Object, ByVal e As EventArgs) Handles lbCount.Click
        Dim ResultField, CriteriaField, sqlstring, ResultSame As String
        Try
            sqlstring = "EXEC S_FNPayTradeConGetInv '','Y',''" '+ ddlReport.SelectedValue.ToString + ""
            'If lbFgModeDt2.Text = "G" Then
            '    If tbSuppCode.Text.Trim = "" Then
            '        'ini diremark, karena product type tidak dipakai lagi
            '        'If tbProductType.Text = "" Then
