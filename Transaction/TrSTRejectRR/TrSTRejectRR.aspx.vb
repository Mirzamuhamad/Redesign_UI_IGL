...
Partial Class TrSTRejectRR
    Inherits System.Web.UI.Page
    Protected con As New SqlConnection
    Protected da As New SqlDataAdapter
    Protected GetStringHd As String = "Select * From V_STRejectRRHd"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            If Not IsPostBack Then
                ...
                lbCount.Text = SQLExecuteScalar("SELECT Count(DISTINCT SJ_No) FROM V_STRejectRRGetProd ", ViewState("DBConnection").ToString)
            ...
            If Not Session("Result") Is Nothing Then
                ...
                If ViewState("Sender") = "btnPO" Then
                    ...
                    End If
                    'SJ_No, Supplier_Code, Supplier_Name, Attn, Report, Warehouse"
    ...
    Private Function GetStringDt(ByVal Nmbr As String) As String
        Return "SELECT * From v_strejectrrDt WHERE TransNmbr = " + QuotedStr(Nmbr) + " "
    End Function
    ...
    Protected Sub BtnGo_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnGo.Click, btnGo2.Click
        ...
        Try
            ...
            If ActionValue = "Print" Then
                ...
                Session("SelectCommand") = "EXEC S_STFormRejectRR " + Result
                'lbStatus.Text = Session("SelectCommand")
                'Exit Sub

                Session("ReportFile") = ".../../../Rpt/FormSTRejectRR.frx"
            ...
            Else
                ...
                For j = 0 To (Nmbr.Length - 1)
                    If Nmbr(j) = "" Then
                        Exit For
                    Else
                        Result = ExecSPCommandGo(ActionValue, "S_strejectrr", Nmbr(j), CInt(ViewState("GLYear")), CInt(ViewState("GLPeriod")), ViewState("UserId").ToString, ViewState("DBConnection").ToString)
                        If Trim(Result) <> "" Then
    ...
    Private Sub SaveAll()
        ...
        Try
            ...
            If ViewState("StateSave") = "Insert" Then
                ...
                tbTransNo.Text = GetAutoNmbr("RJI", "Y", Year(tbDate.SelectedValue), Month(tbDate.SelectedValue), "", ViewState("DBConnection").ToString)

                SQLString = "INSERT INTO STCRejectRRHd (TransNmbr,Status,TransDate,FgReport,Supplier,Attn,SJReject,Warehouse,SubLed,FgSubLed,ReceivedBy,SJSuppNo,SJSuppDate,Remark,UserPrep,DatePrep) " + _
                "SELECT " + QuotedStr(tbTransNo.Text) + ", 'H', '" + Format(tbDate.SelectedValue, "yyyy-MM-dd") + "', " + _
                QuotedStr("Y") + ", " + QuotedStr(tbSuppCode.Text) + ", " + QuotedStr(tbAttn.Text) + ", " + QuotedStr(tbReturNo.Text) + ", " + _
                QuotedStr(ddlWrhs.SelectedValue) + ", " + QuotedStr(tbSubled.Text) + ", " + QuotedStr(tbFgSubled.Text) + "," + QuotedStr(tbReceivedBy.Text) + "," + QuotedStr(tbDNSuppNo.Text) + ", '" + _
                Format(tbDNSuppDate.SelectedValue, "yyyy-MM-dd") + "', " + QuotedStr(tbRemark.Text) + ", " + _
                QuotedStr(ViewState("UserId").ToString) + ", getDate()"
            Else
                Dim cekStatus As String
                cekStatus = SQLExecuteScalar("Select Status FROM STCRejectRRHd WHERE TransNmbr = " + QuotedStr(tbTransNo.Text), ViewState("DBConnection").ToString)
                If cekStatus = "P" Then
                    lbStatus.Text = MessageDlg("Save failed... Data has already posted by another user")
                    Exit Sub
                End If
                SQLString = "UPDATE STCRejectRRHd SET TransDate = " + QuotedStr(Format(tbDate.SelectedValue, "yyyy-MM-dd")) + _
                ", FgReport = " + QuotedStr("Y") + ", Supplier = " + QuotedStr(tbSuppCode.Text) + _
                ", Attn = " + QuotedStr(tbAttn.Text) + ", SJReject = " + QuotedStr(tbReturNo.Text) + _
                ", Warehouse = " + QuotedStr(ddlWrhs.SelectedValue) + ", Subled = " + QuotedStr(tbSubled.Text) + _
                ", ReceivedBy = " + QuotedStr(tbReceivedBy.Text) + ", SJSuppNo = " + QuotedStr(tbDNSuppNo.Text) + _
                ", SJSuppDate = " + QuotedStr(Format(tbDNSuppDate.SelectedValue, "yyyy-MM-dd")) + _
            ...
            con.Open()
            Dim cmdSql As New SqlCommand("SELECT TransNmbr, Product, Qty, Unit, Remark FROM STCRejectRRDt WHERE TransNmbr = '" & ViewState("Reference") & "'", con)
            ...
            Dim Update_Command = New SqlCommand( _
                    "UPDATE stcrejectrrdt SET Product = @Product, Qty = @Qty, Unit = @Unit, Remark = @Remark " + _
            ...
            Dim Delete_Command = New SqlCommand( _
                "DELETE FROM stcrejectrrdt WHERE TransNmbr = '" & ViewState("Reference") & "' AND Product = @Product ", con)
            ' Add the parameters for the DeleteCommand.
            param = Delete_Command.Parameters.Add("@Product", SqlDbType.VarChar, 20, "Product")
            param.SourceVersion = DataRowVersion.Original
            da.DeleteCommand = Delete_Command

            Dim Dt As New DataTable("stcrejectrrdt")
    ...
    Private Sub ClearHd()
        Try
            ...
            tbReceivedBy.Text = ""
            tbDNSuppNo.Text = ""
            tbDNSuppDate.SelectedDate = ViewState("ServerDate") 'Today
        Catch ex As Exception
    ...
    Protected Sub LbAdvSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles LbAdvSearch.Click
        ...
        Try
            ...
            FilterName = "Reference, Date, Status, Supplier, Attention, DN Delivery Retur No., Warehouse, Subled, Received By, DN Supplier No, DN Supplier Date, Fg Report, Remark"
            FilterValue = "TransNmbr, dbo.FormatDate(TransDate), Status, Supplier_Name, Attn, SJReject, Wrhs_Name, Subled_Name, ReceivedBy, SJSuppNo, dbo.FormatDate(SJSuppDate), FgReport, Remark"
            ...
            If e.CommandName = "Go" Then
                ...
                ElseIf DDL.SelectedValue = "Print" Then
                    Try
                        ...
                        End If
                        Session("DBConnection") = ViewState("DBConnection")
                        Session("SelectCommand") = "EXEC S_STFormRejectRR " + "''" + QuotedStr(GVR.Cells(2).Text) + "''"
                        'lbStatus.Text = Session("SelectCommand")
                        'Exit Sub
                        Session("ReportFile") = ".../../../Rpt/FormSTRejectRR.frx"
                        AttachScript("openprintdlg();", Page, Me.GetType)
    ...
    Protected Sub FillTextBoxHd(ByVal Nmbr As String)
        ...
        Try
            ...
            BindToText(tbReturNo, Dt.Rows(0)("SJReject").ToString)
            BindToText(tbRemark, Dt.Rows(0)("Remark").ToString)
            BindToText(tbReceivedBy, Dt.Rows(0)("ReceivedBy").ToString)
            BindToText(tbDNSuppNo, Dt.Rows(0)("SJSuppNo").ToString)
            BindToDate(tbDNSuppDate, Dt.Rows(0)("SJSuppDate").ToString)
    ...
    Protected Sub BtnGetDt_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnGetDt.Click
        ...
        Try
            Session("Result") = Nothing
            Session("DBConnection") = ViewState("DBConnection")
            Session("Filter") = "SELECT Product_Code, Product_Name, Specification, Qty, Unit, Remark FROM V_STRejectRRGetProd WHERE SJ_No = " + QuotedStr(tbReturNo.Text)
    ...
    Protected Sub tbProdCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbProdCode.TextChanged
        ...
        Dim dt As DataTable
        Try
            dt = SQLExecuteQuery("Select Product_Code, Product_Name, Specification, Qty, Unit, Remark from V_STRejectRRGetProd WHERE SJ_No = " + QuotedStr(tbReturNo.Text) + " And Product_Code = " + QuotedStr(tbProdCode.Text), ViewState("DBConnection").ToString).Tables(0)
            If dt.Rows.Count > 0 Then
                Dr = dt.Rows(0)
    ...
    Protected Sub btnSupp_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnSupp.Click
        ...
        Try
            Session("DBConnection") = ViewState("DBConnection")
            Session("filter") = "select Supplier_Code, Supplier_Name, Contact_Person from VMsSupplier"
    ...
    Protected Sub lbCount_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lbCount.Click
        Try
            Session("DBConnection") = ViewState("DBConnection")
            Session("filter") = "SELECT SJ_No, Product_Code, Product_Name, Specification, Qty, Unit, Remark FROM V_STRejectRRGetProd"
