   ...
   <!-- <script type="text/javascript" src="../../JQuery/FathGrid.js"></script> -->
    <script type="text/javascript">   
      ...
      function OpenPopup() {
          ...
          var winOpen = window.open("../../earchDlgV.Aspx", "", 'toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, copyhistory=no, width=' + 600 + ', height=' + 600 + ', top=' + top + ', left=' + left);
          //winOpen.reload(); 
       ...
       /* $(function() {
        ...
        $(document).ready(function() {
           $("#dlgKegiatan tr").click(function(event) {
                ...
                tbNoLandSurvey.value = IDKegiatan
                tbSellerName.value = strKegiatanName
            });
            ClosePopupKegiatan();
        });
...
//----------------------------------------------------------------------------//  
   /*  function Search_GridView(strKey, strGV) {
     ...
     $("[id*=txtSearchKegiatan]").live("keyup", function () {
        GetCustomers(parseInt(1));
     });
    /* 
     function SearchTerm() {
        return jQuery.trim($("[id*=txtSearchKegiatan]").val());
     };
    */
     function GetCustomers(pageIndex) {
        $.ajax({
            type: "POST",
            url: "Transaction_TrLandPurchaseAgremt_TrLandPurchaseAgremt.aspx/GetCustomers",
     ...
     function OnSuccess(response) {
        ...
        var xml = $(xmlDoc);
        var customers = xml.find("V_GLLandPurchaseReqHd");
        if (row == null) {
            row = $("[id*=GVKegiatan] tr:last-child").clone(true);
        }
        $("[id*=GVKegiatan] tr").not($("[id*=GVKegiatan] tr:first-child")).remove();
        if (customers.length > 0) {
            $.each(customers, function () {
                var customer = $(this);
                $("td", row).eq(0).html($(this).find("TransNmbr").text());
                $("td", row).eq(1).html($(this).find("SellName").text());
                $("td", row).eq(2).html($(this).find("City").text()); 
                $("[id*=GVKegiatan]").append(row);
                row = $("[id*=GVKegiatan] tr:last-child").clone(true);
            });
        } else {
            ...
            $("[id*=GVKegiatan]").append(empty_row);
        }
     };        
//----------------------------------------------------------------------------//        
        ...
        <tr>
            ...
            <td><asp:TextBox CssClass="TextBox" runat="server" ID ="tbFilter"/> 
                  <asp:DropDownList CssClass="DropDownList" runat="server" ID="ddlField" >     
                    <asp:ListItem Selected="True" Value="TransNmbr">No.Perjanjian Pembelian</asp:ListItem>
                    <asp:ListItem Value="dbo.FormatDate(TransDate)">Tgl.Perjanjian Pembelian</asp:ListItem>
                    <asp:ListItem Value="Status">Status</asp:ListItem>
                    <asp:ListItem Value="LSNo">No. Land Survey</asp:ListItem>
                    <asp:ListItem Value="SellerCode">Nama Seller </asp:ListItem>
                    <asp:ListItem Value="TypeTJName">Nama Type</asp:ListItem>
      ...
      <table>
        <tr>
            ...
            <td>
              ...
              <asp:Label ID="Label3" runat="server" Visible="True" Text =" Rev : "></asp:Label>
              <asp:Label ID="lbRevisi" runat="server" Visible="True" ></asp:Label>
            </td> 
        ...
        <tr>
            ...
            <td>:</td>
            <td>              
              <asp:TextBox ID="tbSellerCode" runat="server" Visible="false" CssClass="TextBox" Width="60px" ValidationGroup="Input" />
              <asp:TextBox ID="tbSellerName" runat="server" CssClass="TextBox" Width="280px" ValidationGroup="Input" />                         
            </td>            
    ...
    <div id="dlgContainer">
      ...
      <div id="dlgKegiatan" style="display:none;">
       <%-- Search : <asp:TextBox ID="txtSearchKegiatan" runat="server" />--%> 
        Search : <asp:TextBox ID="txtSearchKegiatan" runat="server" CssClass="TextBox" Width="200px" /> 
        <asp:Button ID="btnSearchListLSNo" runat="server" class="bitbtn btnapply" Width="90px" Text="Search" OnClientClick="return Search_GridView(this);" /><%--Gridview-row--%>
        <%--<hr />--%>
        <asp:GridView ID="GVKegiatan" runat="server" CssClass="Grid" AutoGenerateColumns="false"  >  
          ...
          <Columns>
            ...
            <asp:BoundField DataField="TransDate" dataformatstring="{0:dd MMM yyyy}" htmlencode="true" HeaderStyle-Width="80px" SortExpression="TransDate" HeaderText="Date"></asp:BoundField>
            <asp:BoundField DataField="LuasUkur" DataFormatString="{0:#,##0}" ItemStyle-HorizontalAlign="Right" HeaderStyle-Width="80px" HeaderText="Luas (M2)"></asp:BoundField>
            ...
            <asp:BoundField DataField="SellName" HeaderStyle-Width="250px" HeaderText="Seller Name" />            
            <%--<asp:BoundField DataField="LSNo" HeaderStyle-Width="80px" HeaderText="No. LS"></asp:BoundField>
            <asp:BoundField DataField="SellName" HeaderStyle-Width="200px" HeaderText="Pemilik Tanah/Seller"></asp:BoundField>
