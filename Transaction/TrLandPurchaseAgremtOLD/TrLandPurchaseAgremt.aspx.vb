...
Partial Class Transaction_TrLandPurchaseAgremt_TrLandPurchaseAgremt
    ...
    Private Sub BindGridNoLandSurvey(ByVal SearchTerm As String, ByVal PageIndex As Integer, ByVal PageSize As Integer)
        ...
        Try
            'adp = New SqlDataAdapter("S_PRCListNoLandSurvey", con)
            'adp.Fill(dt)
            'If dt.Rows.Count > 0 Then
            '    GVKegiatan.DataSource = dt
            '    GVKegiatan.DataBind()
            'Else
            '    GVKegiatan.DataSource = Nothing
            '    GVKegiatan.DataBind()
            'End If
            '-----------------------------------------------------------------------------'
    ...
    Private Sub GetBindGridKegiatan(ByVal searchBy As String, ByVal searchVal As String)
        Dim conStr As String = ViewState("DBConnection").ToString
        'Using con As New SqlConnection(conStr)
            ...
            Using con As New SqlConnection(conStr)
                Using cmd As New SqlCommand(strSQL, con)
                    Using sda As New SqlDataAdapter()
                        cmd.Connection = con
                        cmd.CommandType = CommandType.StoredProcedure
                        cmd.Parameters.AddWithValue("@SearchTerm", txtSearchKegiatan.Text)
    ...
    Protected Sub btnNoLandSurvey_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNoLandSurvey.Click
        ...
        Try
            ...
            strSQL = "SELECT A.TransNmbr, A.TransDate, A.HrgTanah, A.HrgFix AS [LuasUkur], A.TtlHrgTanah AS [TotalHrgTanah], A.SellCode AS [SellerCode], A.SellName AS [SellerName] " + _
            ...
            ResultField = "TransNmbr, TransDate, HrgTanah, LuasUkur, TotalHrgTanah, SellerCode, SellerName"
            CriteriaField = "TransNmbr, TransDate, HrgTanah, LuasUkur, TotalHrgTanah, SellerCode, SellerName"
            ViewState("CriteriaField") = CriteriaField.Split(",")
            Session("Column") = ResultField.Split(",")
