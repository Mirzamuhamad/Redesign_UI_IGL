...
Partial Class Transaction_TrPayTrade_TrPayTrade
    ...
    Protected GetStringHd As String = "Select distinct TransNmbr, Nmbr, TransDate, Status, FgReport, SupplierCode, SupplierName, Supplier, Attn, Currency, TotalPayForexStr, TotalPayForex, TotalPayment, TotalOthers, TotalInvoice, TotalPPh, TotalCharge, TotalKurs, TotalSelisih, TotalDP, Remark From V_FNPayTradePKSHd "
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            If Not IsPostBack Then
                ...
                lbCount.Text = SQLExecuteScalar("SELECT * FROM V_CountSiPKSPosting", ViewState("DBConnection").ToString)
            End If
    ...
    Private Function GetStringDt(ByVal Nmbr As String) As String
        Return "SELECT * From V_FNPayTradePKSDt WHERE TransNmbr = " + QuotedStr(Nmbr)
    End Function
    Private Function GetStringDt2(ByVal Nmbr As String) As String
        Return "SELECT * From V_FNPayTradePKSInv WHERE TransNmbr = " + QuotedStr(Nmbr)
    End Function
    ...
    Protected Sub BtnGo_Click(ByVal sender As Object, ByVal e As EventArgs) Handles BtnGo.Click, btnGo2.Click
        ...
        Try
            ...
            If (ActionValue = "Print") Or (ActionValue = "Print Full") Then
                ...
                Session("SelectCommand") = "EXEC S_FNFormVoucherPKS" + Result + ",'PPKS'," + QuotedStr(ViewState("UserId"))
                Session("ReportFile") = ".../../../Rpt/FormVoucherPKS.frx"
            ...
            Else
                ...
                For j = 0 To (Nmbr.Length - 1)
                    If Nmbr(j) = "" Then
                        Exit For
                    Else
                        Result = ExecSPCommandGo(ActionValue, "S_FNPayTradePKS", Nmbr(j), CInt(Session(Request.QueryString("KeyId"))("Year")), CInt(Session(Request.QueryString("KeyId"))("Period")), ViewState("UserId").ToString, ViewState("DBConnection").ToString)
                        If Trim(Result) <> "" Then
    ...
    Private Sub SaveAll()
        ...
        Try
            ...
            If ViewState("StateSave") = "Insert" Then
                ...
                tbCode.Text = GetAutoNmbr("PPKS", "Y", Year(tbDate.SelectedValue), Month(tbDate.SelectedValue), "", ViewState("DBConnection").ToString)

                'FORMAT(A.TotalCharge, DigitCurrHome()) AS TotalCharge, A.Remark
                SQLString = "INSERT INTO FINPayTradePKSHd (TransNmbr, TransDate, STATUS, FgReport, " + _
            ...
            Else
                ...
                cekStatus = SQLExecuteScalar("Select Status FROM FINPayTradePKSHd WHERE TransNmbr = " + QuotedStr(tbCode.Text), ViewState("DBConnection").ToString)
                If cekStatus = "P" Then
                    lbStatus.Text = MessageDlg("Save failed... Data has already posted by another user")
                    Exit Sub
                End If
                SQLString = "UPDATE FINPayTradePKSHd SET FgReport =" + QuotedStr("Y") + ", Supplier=" + QuotedStr(tbSuppCode.Text) + _
            ...
            Dim cmdSql As New SqlCommand("SELECT TransNmbr, ItemNo, PaymentType, PaymentDate, DocumentNo, Reference, GiroDate, DueDate, BankPayment, Currency, ForexRate, PaymentForex, PaymentHome, Remark, FgMode, FgValue, DPRate FROM FINPayTradePKSDt WHERE TransNmbr = " + QuotedStr(ViewState("TransNmbr")), con)
            ...
            "UPDATE FINPayTradePKSDt SET ItemNo = @ItemNo, PaymentType = @PaymentType, " + _
            ...
            Dim Delete_Command = New SqlCommand( _
                "DELETE FROM FINPayTradePKSDt WHERE TransNmbr = '" & ViewState("TransNmbr") & "' AND ItemNo = @ItemNo ", con)
            ' Add the parameters for the DeleteCommand.
            param = Delete_Command.Parameters.Add("@ItemNo", SqlDbType.Int, 4, "ItemNo")
            param.SourceVersion = DataRowVersion.Original
            da.DeleteCommand = Delete_Command

            Dim Dt As New DataTable("FINPayTradePKSDt")
            ...
            If Not ViewState("Dt2") Is Nothing Then
                ...
                Dim Dt2 As New DataTable("FINPayTradePKSDtInv")
            ...
            If e.CommandName = "Go" Then
                ...
                ElseIf (DDL.SelectedValue = "Print") Or DDL.SelectedValue = "Print Full" Then
                    Try
                        ...
                        End If
                        Session("DBConnection") = ViewState("DBConnection")
                        Session("SelectCommand") = "EXEC S_FNFormVoucherPKS '''" + GVR.Cells(2).Text + "''','PPKS'," + QuotedStr(ViewState("UserId"))
                        Session("ReportFile") = ".../../../Rpt/FormVoucherPKS.frx"
                        AttachScript("openprintdlg();", Page, Me.GetType)
                    Catch ex As Exception
    ...
    Protected Sub btnGetInv_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnGetInv.Click
        ...
        Try
            sqlstring = "EXEC S_FNPayTradePKSGetInv '" + tbSuppCode.Text.Trim + "','Y'," + QuotedStr(ViewState("TransNmbr")) '" + ddlReport.SelectedValue.ToString + "
    ...
    Protected Sub lbCount_Click(ByVal sender As Object, ByVal e As EventArgs) Handles lbCount.Click
        Dim ResultField, CriteriaField, sqlstring, ResultSame As String
        Try
            sqlstring = "EXEC S_FNPayTradePKSGetInv '','Y',''" '+ ddlReport.SelectedValue.ToString + ""
            'If lbFgModeDt2.Text = "G" Then
            '    If tbSuppCode.Text.Trim = "" Then
            '        'ini diremark, karena product type tidak dipakai lagi
            '        'If tbProductType.Text = "" Then
