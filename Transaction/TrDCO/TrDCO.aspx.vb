...
Partial Class TrDCO
    ...
    Private Function GetStringHd() As String
        Return "Select * From V_PLDCOHd " 'WHERE UserPrep = '" + ViewState("UserId") + "' "
                ...
                If HiddenRemarkDelete.Value = "true" Then
                    ...
                    For j = 0 To (Nmbr.Length - 1)
                        If Nmbr(j) = "" Then
                            Exit For
                        Else
                            Result = ExecSPCommandGo("Delete", "S_PLDCO", Nmbr(j), CInt(ViewState("GLYear")), CInt(ViewState("GLPeriod")), ViewState("UserId").ToString, ViewState("DBConnection").ToString)
    ...
    Protected Sub BtnGo_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnGo.Click, btnGo2.Click
        ...
        Try
            ...
            If ActionValue = "Print" Then
                ...
                Next
                Result = Result + "'"
                Session("SelectCommand") = "EXEC S_PLFormDCO" + Result
                'lbStatus.Text = Session("SelectCommand")
                'Exit Sub
                Session("ReportFile") = ".../../../Rpt/FormDCO.frx"
            ...
            Else
                ...
                For j = 0 To (Nmbr.Length - 1)
                    If Nmbr(j) = "" Then
                        Exit For
                    Else
                        Result = ExecSPCommandGo(ActionValue, "S_PLDCO", Nmbr(j), CInt(Session(Request.QueryString("KeyId"))("Year")), CInt(Session(Request.QueryString("KeyId"))("Period")), ViewState("UserId").ToString, ViewState("DBConnection").ToString)
                        If Trim(Result) <> "" Then
                            lbStatus.Text = lbStatus.Text + Result + " <br/>"
                        End If
                    End If
    ...
    Private Sub SaveAll()
        ...
        Try
            ...
            If ViewState("StateSave") = "Insert" Then
                tbRef.Text = GetAutoNmbr("OCD", "Y", Year(tbDate.SelectedValue), Month(tbDate.SelectedValue), "", ViewState("DBConnection").ToString)
                lbStatus.Text = ""

                SQLString = "INSERT INTO PLDCOHd (TransNmbr, Status, TransDate, KernelTBS, CpoOutput, KernelOutput, CangkangOutput, " + _
                "Remark, UserPrep, DatePrep) " + _
                "SELECT " + QuotedStr(tbRef.Text) + ", 'H', '" + Format(tbDate.SelectedValue, "yyyy-MM-dd") + "',  " + _
                QuotedStr(tbKernelTBS.Text) + "," + QuotedStr(tbCpoOutput.Text) + ", " + QuotedStr(tbKernelOutput.Text) + ", " + _
                QuotedStr(tbCangkangOutput.Text) + "," + QuotedStr(tbRemark.Text) + "," + _
                QuotedStr(ViewState("UserId").ToString) + ", getDate()"
            Else
                Dim cekStatus As String
                cekStatus = SQLExecuteScalar("Select Status FROM PLDCOHd WHERE TransNmbr = " + QuotedStr(tbRef.Text), ViewState("DBConnection").ToString)

                If cekStatus = "P" Then
                    lbStatus.Text = MessageDlg("Save failed... Data has already posted by another user")
                    Exit Sub
                End If
                SQLString = "UPDATE PLDCOHd SET KernelTBS = " + QuotedStr(tbKernelTBS.Text) + _
                ", CpoOutput = " + QuotedStr(tbCpoOutput.Text) + ", KernelOutput = " + QuotedStr(tbKernelOutput.Text) + ", CangkangOutput = " + QuotedStr(tbCangkangOutput.Text) + _
    ...
    Function CekHd() As Boolean
        Dim Cekdate As String
        Try

            Cekdate = SQLExecuteScalar("Select TransDate FROM PLDCOHd WHERE Status <> 'D' AND TransDate = " + QuotedStr(tbDate.SelectedDate), ViewState("DBConnection").ToString)

            If Cekdate <> "" Then
                lbStatus.Text = MessageDlg("Save failed... Date has already Exist")
                Exit Function
            End If
            ...
            If tbKernelTBS.Text = "" Then
                lbStatus.Text = MessageDlg("Kernel TBS must have value")
                tbKernelTBS.Focus()
                Return False
            End If

            If tbCpoOutput.Text = "" Then
                lbStatus.Text = MessageDlg("CPO Output must have value")
                tbCpoOutput.Focus()
                Return False
            End If

            If tbKernelOutput.Text = "" Then
                lbStatus.Text = MessageDlg(" Kernel Output must have value")
                tbKernelOutput.Focus()
                Return False
            End If

            If tbCangkangOutput.Text = "" Then
                lbStatus.Text = MessageDlg("Cangkang must have value")
                tbCangkangOutput.Focus()
                Return False
            End If
    ...
    Protected Sub LbAdvSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles LbAdvSearch.Click
        ...
        Try
            ...
            FilterValue = "Reference, dbo.FormatDate(TrakmnsDate), Date, Status, Remark"
                ...
                ElseIf DDL.SelectedValue = "Print" Then
                    Try
                        ...
                        If CekMenu <> "" Then
                            ...
                            Exit Sub
                        End If
                        Session("SelectCommand") = "EXEC S_PLFormDCO ''" + QuotedStr(GVR.Cells(2).Text) + "''"
    ...
    Protected Sub FillTextBoxHd(ByVal Nmbr As String)
        ...
        Try
            ...
            BindToText(tbKernelTBS, Dt.Rows(0)("KernelTBS").ToString)
            BindToText(tbCpoOutput, Dt.Rows(0)("CpoOutput").ToString)
            BindToText(tbKernelOutput, Dt.Rows(0)("KernelOutput").ToString)
            BindToText(tbCangkangOutput, Dt.Rows(0)("CangkangOutput").ToString)
