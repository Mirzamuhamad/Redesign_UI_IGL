...
Partial Class TrPriceListSupplier
    Inherits System.Web.UI.Page

    Protected con As New SqlConnection
    Protected da As New SqlDataAdapter
    Protected GetStringHd As String = "Select * From V_MKPriceListSupplierHd "
        ...
        Try
            ...
            If Not Session("Result") Is Nothing Then
                ...
                If ViewState("Sender") = "btnSupp" Then
                    ...
                    Dim Supp As String
                    Supp = SQLExecuteScalar("SELECT TodayPrice FROM V_MKPriceListSupplierDt WHERE  EndDate = (SELECT MAX(EndDate) FROM V_MKPriceListSupplierDt) And Supplier ='" + tbSuppCode.Text + "'", ViewState("DBConnection").ToString)
    ...
    Private Function GetStringDt(ByVal Nmbr As String) As String
        Return "SELECT * From V_MKPriceListSupplierDt WHERE TransNmbr = " + QuotedStr(Nmbr)
    End Function
    ...
    Protected Sub btnSaveDt_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSaveDt.Click
        ...
        Try
            If ViewState("StateDt") = "Edit" Then
                If ViewState("DtValue") <> tbSuppCode.Text Then
                    'If CekExistData(ViewState("Dt"), "Supplier", tbSuppCode.Text) Then
                    '    lbStatus.Text = "Supplier " + tbSuppName.Text + " has already been exist"
                    '    Exit Sub
                    'End If
                End If
                Row = ViewState("Dt").Select("Supplier = " + QuotedStr(ViewState("DtValue")))(0)
                If CekDt() = False Then
                    Exit Sub
                End If
                Row.BeginEdit()
                ...
                Row("No_SPK") = tbNoSpk.Text
                Row("PreviousPrice") = tbPreviousPrice.Text
                Row("TodayPrice") = tbTodayPrice.Text
            ...
            Else
                ...
                If CekDt() = False Then
                    Exit Sub
                End If
                If CekExistData(ViewState("Dt"), "Supplier", tbSuppCode.Text) = True Then
                    lbStatus.Text = "Supplier " + tbSuppName.Text + " has already been exist"
                    Exit Sub
                End If
                Dim dr As DataRow
                ...
                dr("No_SPK") = tbNoSpk.Text
                dr("PreviousPrice") = tbPreviousPrice.Text
                dr("TodayPrice") = tbTodayPrice.Text
    ...
    Private Sub SaveAll()
        ...
        Try
            ...
            If ViewState("StateSave") = "Insert" Then
                tbRef.Text = GetAutoNmbr("PLS", "Y", Year(tbDate.SelectedValue), Month(tbDate.SelectedValue), "", ViewState("DBConnection").ToString)

                SQLString = "INSERT INTO MKPriceListSupplierHd (TransNmbr  ,Status, TransDate, EffectiveDate, Product,  Remark,UserPrep,DatePrep ) " + _
                "SELECT " + QuotedStr(tbRef.Text) + ", 'H','" + Format(tbDate.SelectedValue, "yyyy-MM-dd") + "','" + _
                Format(tbEffectiveDate.SelectedDate, "yyyy-MM-dd") + "', " + QuotedStr(tbProductCode.Text) + "," + _
                QuotedStr(tbRemark.Text) + "," + QuotedStr(ViewState("UserId").ToString) + ", getDate()"


            Else
                Dim cekStatus As String
                cekStatus = SQLExecuteScalar("Select Status FROM MKPriceListSupplierHd WHERE TransNmbr = " + QuotedStr(tbRef.Text), ViewState("DBConnection").ToString)
                If cekStatus = "P" Then
                    lbStatus.Text = MessageDlg("Save failed... Data has already posted by another user")
                    Exit Sub
                End If
                SQLString = "UPDATE MKPriceListSupplierHd SET Product = " + QuotedStr(tbProductCode.Text) + ",TransDate= '" + Format(tbDate.SelectedValue, "yyyy-MM-dd") + _
                "', Remark = " + QuotedStr(tbRemark.Text) + ",  EffectiveDate = '" + Format(tbEffectiveDate.SelectedDate, "yyyy-MM-dd") + "', DatePrep = getDate()" + _
            ...
            con.Open()
            Dim cmdSql As New SqlCommand("SELECT TransNmbr , Supplier, No_SPK, PreviousPrice, TodayPrice ,Remark  FROM MKPriceListSupplierDt WHERE TransNmbr = '" & ViewState("TransNmbr") & "'", con)
            ...
            Dim Dt As New DataTable("MKPriceListSupplierDt")
    ...
    Protected Sub btnAddDt_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAddDt.Click, btnAddDt2.Click
        ...
        tbNoSpk.Enabled = True
        tbPreviousPrice.Enabled = True
    ...
    Function CekHd() As Boolean
        Try
            ...
            Dim Supp As String
            Supp = SQLExecuteScalar("SELECT dbo.FormatDate(EffectiveDate) FROM V_MKPriceListSupplierDt WHERE  dbo.FormatDate(EffectiveDate) ='" + tbEffectiveDate.Text + "' And Supplier ='" + tbSuppCode.Text + "'", ViewState("DBConnection").ToString)
            'lbStatus.Text = tbEffectiveDate.Text


            If Supp = tbEffectiveDate.Text Then
                lbStatus.Text = MessageDlg("Effective Date Already Exit With the Same Date and Supplier")
    ...
    Function CekDt(Optional ByVal Dr As DataRow = Nothing) As Boolean
        Try
            If Not Dr Is Nothing Then
                ...
                If Dr("No_SPK").ToString.Trim = "" Then
                    lbStatus.Text = MessageDlg("No SPK Must Have Value")
                    Return False
                End If
                ...
                '    Return False
                'End If
                If CFloat(Dr("TodayPrice").ToString) < 0 Then
                    lbStatus.Text = MessageDlg("TodayPrice Must Have Value")
                    Return False
                End If
            Else
                If tbSuppCode.Text.Trim = "" Then
                    ...
                    Return False
                End If
                If tbNoSpk.Text.Trim = "" Then
                    lbStatus.Text = MessageDlg("No SPK Must Have Value")
                    tbNoSpk.Focus()
                    Return False
                End If
                'If CFloat(tbPreviousPrice.Text) ="" Then
                '    lbStatus.Text = MessageDlg("Previous Price Order Must Have Value")
                '    tbPreviousPrice.Focus()
                '    Return False
                'End If
                If CFloat(tbTodayPrice.Text) < 0 Then
                    lbStatus.Text = MessageDlg("Today Price Must Have Value")
                    tbTodayPrice.Focus()
                    Return False
                End If

                If tbTodayPrice.Text = "" Then
                    lbStatus.Text = MessageDlg("Today Price Must Have Value")
                    tbTodayPrice.Focus()
                    Return False
                End If
    ...
    Protected Sub GridDt_RowDeleting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewDeleteEventArgs) Handles GridDt.RowDeleting
        ...
        dr = ViewState("Dt").Select("Supplier = " + QuotedStr(GVR.Cells(1).Text))
        dr(0).Delete()
        BindGridDt(ViewState("Dt"), GridDt)
        EnableHd(GetCountRecord(ViewState("Dt")) = 0)
    End Sub
    ...
    Protected Sub GridDt_RowEditing(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewEditEventArgs) Handles GridDt.RowEditing
        ...
        Try
            ...
            ViewState("DtValue") = tbSuppCode.Text
            tbSuppCode.Focus()
            StatusButtonSave(False)
        Catch ex As Exception
    ...
    Protected Sub FillTextBoxDt(ByVal Supplier As String)
        Dim Dr As DataRow()
        Try
            Dr = ViewState("Dt").select("Supplier = " + QuotedStr(Supplier))
            If Dr.Length > 0 Then
                BindToText(tbSuppCode, Dr(0)("Supplier").ToString)
                BindToText(tbSuppName, Dr(0)("SupplierName").ToString)
                BindToText(tbNoSpk, Dr(0)("No_SPK").ToString)
                BindToText(tbPreviousPrice, FormatNumber(Dr(0)("PreviousPrice").ToString), 2)
                BindToText(tbTodayPrice, FormatNumber(Dr(0)("TodayPrice").ToString), 2)
                BindToText(tbPrevious, FormatNumber(Dr(0)("TodayPrice").ToString), 2)
    ...
    End Sub
    Protected Sub tbTodayPrice_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbTodayPrice.TextChanged
        Try
            'tbTodayPrice.Text = FindConvertUnit(tbProductCode.Text, ddlUnit.SelectedValue, tbQty.Text, ViewState("DBConnection").ToString).ToString
            'tbTodayPrice.Text = FormatFloat(tbTodayPrice.Text, ViewState("DigitQty"))
            'tbPreviousPrice.Text = FormatFloat(tbPreviousPrice.Text, ViewState("DigitQty"))
            'tbTodayPrice.Focus()

            'If ViewState("StateDt") = "Insert" Then
            '    tbPreviousPrice.Text = tbTodayPrice.Text
            'End If
            ...
            If tbPreviousPrice.Text = 0 Then
                tbPreviousPrice.Text = tbPrevious.Text
                tbPrevious.Text = tbTodayPrice.Text

            End If
            tbTodayPrice.Text = FormatNumber(tbTodayPrice.Text, 2)
