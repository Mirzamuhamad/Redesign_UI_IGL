...
Partial Class CNSupp
    Inherits System.Web.UI.Page

    Protected con As New SqlConnection
    Protected da As New SqlDataAdapter
    Protected GetStringHd As String = "Select * From V_FNCNSuppHd "
    ...
    Private Function GetStringDt(ByVal Nmbr As String) As String
        Return "SELECT * From V_FNCNSuppDt WHERE TransNmbr = " + QuotedStr(Nmbr)
    End Function
    ...
    Protected Sub BtnGo_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnGo.Click, btnGo2.Click
        ...
        Try
            ...
            If ActionValue = "Print" Then
                ...
                Result = Result + "'"
                Session("SelectCommand") = "EXEC S_FNFormCNDNSupp " + Result + ", 'CN' "
                Session("ReportFile") = ".../../../Rpt/FormCNDNSupp.frx"
            ...
            Else
                ...
                For j = 0 To (Nmbr.Length - 1)
                    If Nmbr(j) = "" Then
                        Exit For
                    Else
                        Result = ExecSPCommandGo(ActionValue, "S_FNSICN", Nmbr(j), CInt(Session(Request.QueryString("KeyId"))("Year")), CInt(Session(Request.QueryString("KeyId"))("Period")), ViewState("UserId").ToString, ViewState("DBConnection").ToString)
                        If Trim(Result) <> "" Then
    ...
    Private Sub SaveAll()
        ...
        Try
            ...
            If ViewState("StateSave") = "Insert" Then
                ...
                tbRef.Text = GetAutoNmbr("CNS", "Y", Year(tbDate.SelectedValue), Month(tbDate.SelectedValue), "", ViewState("DBConnection").ToString)

                SQLString = "INSERT INTO FINCNSuppHd (TransNmbr, FgReport, Status, TransDate, Supplier, Attn, " + _
                "CostCtr, Term, DueDate, SuppInvNo, SuppInvDate, PPnNo, PPnDate, PPnRate, Currency, ForexRate, BaseForex, Disc, DiscForex, PPn, PPnForex, TotalForex, " + _
                "Remark, UserPrep, DatePrep) " + _
                "SELECT " + QuotedStr(tbRef.Text) + ", " + QuotedStr("Y") + ", 'H', '" + Format(tbDate.SelectedDate, "yyyy-MM-dd") + "', " + _
                QuotedStr(tbSuppCode.Text) + ", " + QuotedStr(tbAttn.Text) + ", " + QuotedStr(ddlCostCtr.SelectedValue) + ", " + _
                QuotedStr(ddlTerm.SelectedValue) + ", '" + Format(tbDueDate.SelectedDate, "yyyy-MM-dd") + "', " + QuotedStr(tbSuppInvNo.Text) + ", '" + Format(tbSuppInvDate.SelectedDate, "yyyy-MM-dd") + "', " + _
            ...
            Else
                ...
                cekStatus = SQLExecuteScalar("Select Status FROM FINCNSuppHd WHERE TransNmbr = " + QuotedStr(tbRef.Text), ViewState("DBConnection").ToString)
                If cekStatus = "P" Then
                    lbStatus.Text = MessageDlg("Save failed... Data has already posted by another user")
                    Exit Sub
                End If
                SQLString = "UPDATE FINCNSuppHd SET Supplier = " + QuotedStr(tbSuppCode.Text) + ", Attn = " + QuotedStr(tbAttn.Text) + _
                ", CostCtr = " + QuotedStr(ddlCostCtr.SelectedValue) + _
                ", Term = " + QuotedStr(ddlTerm.SelectedValue) + ", SuppInvNo = " + QuotedStr(tbSuppInvNo.Text) + _
                ", SuppInvDate = '" + Format(tbSuppInvDate.SelectedDate, "yyyy-MM-dd") + "', PPnNo = " + QuotedStr(tbPPnNo.Text) + _
            ...
            con.Open()
            Dim cmdSql As New SqlCommand("SELECT TransNmbr, ItemNo, Account, FgSubled, Subled, CostCtr, Qty, Unit, PriceForex, AmountForex, Remark FROM FINCNSuppDt WHERE TransNmbr = '" & ViewState("TransNmbr") & "'", con)
            ...
            "UPDATE FINCNSuppDt SET ItemNo = @ItemNo, Account = @Account, " + _
            ...
            Dim Delete_Command = New SqlCommand( _
                "DELETE FROM FINCNSuppDt WHERE TransNmbr = '" & ViewState("TransNmbr") & "' AND ItemNo = @ItemNo ", con)
            ' Add the parameters for the DeleteCommand.
            param = Delete_Command.Parameters.Add("@ItemNo", SqlDbType.Int, 4, "ItemNo")
            param.SourceVersion = DataRowVersion.Original
            da.DeleteCommand = Delete_Command

            Dim Dt As New DataTable("FINCNSuppDt")
    ...
    Private Sub ClearHd()
        Try
            ...
            tbSuppInvDate.SelectedDate = ViewState("ServerDate") 'Today
            tbSuppCode.Text = ""
            tbSuppName.Text = ""
            tbAttn.Text = ""
    ...
    Protected Sub ddlCurr_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlCurr.SelectedIndexChanged
        Try
            ...
            tbRate.Focus()
        Catch ex As Exception
            Throw New Exception("ddlCurr_SelectedIndexChanged Error : " + ex.ToString)
        End Try        
    End Sub
    ...
    Protected Sub LbAdvSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles LbAdvSearch.Click
        ...
        Try
            FDateName = "Date, Invoice Date, Due Date"
            FDateValue = "TransDate, SuppInvDate, DueDate"
            FilterName = "Reference, Date, Status, Supplier Code, Supplier Name, Term, Due Date, Invoice No, Remark"
            FilterValue = "TransNmbr, dbo.FormatDate(TransDate), Status, Supplier_Code, Supplier_Name, Term_Name, dbo.FormatDate(DueDate), SuppInvNo, Remark"
                ...
                ElseIf DDL.SelectedValue = "Print" Then
                    Try
                        ...
                        If CekMenu <> "" Then
                            ...
                            Exit Sub
                        End If
                        Session("SelectCommand") = "EXEC S_FNFormCNDNSupp '''" + GVR.Cells(2).Text + "''', 'CN' "
                        Session("ReportFile") = ".../../../Rpt/FormCNDNSupp.frx"
                        Session("DBConnection") = ViewState("DBConnection")
                        AttachScript("openprintdlg();", Page, Me.GetType)
                    Catch ex As Exception
    ...
    Protected Sub FillTextBoxHd(ByVal Nmbr As String)
        ...
        Try
            ...
            BindToDate(tbSuppInvDate, Dt.Rows(0)("SuppInvDate").ToString)
    ...
    Protected Sub btnAccount_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnAccount.Click
        Dim ResultField As String
        Try
            Session("filter") = "SELECT * FROM V_MsAccountDt WHERE TransType = 'CNS' AND ( Currency = " + QuotedStr(ddlCurr.SelectedValue) + " OR Currency = " + QuotedStr(ViewState("Currency").ToString) + ") "
    ...
    Protected Sub tbAccount_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbAccount.TextChanged
        ...
        Dim Dt As DataTable
        Try
            'Dr = FindMaster("Account", tbAccount.Text + "|CNS", ViewState("DBConnection").ToString)
            'Dt = SQLExecuteQuery("SELECT * FROM V_MsAccountDt WHERE TransType = 'CNS' AND ( Currency = " + QuotedStr(ddlCurr.SelectedValue) + " OR Currency = " + QuotedStr(ViewState("Currency").ToString) + ") ")
            Dt = SQLExecuteQuery("SELECT * FROM V_MsAccountDt WHERE TransType = 'CNS' AND ( Currency = " + QuotedStr(ddlCurr.SelectedValue) + " OR Currency = " + QuotedStr(ViewState("Currency").ToString) + ") AND Account = " + QuotedStr(tbAccount.Text), ViewState("DBConnection").ToString).Tables(0)
            If Dt.Rows.Count > 0 Then
                Dr = Dt.Rows(0)
                tbAccount.Text = TrimStr(Dr("Account").ToString)
                tbAccountName.Text = TrimStr(Dr("Description").ToString)
    ...
    Private Sub TotalHd()
        tbDiscForex.Text = FormatFloat((CFloat(tbBaseForex.Text) * CFloat(tbDisc.Text)) / 100, ViewState("DigitCurr").ToString)
        tbPPNForex.Text = FormatFloat(((CFloat(tbBaseForex.Text) - CFloat(tbDiscForex.Text)) * CFloat(tbPPN.Text)) / 100, ViewState("DigitCurr").ToString)
        tbTotalForex.Text = FormatFloat(CFloat(tbBaseForex.Text) - CFloat(tbDiscForex.Text) + CFloat(tbPPNForex.Text), ViewState("DigitCurr").ToString)
    End Sub
