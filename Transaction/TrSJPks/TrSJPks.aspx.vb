...
Partial Class Transaction_TrSJPks
    Inherits System.Web.UI.Page
    Protected con As New SqlConnection
    Protected da As New SqlDataAdapter

    Private Function GetStringHd() As String
        Return "Select * From V_PLSJPksHD " ' WHERE UserPrep = '" + ViewState("UserId") + "' "
                ...
                If HiddenRemarkDelete.Value = "true" Then
                    ...
                    For j = 0 To (Nmbr.Length - 1)
                        ...
                        Else
                            ...
                            Result = ExecSPCommandGo("Delete", "S_PLSJPKS", Nmbr(j), CInt(ViewState("GLYear")), CInt(ViewState("GLPeriod")), ViewState("UserId").ToString, ViewState("DBConnection").ToString)
    ...
    Private Function GetStringDt(ByVal Nmbr As String) As String
        Return "SELECT * From V_PLSJPksDt WHERE TransNmbr = " + QuotedStr(Nmbr)
    End Function
    ...
    Protected Sub BtnGo_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnGo.Click, btnGo2.Click
        ...
        Try
            ...
            If ActionValue = "Print" Then
                ...
                Next
                Result = Result + "'"
                Session("SelectCommand") = "EXEC S_PLFormSJPks" + Result
                'lbStatus.Text = Session("SelectCommand")
                'Exit Sub
                Session("ReportFile") = ".../../../Rpt/FormSJPks.frx"
            ...
            Else
                ...
                For j = 0 To (Nmbr.Length - 1)
                    If Nmbr(j) = "" Then
                        Exit For
                    Else
                        Result = ExecSPCommandGo(ActionValue, "S_PLSJPks", Nmbr(j), CInt(Session(Request.QueryString("KeyId"))("Year")), CInt(Session(Request.QueryString("KeyId"))("Period")), ViewState("UserId").ToString, ViewState("DBConnection").ToString)
                        If Trim(Result) <> "" Then
                            lbStatus.Text = lbStatus.Text + Result + " <br/>"
    ...
    Private Sub SaveAll()
        ...
        Try
            ...
            If ViewState("StateSave") = "Insert" Then
                tbRef.Text = GetAutoNmbr("PSJ", "Y", Year(tbDate.SelectedValue), Month(tbDate.SelectedValue), "", ViewState("DBConnection").ToString)
                lbStatus.Text = ""

                SQLString = "INSERT INTO PLSJPksHd (TransNmbr, Status, TransDate, No_SPTOut, No_Timbang, Customer, JamMasuk, JamKeluar, " + _
                "CarNo, Operator,Remark, UserPrep, DatePrep) " + _
                "SELECT " + QuotedStr(tbRef.Text) + ", 'H', '" + Format(tbDate.SelectedValue, "yyyy-MM-dd") + "',  " + _
                QuotedStr(tbNoSptOut.Text) + "," + QuotedStr(tbNoTimbang.Text) + ", " + QuotedStr(tbCustomer.Text) + ", " + _
                QuotedStr(tbJamMasuk.Text) + "," + QuotedStr(tbJamkeluar.Text) + "," + _
                QuotedStr(tbCar.Text) + "," + QuotedStr(tbOperator.Text) + "," + QuotedStr(tbRemark.Text) + "," + _
            ...
            Else
                ...
                If cekStatus = "P" Then
                    ...
                    Exit Sub
                End If
                SQLString = "UPDATE PLSJPksHd SET No_SPTOut = " + QuotedStr(tbNoSptOut.Text) + _
                ", No_Timbang = " + QuotedStr(tbNoTimbang.Text) + ", Customer = " + QuotedStr(tbCustomer.Text) + ", JamMasuk = " + QuotedStr(tbJamMasuk.Text) + _
                ", Remark = " + QuotedStr(tbRemark.Text) + ",JamKeluar = " + QuotedStr(tbJamkeluar.Text) + _
                ", CarNo = " + QuotedStr(tbCar.Text) + ",Operator = " + QuotedStr(tbOperator.Text) + _
            ...
            con.Open()
            Dim cmdSql As New SqlCommand("SELECT TransNmbr, ItemNo, Timbang1, Timbang2, Potongan, Netto1, Netto2,ItemType, So_No, Do_No, ContractNo,SisaContract,NoSegel, Shipper, Remark FROM PLSJPksDt WHERE TransNmbr = '" & ViewState("Reference") & "'", con)
            ...
            Dim Update_Command = New SqlCommand( _
                    "UPDATE PLSJPksDt SET ItemNo = @Item, Timbang1 = @Timbang1, Timbang2 = @Timbang2, Potongan = @Potongan " + _
                    ", Netto1 = @Netto1, Netto2 = @Netto2, ItemType = @ItemType, So_No = @So_No, Do_No = @Do_No, ContractNo = @ContractNo ,SisaContract = @SisaContract " + _
                    ", NoSegel = @NoSegel, Shipper = @Shipper " + _
                    ", Remark = @Remark WHERE TransNmbr = '" & ViewState("Reference") & "' AND ItemNo = @OldItem ", con)
            ...
            Update_Command.Parameters.Add("@ItemType", SqlDbType.VarChar, 22, "ItemType")
            Update_Command.Parameters.Add("@So_No", SqlDbType.VarChar, 30, "So_No")
            Update_Command.Parameters.Add("@Do_No", SqlDbType.VarChar, 30, "Do_No")
            Update_Command.Parameters.Add("@ContractNo", SqlDbType.VarChar, 30, "ContractNo")
            Update_Command.Parameters.Add("@NoSegel", SqlDbType.VarChar, 30, "NoSegel")
            Update_Command.Parameters.Add("@Shipper", SqlDbType.VarChar, 50, "Shipper")
            Update_Command.Parameters.Add("@SisaContract", SqlDbType.Float, 22, "SisaContract")
            Update_Command.Parameters.Add("@Remark", SqlDbType.VarChar, 100, "Remark")
            ...
            Dim Delete_Command = New SqlCommand( _
                "DELETE FROM PLSJPksDt WHERE TransNmbr = '" & ViewState("Reference") & "' AND ItemNo = @oldItem ", con)
            ' Add the parameters for the DeleteCommand.
            param = Delete_Command.Parameters.Add("@oldItem", SqlDbType.Int, 5, "ItemNo")
            param.SourceVersion = DataRowVersion.Original
            da.DeleteCommand = Delete_Command

            Dim Dt As New DataTable("PLSJPksDt")
    ...
    Protected Sub btnSaveTrans_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSaveTrans.Click
        ...
        Try
            ...
            Dim Status As String
            Status = SQLExecuteScalar("SELECT Status FROM PlSjpksHd WHERE TransNmbr = '" + tbRef.Text + "'", ViewState("DBConnection"))
            ...
            If Status <> "G" Then
                ...
                'Exit Sub
                ResultGetAppr = ExecSPPosting("S_PLSJPksGetAppr", tbRef.Text, CInt(Session(Request.QueryString("KeyId"))("Year")), CInt(Session(Request.QueryString("KeyId"))("Period")), ViewState("UserId").ToString, ViewState("DBConnection").ToString)
            ...
            'lbStatus.Text = "TestDeviceFilter" + tbRef.Text
            'Exit Sub 
            ResultPOst = ExecSPPosting("S_PLSJPksPost", tbRef.Text, CInt(Session(Request.QueryString("KeyId"))("Year")), CInt(Session(Request.QueryString("KeyId"))("Period")), ViewState("UserId").ToString, ViewState("DBConnection").ToString)
    ...
    Private Sub EnableDt(ByVal State As Boolean)
        Try
            tbTimbang1.Enabled = State
            tbTimbang2.Enabled = State
            tbPotongan.Enabled = State
            tbNetto1.Enabled = State
            tbNetto2.Enabled = State
            tbSo.Enabled = State
            tbDo.Enabled = State
            tbItemType.Enabled = State
            tbKontrak.Enabled = State
            tbSisaKontrak.Enabled = State
            lbItemNo.Enabled = State
            tbNoSegel.Enabled = State
            tbShipper.Enabled = State
        Catch ex As Exception
    ...
    Private Sub EnableHd(ByVal State As Boolean)
        Try
            tbNoSptOut.Enabled = State
            tbNoTimbang.Enabled = State
            tbCustomer.Enabled = State
            tbCustomerName.Enabled = State
            tbCar.Enabled = State
            tbCarName.Enabled = State
            tbOperator.Enabled = State
            tbOperatorName.Enabled = State
            tbJamMasuk.Enabled = State
            tbJamkeluar.Enabled = State
    ...
    Function CekHd() As Boolean
        Try
            ...
            If tbNoSptOut.Text = "" Then
                lbStatus.Text = MessageDlg("No SPTOut must have value")
                tbNoSptOut.Focus()
                Return False
            End If
    ...
    Protected Sub LbAdvSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles LbAdvSearch.Click
        ...
        Try
            ...
            FilterValue = "Reference, dbo.FormatDate(TransDate), Date, No_SpTOut, No_Timbang,Remark"
            ...
            If e.CommandName = "Go" Then
                ...
                ElseIf DDL.SelectedValue = "Print" Then
                    Try
                        ...
                        Session("SelectCommand") = "EXEC S_PLFormSJPks ''" + QuotedStr(GVR.Cells(2).Text) + "''"
                ...
                ElseIf DDL.SelectedValue = "Print Lampiran" Then
                    Try
                        CekMenu = CheckMenuLevel("Print", ViewState("MenuLevel").Rows(0))
                        If CekMenu <> "" Then
                            lbStatus.Text = CekMenu
                        ...
                        If GVR.Cells(3).Text <> "P" Then
                            lbStatus.Text = MessageDlg("Data Must Be Posting First to Print Lampiran!")
                            Exit Sub
                        End If
                        ...
                        'Session("SelectCommand") = "EXEC S_STFormIssueReq ''" + QuotedStr(GVR.Cells(2).Text) "''"
                        Session("ReportFile") = ".../../../Rpt/FormSJPksLam.frx"
                        Session("DBConnection") = ViewState("DBConnection")
                        AttachScript("openprintdlg();", Page, Me.GetType)
                    Catch ex As Exception
                        lbStatus.Text = "btn print Error = " + ex.ToString
    ...
    Protected Sub FillTextBoxHd(ByVal Nmbr As String)
        ...
        Try
            ...
            BindToText(tbNoSptOut, Dt.Rows(0)("No_SPTOut").ToString)
            ...
            BindToText(tbOperatorName, Dt.Rows(0)("OperatorName").ToString)
            BindToText(tbJamMasuk, Dt.Rows(0)("JamMasuk").ToString)
    ...
    Protected Sub FillTextBoxDt(ByVal ItemNo As String)
        ...
        Try
            ...
            If Dr.Length > 0 Then
                ...
                BindToText(tbDo, Dr(0)("Do_no").ToString)
                BindToText(tbSo, Dr(0)("So_No").ToString)
                BindToText(tbItemType, Dr(0)("ItemType").ToString)
                BindToText(tbKontrak, Dr(0)("ContractNo").ToString)
                BindToText(tbNoSegel, Dr(0)("NoSegel").ToString)
                BindToText(tbShipper, Dr(0)("Shipper").ToString)
                BindToText(tbSisaKontrak, FormatNumber(Dr(0)("SisaContract").ToString), 2)
    ...
    Protected Sub btnSaveDt_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnSaveDt.Click
        ...
        Try
            ...
            If ViewState("StateDt") = "Edit" Then
                ...
                Row("Itemtype") = tbItemType.Text
                Row("Do_No") = tbDo.Text
                Row("So_No") = tbSo.Text
                Row("NoSegel") = tbNoSegel.Text
                Row("Shipper") = tbShipper.Text
                Row("ContractNo") = tbKontrak.Text
                Row("SisaContract") = tbSisaKontrak.Text
            ...
            Else
                ...
                dr("Itemtype") = tbItemType.Text
                dr("Do_No") = tbDo.Text
                dr("So_No") = tbSo.Text
                dr("NoSegel") = tbNoSegel.Text
                dr("Shipper") = tbShipper.Text
                dr("ContractNo") = tbKontrak.Text
                dr("SisaContract") = tbSisaKontrak.Text
    ...
    Protected Sub tbScanSlip_textChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbScanSlip.TextChanged
        ...
        Try
            ...
            'CekString = SQLExecuteScalar("SELECT No_Timbang FROM PLSJPksHD WHERE Status <> 'D' AND No_Timbang = " + QuotedStr(Left(tbScanSlip.Text, 6)), ViewState("DBConnection").ToString)
            ...
            If dtUnit.Rows.Count > 0 Then
                ...
                CekString = SQLExecuteScalar("SELECT No_Timbang FROM PLSJPksHD WHERE Status <> 'D' AND No_Timbang ='" + drUnit("No_Timbang").ToString + "'", ViewState("DBConnection").ToString)
                If drUnit("No_Timbang").ToString = CekString Then
                    'lbStatus.Text = MessageDlg("Qrcode is already Scan!!")
                    btnBack_Click(Nothing, Nothing)
                    lbStatus.Text = "Qrcode is already Scan or Availeble on your database!!"
                    tbScanSlip.Text = ""
                ...
                tbNoSptOut.Text = drUnit("No_SPTIN").ToString
                tbNoTimbang.Text = drUnit("No_Timbang").ToString
                tbRemark.Text = drUnit("Remark").ToString


                'Dari View ============
                DataString = "SELECT * FROM V_PLSPTOutHD WHERE TransNmbr = '" + drUnit("No_SPTIN").ToString + "'"
                DtData = SQLExecuteQuery(DataString, ViewState("DBConnection")).Tables(0)
                DrData = DtData.Rows(0)

                tbCustomer.Text = DrData("Customer").ToString
                tbCustomerName.Text = DrData("CustomerName").ToString
                tbCar.Text = DrData("Car").ToString
                tbCarName.Text = DrData("CarName").ToString
                tbOperator.Text = DrData("Operator").ToString
                tbOperatorName.Text = DrData("OperatorName").ToString


                '==================== 
                'Ambil Nomor Contract dari Do
                Dim NoKontrak As String
                Dim QtyOrderDo, SisaKontrak, TotalNetto, TotalNetto2 As Integer
                NoKontrak = SQLExecuteScalar("SELECT ContractNo FROM V_MKDOHd WHERE TransNmbr = '" + DrData("Do_No").ToString + "'", ViewState("DBConnection"))
                QtyOrderDo = SQLExecuteScalar("SELECT QtyOrder FROM V_MKDODt WHERE TransNmbr = '" + DrData("Do_No").ToString + "'", ViewState("DBConnection"))
                TotalNetto = SQLExecuteScalar(" SELECT COALESCE(SUM (Netto2),0) FROM V_PLSJPksDt WHERE Status = 'P' AND DO_No = '" + DrData("Do_No").ToString + "'", ViewState("DBConnection"))
                TotalNetto2 = Val(drUnit("BrtNetto2").ToString) + Val(TotalNetto)
                SisaKontrak = QtyOrderDo - TotalNetto2
                'lbStatus.Text = TotalNetto
                'lbStatus.Text = QtyOrderDo
                'Exit Sub

                'Proteksi untuk 

                If TotalNetto = 0 Then
                    If drUnit("BrtNetto2").ToString > QtyOrderDo Then
                        lbStatus.Text = MessageDlg("Total Qty on SJ-PKS (posted) may not exceed the qty on DO")
                        btnBack_Click(Nothing, Nothing)
                        tbScanSlip.Text = ""
                        Exit Sub
                    End If
                ElseIf TotalNetto > 0 Then
                    If TotalNetto2 > QtyOrderDo Then
                        lbStatus.Text = MessageDlg("Total Qty on SJ-PKS (posted) may not exceed the qty on DO")
                        btnBack_Click(Nothing, Nothing)
                        tbScanSlip.Text = ""
                        Exit Sub
                ...
                dr("So_No") = DrData("So_No").ToString
                dr("Do_No") = DrData("Do_No").ToString
                dr("ItemType") = DrData("ItemType").ToString
                dr("ItemTypeName") = DrData("ItemTypeName").ToString
                dr("ContractNo") = NoKontrak
                dr("SisaContract") = SisaKontrak
                dr("So_No") = DrData("So_No").ToString
                dr("So_No") = DrData("So_No").ToString
                ...
                dr("NoSegel") = drUnit("NoSegel").ToString
                dr("Shipper") = drUnit("Shipper").ToString
