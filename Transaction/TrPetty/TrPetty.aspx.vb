...
Partial Class Transaction_TrPetty_TrPetty
    ...
    Protected GetStringHd As String = "Select * From V_FNPettyHd"
    ...
    Private Sub SetInit()
        Try
            ...
            FillCombo(ddlPetty, "EXEC S_GetPettyUser " + QuotedStr(ViewState("UserId")), True, "Petty_Code", "Petty_Name", ViewState("DBConnection"))
    ...
    Private Function GetStringDt(ByVal Nmbr As String) As String
        Return "SELECT * From V_FNPettyDt WHERE TransNmbr = " + QuotedStr(Nmbr)
    End Function
    ...
    Protected Sub BtnGo_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnGo.Click, btnGo2.Click
        ...
        Try
            ...
            If ActionValue = "Print" Then
                ...
                Next
                Result = Result + "'"
                Session("SelectCommand") = "EXEC S_FNPettyForm " + Result + ""
                Session("ReportFile") = ".../../../Rpt/FormTrFNPetty.frx"
            ...
            Else
                ...
                For j = 0 To (Nmbr.Length - 1)
                    If Nmbr(j) = "" Then
                        Exit For
                    Else
                        Result = ExecSPCommandGo(ActionValue, "S_FNPetty", Nmbr(j), CInt(Session(Request.QueryString("KeyId"))("Year")), CInt(Session(Request.QueryString("KeyId"))("Period")), ViewState("UserId").ToString, ViewState("DBConnection").ToString)
                        If Trim(Result) <> "" Then
    ...
    Private Sub EnableHd(ByVal State As Boolean)
        Try
            ddlPetty.Enabled = State
            'ddlFgReport.Enabled = State
        Catch ex As Exception
    ...
    Private Sub Cleardt()
        Try
            ...
            tbSubled.Text = ""
            tbSubledName.Text = ""
            lbcurrDt.Text = ddlCurr.SelectedValue
            ddlCurr.Enabled = False
        Catch ex As Exception
    ...
    Function CekHd() As Boolean
        Try
            ...
            If CFloat(tbRate.Text) = 0 Then
                ...
                Return False
            End If
            If ddlPetty.SelectedValue = "" Then
                lbStatus.Text = MessageDlg("Petty Type Must Have Value")
                ddlPetty.Focus()
                Return False
            End If
            If tbPayTo.Text = "" Then
                lbStatus.Text = MessageDlg("Pay To Must Have Value")
                tbPayTo.Focus()
                Return False
            End If
            'If ddlFgReport.SelectedValue.Trim = "" Then
    ...
    Protected Sub FillTextBoxHd(ByVal Nmbr As String)
        ...
        Try
            ...
            BindToDropList(ddlPetty, Dt.Rows(0)("PettyType").ToString)
            ...
            BindToText(tbPayTo, Dt.Rows(0)("PayTo").ToString)
            'BindToDropList(ddlFgReport, Dt.Rows(0)("FgReport").ToString)
    ...
    Private Sub SaveAll()
        ...
        Try
            ...
            If ViewState("StateSave") = "Insert" Then
                tbCode.Text = GetAutoNmbr("PCO", "Y", Year(tbDate.SelectedValue), Month(tbDate.SelectedValue), "", ViewState("DBConnection").ToString)

                SQLString = "INSERT INTO FINPettyHd (TransNmbr, transDate, STATUS, PettyType, " + _
                "Currency, ForexRate, TotalForex, PayTo, FgReport, Remark, UserPrep, DatePrep) " + _
                "SELECT " + QuotedStr(tbCode.Text) + ", " + QuotedStr(Format(tbDate.SelectedValue, "yyyy-MM-dd")) + ", 'H', " + _
                QuotedStr(ddlPetty.SelectedValue) + ", " + QuotedStr(ddlCurr.SelectedValue) + ", " + _
                QuotedStr(tbRate.Text.Replace(",", "")) + ", " + QuotedStr(tbTotalForex.Text.Replace(",", "")) + _
                ", " + QuotedStr(tbPayTo.Text) + ", " + QuotedStr("Y") + _
            ...
            Else
                ...
                cekStatus = SQLExecuteScalar("Select Status FROM FINPettyHd WHERE TransNmbr = " + QuotedStr(tbCode.Text), ViewState("DBConnection").ToString)
                If cekStatus = "P" Then
                    lbStatus.Text = MessageDlg("Save failed... Data has already posted by another user")
                    Exit Sub
                End If
                SQLString = "UPDATE FINPettyHd SET PettyType=" + QuotedStr(ddlPetty.SelectedValue) + _
                ", Currency=" + QuotedStr(ddlCurr.SelectedValue) + _
                ", ForexRate=" + QuotedStr(tbRate.Text.Replace(",", "")) + ", TotalForex=" + QuotedStr(tbTotalForex.Text.Replace(",", "")) + _
                ", PayTo=" + QuotedStr(tbPayTo.Text) + ", FgReport = " + QuotedStr("Y") + ", Remark = " + QuotedStr(tbRemark.Text) + _
            ...
            con.Open()
            Dim cmdSql As New SqlCommand("SELECT TransNmbr, ItemNo, Account, FgSubled, Subled, CostCtr, AmountForex, Remark FROM FINPettyDt WHERE TransNmbr = " + QuotedStr(ViewState("TransNmbr")), con)
            ...
            Dim Update_Command = New SqlCommand( _
                    "UPDATE FINPettyDt SET Account = @Account, FgSubled = @FgSubled, Subled = @Subled, CostCtr = @CostCtr, AmountForex = @AmountForex, Remark = @Remark WHERE TransNmbr = '" & ViewState("TransNmbr") & "' AND ItemNo = @OldItemNo", con)
            ...
            Dim Delete_Command = New SqlCommand( _
                "DELETE FROM FINPettyDt WHERE TransNmbr = '" & ViewState("TransNmbr") & "' AND ItemNo = @ItemNo", con)
            ' Add the parameters for the DeleteCommand.
            param = Delete_Command.Parameters.Add("@ItemNo", SqlDbType.Int, 10, "ItemNo")
            param.SourceVersion = DataRowVersion.Original
            da.DeleteCommand = Delete_Command

            Dim Dt As New DataTable("FINPettyDt")
            ...
            If e.CommandName = "Go" Then
                ...
                ElseIf DDL.SelectedValue = "Print" Then
                    Try
                        ...
                        End If
                        Session("DBConnection") = ViewState("DBConnection")
                        Session("SelectCommand") = "EXEC S_FNPettyForm '''" + GVR.Cells(2).Text + "'''"
    ...
    Protected Sub btnAccount_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAccount.Click
        ...
        Try
            Session("filter") = "SELECT * FROM V_MsAccountDt WHERE TransType = 'PCO' AND (( Currency = " + QuotedStr(ddlCurr.SelectedValue) + " AND FgType = 'BS') OR (FgType = 'PL')) " '" OR Currency = " + QuotedStr(ViewState("Currency").ToString) +
    ...
    Protected Sub tbAccount_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbAccount.TextChanged
        ...
        Try
            If tbAccount.Text.Trim = "" Then
                ...
                Exit Sub
            End If
            Dr = FindMaster("Account", tbAccount.Text + "|PCO", ViewState("DBConnection").ToString)
            If Not Dr Is Nothing Then
    ...
    Protected Sub ddlPetty_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlPetty.SelectedIndexChanged
        Dim Dt As DataTable
        Dim Dr As DataRow
        Try
            Dt = SQLExecuteQuery("EXEC S_GetCurrForFINPetty  " + QuotedStr(ViewState("UserId")) + ", " + QuotedStr(ddlPetty.SelectedValue), ViewState("DBConnection").ToString).Tables(0)
            If Dt.Rows.Count > 0 Then
                Dr = Dt.Rows(0)
                ddlCurr.SelectedValue = Dr("Currency")
            Else
                ddlCurr.SelectedValue = "IDR"
            End If
            ChangeCurrency(ddlCurr, tbDate, tbRate, ViewState("Currency"), ViewState("DigitCurr"), ViewState("DBConnection"), ViewState("DigitHome"), ViewState("DigitRate"))
            tbRate.Focus()
        Catch ex As Exception
            Throw New Exception("ddlPetty_SelectedIndexChanged Error : " + ex.ToString)
        End Try
    End Sub
End Class
