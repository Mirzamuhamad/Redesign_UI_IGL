...
Partial Class TrBOM
    ...
    Private Function GetStringHd() As String
        Return " SELECT DISTINCT TransNmbr, Nmbr, Status, TransDate, EffectiveDate, BOMType, Product, Product_Name, Specification, Customer, Customer_Name, Remark, " + _
               " TSLength, TSWidth, TSSheetPerPack, TSGSM, TSRatioPremix, TSTotalGR, TSTotalPremixGR, LevelProduct FROM V_PROBOMHd "
    End Function
        ...
        Try
            ...
            If Not Session("Result") Is Nothing Then
                If ViewState("Sender") = "btnProduct" Then
                    ...
                    lbProductLevel.Text = Session("Result")(4).ToString
    ...
    Protected Sub BtnGo_Click(ByVal sender As Object, ByVal e As EventArgs) Handles BtnGo.Click, btnGo2.Click
        ...
        Try
            ...
            Else
                ...
                For Each GVR In GridView1.Rows
                    If ((ddlCommand.SelectedValue = "Get Approval") Or (ddlCommand.SelectedValue = "Post")) And ((GVR.Cells(3).Text = "H") Or (GVR.Cells(3).Text = "G")) Then
                        CB = GVR.FindControl("cbSelect")
                        If CB.Checked Then
                            ListSelectNmbr = GVR.Cells(2).Text
                            SQLString2 = " SELECT Max(EffectiveDate) As EffectiveDate FROM PROBOMHd WHERE Status = 'P' AND Product = " + QuotedStr(GVR.Cells(7).Text) + " And EffectiveDate > " + QuotedStr(Format(CDate(GVR.Cells(5).Text), "yyyy-MM-dd"))
                            Dt2 = SQLExecuteQuery(SQLString2, ViewState("DBConnection").ToString).Tables(0)
                            Dr2 = Dt2.Rows(0)
                            'lbStatus.Text = CStr(Dr2("EffectiveDate").ToString.Length)
                            'Exit Sub
                            If Dr2("EffectiveDate").ToString.Length > 4 Then
                                'If Dt2.Rows.Count > 0 Then
                                EffDate = Dr2("EffectiveDate")
                                'MessageDlg("EffectiveDate must greater than ")
                                lbStatus.Text = "EffectiveDate must greater than " + Format(CDate(EffDate), "dd MMM yyyy")
                                Exit Sub
                            End If
                        End If
                    End If
    ...
    Protected Sub btnCancelDt_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnCancelDt.Click
        Try
            ...
            Label9.Visible = True
            lblTotalQtyCompare.Visible = True
        Catch ex As Exception
    ...
    Private Sub SaveAll()
        ...
        Try
            ...
            If ViewState("StateSave") = "Insert" Then
                ...
                SQLString2 = " SELECT * FROM PROBOMHd WHERE Product = " + QuotedStr(tbProductCode.Text) + " AND EffectiveDate = " + QuotedStr(Format(tbEffectiveDate.SelectedValue, "yyyy-MM-dd")) + " AND Status <> 'D' "
                Dt2 = SQLExecuteQuery(SQLString2, ViewState("DBConnection").ToString).Tables(0)
                If Dt2.Rows.Count > 0 Then
                    lbStatus.Text = MessageDlg("Save failed... Data has already exists")
                    Exit Sub
                End If
                ...
                SQLString = "INSERT INTO PROBOMHd (TransNmbr, Status, Transdate, EffectiveDate, Product, Remark, UserPrep, DatePrep, BOMType, TSLength, TSWidth, TSSheetPerPack, TSGSM, TSRatioPremix, TSTotalGr, TSTotalPremixGr, LevelProduct) " + _
                ...
                tbSheet.Text.Replace(",", "") + ", " + tbGSM.Text.Replace(",", "") + ", " + tbRatio.Text.Replace(",", "") + ", " + tbTotalgr.Text.Replace(",", "") + ", " + tbTotalPremix.Text.Replace(",", "") + ", " + QuotedStr(lbProductLevel.Text) + ""
            ...
            Else
                ...
                If cekStatus = "P" Then
                    ...
                    Exit Sub
                End If
                SQLString = "UPDATE PROBOMHd SET Transdate = '" + Format(tbDate.SelectedValue, "yyyy-MM-dd") + "', EffectiveDate = '" + Format(tbEffectiveDate.SelectedValue, "yyyy-MM-dd") + "' " + _
                ...
                ", TSGSM = " + tbGSM.Text.Replace(",", "") + ", TSRatioPremix = " + tbRatio.Text.Replace(",", "") + ", TSTotalGr = " + tbTotalgr.Text.Replace(",", "") + ", TSTotalPremixGr = " + tbTotalPremix.Text.Replace(",", "") + ", LevelProduct = " + lbProductLevel.Text + _
    ...
    Protected Sub btnAddDt_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnAddDt.Click, btnAddDt2.Click
        ...
        lblTotalQtyCompare.Visible = False
    End Sub
    ...
    Protected Sub FillTextBoxHd(ByVal Nmbr As String)
        ...
        Try
            ...
            lbProductLevel.Text = Dt.Rows(0)("LevelProduct").ToString
        Catch ex As Exception
    ...
    Protected Sub FillTextBoxProcess(ByVal KeyDt As String)
        ...
        Try
            ...
            If Dr.Length > 0 Then
                ...
                ViewState("FormulaName") = Dr(0)("FormulaName").ToString
            End If
        Catch ex As Exception
    ...
    Protected Sub btnProduct_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnProduct.Click
        ...
        Try
            Session("filter") = "SELECT Product_Code, Product_Name, Specification, Unit, '' AS LevelProduct FROM VMsProduct WHERE Fg_Active = 'Y'"
    ...
    Protected Sub tbProductCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbProductCode.TextChanged
        ...
        Dim SQLString As String
        Try
            SQLString = "SELECT Product_Code AS ProductCode, Product_Name AS ProductName, Specification, Unit, LevelProduct FROM VMsProduct WHERE Fg_Active = 'Y' AND Product_Code = " + QuotedStr(tbProductCode.Text)
            ...
            If Dt.Rows.Count > 0 Then
                ...
                lbProductLevel.Text = Dr("LevelProduct").ToString
    ...
    Protected Sub btnMaterial_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnMaterial.Click
        Dim ResultField As String
        Try
            Session("filter") = "EXEC S_PDBOMGetMaterial '', " + QuotedStr(lbProductLevel.Text)
    ...
    Protected Sub tbMaterialCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbMaterialCode.TextChanged
        ...
        Try
            ...
            SQLString = "EXEC S_PDBOMGetMaterial " + QuotedStr(tbMaterialCode.Text) + ", " + lbProductLevel.Text
            Dt = SQLExecuteQuery(SQLString, ViewState("DBConnection").ToString).Tables(0)
            If Dt.Rows.Count > 0 Then
                Dr = Dt.Rows(0)
    ...
    Protected Sub btnSaveProcess_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSaveProcess.Click
        ...
        Try
            ...
            If tbFormulaProcess.Text.Trim <> "" Then
                GetDt2()
            End If
            MovePanel(pnlEditDtProcess, pnlDtProcess)
            EnableHd(GetCountRecord(ViewState("Process")) = 0)
    ...
    Private Sub GetDt2()
        ...
        Try
            If ViewState("FormulaName") <> tbFormulaName.Text Or ViewState("FormulaName") = "" Then
                'BindDataDt("")

                dr2 = ViewState("Dt").Select("Process = " + QuotedStr(tbProcessCode.Text))
                For j As Integer = 0 To (dr2.Count - 1)
                    dr2(j).Delete()
                Next

                Dt = SQLExecuteQuery("SELECT * FROM V_PDBOMGetFormulaMaterial WHERE FormulaNo = " + QuotedStr(tbFormulaProcess.Text), ViewState("DBConnection").ToString).Tables(0)

                For i = 0 To Dt.Rows.Count - 1
                    If Not CekExistData(ViewState("Dt"), "Process,Material", tbProcessCode.Text + "|" + (Dt.Rows(i)("Material"))) Then
                        Dr = ViewState("Dt").NewRow
                        Dr("Process") = tbProcessCode.Text
                        Dr("Material") = Dt.Rows(i)("Material")
                        Dr("MaterialName") = Dt.Rows(i)("MaterialName")
                        Dr("MaterialAlternate") = ""
                        Dr("MaterialAlternateName") = ""
                        Dr("Qty") = Dt.Rows(i)("QtyBOM")
                        Dr("Unit") = Dt.Rows(i)("Unit")
                        Dr("Remark") = ""
                        Dr("Specification") = Dt.Rows(i)("Specification")
                        Dr("QtyCompare") = Dt.Rows(i)("QtyCompare")
                        Dr("UnitCompare") = Dt.Rows(i)("UnitCompare")
                        ViewState("Dt").Rows.Add(Dr)
                    End If
                Next
            ...
            ViewState("FormulaName") = ""
        Catch ex As Exception
            Throw New Exception("GetDt Error : " + ex.ToString)
        End Try
    End Sub
    ...
    Protected Sub btnGetDt_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnGetDt.Click
        ...
        Try
            ...
            Session("Filter") = "EXEC S_PDBOMGetMaterial  '', " + lbProductLevel.Text
    ...
    Private Sub totalingDt()
        Dim dr As DataRow
        Dim Qty, QtyCompare As Double
        Try
            Qty = 0
            QtyCompare = 0
            For Each dr In ViewState("Dt").Select("Process = " + QuotedStr(lblProcessCodeDt.Text.Trim))
                If Not dr.RowState = DataRowState.Deleted Then
                    Qty = Qty + CFloat(dr("Qty").ToString)
                    QtyCompare = QtyCompare + CFloat(dr("QtyCompare").ToString)
                End If
            Next
            lblTotalQty.Text = FormatFloat(Qty, 4)
            lblTotalQtyCompare.Text = FormatFloat(QtyCompare, 4)
        Catch ex As Exception
