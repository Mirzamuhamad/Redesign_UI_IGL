...
Partial Class TrMTNServiceRequest
    ...
    Protected Sub FillTextBoxHd(ByVal Nmbr As String)
        ...
        Try
            ...
            Else
                lbDokSvrReq.Text = Dt.Rows(0)("FileSrvReq").ToString
            End If
        Catch ex As Exception
    ...
    Protected Sub btnSaveDoc_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSaveDoc.Click
        Try
            ...
            If fubSvrReq.FileBytes.Length > 3500000 Then
                lbStatus.Text = MessageDlg("Ukuran File Terlalu Besar. !! Max Upload 3.5Mb")
                Exit Sub
            End If

            If Right(fubSvrReq.FileName, 4) <> ".pdf" Then
                lbStatus.Text = MessageDlg("Upload Pdf File Only !")
                Exit Sub
            End If
            Dim path2, namafile2, SQLString1 As String
            Dim dt As DataTable
            path2 = Server.MapPath("~/Image/DocumentWOPdf/") + tbCode.Text.Trim.Replace("/", "") + Format(Now, "-yyMMddHHmmss-") + fubSvrReq.FileName
            namafile2 = tbCode.Text.Trim.Replace("/", "") + Format(Now, "-yyMMddHHmmss-") + fubSvrReq.FileName

            SQLString1 = "UPDATE MTNServiceRequestHd SET FileSrvReq = " + QuotedStr(namafile2) + _
            " WHERE TransNmbr = " + QuotedStr(tbCode.Text)
            fubSvrReq.SaveAs(path2)
            SQLExecuteNonQuery(SQLString1, ViewState("DBConnection").ToString)

            dt = BindDataTransaction(GetStringHd, "TransNmbr = " + QuotedStr(tbCode.Text), ViewState("DBConnection").ToString)
            lbDokSvrReq.Text = dt.Rows(0)("FileSrvReq").ToString
            'lblmassageKTP.Visible = True
            fubSvrReq.Visible = False
            btnClearSvrReq.Visible = True

        Catch ex As Exception
            lbStatus.Text = "btnSaveDoc_Click Error : " + ex.ToString
        End Try
    End Sub

    Protected Sub lbDokSvrReq_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lbDokSvrReq.Click
        Try
            Dim dr As DataTable
            Dim filePath, URL As String
            dr = BindDataTransaction(GetStringHd, "TransNmbr = " + QuotedStr(tbCode.Text), ViewState("DBConnection").ToString)
        ...
        Catch ex As Exception
            lbStatus.Text = "lbDokSvrReq_Click Error : " + ex.ToString
        End Try
    End Sub

    Protected Sub btnClearSvrReq_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnClearSvrReq.Click
        Try
            Dim dr As DataTable
            Dim filePath As String
            dr = BindDataTransaction(GetStringHd, "TransNmbr = " + QuotedStr(tbCode.Text), ViewState("DBConnection").ToString)
            ...
            If File.Exists(Server.MapPath("~/Image/DocumentWOPdf/" + filePath)) = True Then
                ...
                lbDokSvrReq.Text = "Not yet uploaded"
                fubSvrReq.Visible = True
                btnClearSvrReq.Visible = False
            End If

        Catch ex As Exception
            lbStatus.Text = "btnClearSvrReq_Click Error : " + ex.ToString
        End Try
    End Sub
