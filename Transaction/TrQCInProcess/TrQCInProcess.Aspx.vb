...
Partial Class TrQCInProcess
    Inherits System.Web.UI.Page

    Protected con As New SqlConnection
    Protected da As New SqlDataAdapter
    'Protected GetStringHd As String = "Select * From V_QCInProcessHd WHERE UserId = " + QuotedStr(ViewState("UserId").ToString)

    Private Function GetStringHd() As String
        Return "Select DISTINCT   TransNmbr, Nmbr, TransDate, Status, QCType, ReffNmbr, ReffType, WorkCtr, WorkCtrName, Shift, ShiftName, InspectedDate, InspectedBy, Remark From V_QCInProcessHd "
    End Function
        ...
        Try
            ...
            If Not Session("Result") Is Nothing Then
                If ViewState("Sender") = "btnReffNmbr" Then
                    ...
                    ddlWorkCtr.SelectedValue = Session("Result")(1).ToString
                    ddlShift.SelectedValue = Session("Result")(2).ToString
                End If
                If ViewState("Sender") = "btnProd" Then
                    'LHP_No, WO_No, Product, Product_Name, specification, Warehouse, Location, Qty, Unit
                    tbLHPNo.Text = Session("Result")(0).ToString
                    tbWONo.Text = Session("Result")(1).ToString
                    tbProdCode.Text = Session("Result")(2).ToString
                    tbProdName.Text = Session("Result")(3).ToString
                    BindToText(tbSpec, Session("Result")(4).ToString)
                    ddlWrhsQC.SelectedValue = Session("Result")(5).ToString
                    ddlLocation.SelectedValue = Session("Result")(6).ToString
                    tbQty.Text = Session("Result")(7).ToString
                    BindToText(tbUnit, Session("Result")(8).ToString)
                End If
                ...
                If ViewState("Sender") = "btnGetLHP" Then
                    ...
                    For Each drResult In Session("Result").Rows
                        Row = ViewState("Dt").Select("LHPNo+'|'+WONo = " + QuotedStr(drResult("LHP_No") + "|" + drResult("WO_No")))
                        If Row.Count = 0 Then
                            dr = ViewState("Dt").NewRow
                            dr("LHPNo") = drResult("LHP_No")
                            dr("WONo") = drResult("WO_No")
                            ...
                            dr("Location") = TrimStr(drResult("Location"))
    ...
    Private Sub SetInit()
        FillRange(ddlRange)
        FillCombo(ddlWorkCtr, "select WorkCtrCode, WorkCtrName from MsWorkCtr", True, "WorkCtrCode", "WorkCtrName", ViewState("DBConnection"))
        ...
        FillCombo(ddlWrhsQC, "SELECT Wrhs_Code, Wrhs_Name FROM VMsWarehouse WHERE Wrhs_Type IN ( 'Owner', 'Deposit Out')", True, "Wrhs_Code", "Wrhs_Name", ViewState("DBConnection"))
        FillCombo(ddlLocation, "SELECT Location_Code, Location_Name FROM VMsWrhsLocation ", True, "Location_Code", "Location_Name", ViewState("DBConnection"))
        ...
        Me.tbQtyReject.Attributes.Add("OnKeyDown", "return PressNumeric();")
        Me.tbAcceptReject.Attributes.Add("OnKeyDown", "return PressNumeric();")
        Me.tbKoefAQL.Attributes.Add("OnKeyDown", "return PressNumeric();")
    ...
    Private Function GetStringDt(ByVal Nmbr As String) As String
        Return "SELECT * From V_QCInProcessDt WHERE TransNmbr = " + QuotedStr(Nmbr)
    End Function
    Private Function GetStringDtPartial(ByVal Nmbr As String) As String
        Return "SELECT * From V_QCInProcessDt2 WHERE TransNmbr = " + QuotedStr(Nmbr)
    End Function
            ...
            Else
                ...
                For j = 0 To (Nmbr.Length - 1)
                    ...
                    ElseIf ActionValue = "Post" Then
                        Result = ExecSPCommandGo(ActionValue, "S_QCInProcess", Nmbr(j), CInt(ViewState("GLYear")), CInt(ViewState("GLPeriod")), ViewState("UserId").ToString, ViewState("DBConnection").ToString)
                        If Trim(Result) <> "" Then
    ...
    Private Sub EnableHd(ByVal State As Boolean)
        Try
            ...
            btnReffNmbr.Visible = State And (ddlQCType.SelectedValue = "Outgoing Inspection" Or ddlQCType.SelectedValue = "Repeated")
            ddlWorkCtr.Enabled = State And ddlQCType.SelectedValue <> "Outgoing Inspection"
            ddlShift.Enabled = State And ddlQCType.SelectedValue <> "Outgoing Inspection"
        Catch ex As Exception
    ...
    Private Sub BindDataDtPartial(ByVal Nmbr As String)
        ...
        Try
            ...
            Drow = ViewState("Dt2").Select("LHPNo=" + QuotedStr(lbProductDt2.Text))
    ...
    Protected Sub btnSaveDt_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnSaveDt.Click
        ...
        Try
            If ViewState("StateDt") = "Edit" Then
                If ViewState("DtValue") <> tbLHPNo.Text + "|" + tbWONo.Text Then
                    If CekExistData(ViewState("Dt"), "LHPNo,WONo", tbLHPNo.Text + "|" + tbWONo.Text) Then
                        lbStatus.Text = "LHP " + tbLHPNo.Text + " WO " + tbWONo.Text + " Product '" + tbProdName.Text + "' has already exists"
                        Exit Sub
                    End If
                End If
                Row = ViewState("Dt").Select("LHPNo+'|'+WONo = " + QuotedStr(ViewState("DtValue")))(0)
                If CekDt() = False Then
                    Exit Sub
                End If
                Row.BeginEdit()
                Row("LHPNo") = tbLHPNo.Text
                Row("WONo") = tbWONo.Text
                Row("Product") = tbProdCode.Text
                Row("ProductName") = tbProdName.Text
                Row("Specification") = tbSpec.Text
                Row("Warehouse") = ddlWrhsQC.SelectedValue
                Row("Warehouse_Name") = ddlWrhsQC.SelectedItem.Text
                ...
                Row("QCStatus") = ddlStatusQC.SelectedValue
            ...
            Else
                ...
                Dim dr As DataRow
                dr = ViewState("Dt").NewRow
                dr("LHPNo") = tbLHPNo.Text
                dr("WONo") = tbWONo.Text
                dr("Product") = tbProdCode.Text
                dr("ProductName") = tbProdName.Text
                dr("Specification") = tbSpec.Text
                dr("Warehouse") = ddlWrhsQC.SelectedValue
                dr("Warehouse_Name") = ddlWrhsQC.SelectedItem.Text
                ...
                dr("QCStatus") = ddlStatusQC.SelectedValue
    ...
    Private Sub SaveAll()
        ...
        Try
            ...
            If ViewState("StateSave") = "Insert" Then
                tbTransNo.Text = GetAutoNmbr("QCIP", "N", Year(tbDate.SelectedValue), Month(tbDate.SelectedValue), "", ViewState("DBConnection").ToString)
                ...
                SQLString = "INSERT INTO QCInProcessHd (TransNmbr, STATUS, Transdate, QCType, WorkCtr, ReffType, ReffNmbr, Shift, InspectedDate, InspectedBy, Remark, UserPrep, DatePrep) " + _
                "SELECT '" + tbTransNo.Text + "', 'H', '" + Format(tbDate.SelectedValue, "yyyy-MM-dd") + "', " + QuotedStr(ddlQCType.SelectedValue) + ", " + QuotedStr(ddlWorkCtr.SelectedValue) + ", " + QuotedStr(tbReffType.Text) + ", " + QuotedStr(tbReffNmbr.Text) + ", " + QuotedStr(ddlShift.SelectedValue) + ", '" + Format(tbInspectionDate.SelectedValue, "yyyy-MM-dd") + "', " + _
                QuotedStr(tbInspectedBy.Text) + ", " + QuotedStr(tbRemark.Text) + "," + QuotedStr(ViewState("UserId").ToString) + ", getDate()"
                ViewState("Reference") = tbTransNo.Text
            Else
                Dim cekStatus As String
                cekStatus = SQLExecuteScalar("Select Status FROM QCInProcessHd WHERE TransNmbr = " + QuotedStr(tbTransNo.Text), ViewState("DBConnection").ToString)
                If cekStatus = "P" Then
                    lbStatus.Text = MessageDlg("Save failed... Data has already posted by another user")
                    Exit Sub
                End If
                SQLString = "UPDATE QCInProcessHd SET Transdate = '" + Format(tbDate.SelectedValue, "yyyy-MM-dd") + "', QCType = " + QuotedStr(ddlQCType.SelectedValue) + ", WorkCtr = " + QuotedStr(ddlWorkCtr.SelectedValue) + ", Shift = " + QuotedStr(ddlShift.SelectedValue) + ", InspectedBy = " + QuotedStr(tbInspectedBy.Text) + _
                ", ReffType = " + QuotedStr(tbReffType.Text) + ", ReffNmbr = " + QuotedStr(tbReffNmbr.Text) + ", InspectedDate = '" + Format(tbInspectionDate.SelectedValue, "yyyy-MM-dd") + "',  Remark = " + QuotedStr(tbRemark.Text) + ", DatePrep = getDate()" + _
            ...
            con.Open()
            Dim cmdSql As New SqlCommand("SELECT  TransNmbr, LHPNo, WONo, Product, Qty, SamplingSize, QtySS, Unit, Warehouse, Location, QCStatus, Remark FROM QCInProcessDt WHERE TransNmbr = '" & ViewState("Reference") & "'", con)
            ...
            Dim Dt As New DataTable("QCInProcessDt")
            ...
            If Not ViewState("Dt2") Is Nothing Then
                SqlString2 = " SELECT  TransNmbr, LHPNo, WONo, ItemNo, Defect, KoefAQL, AcceptReject, QtyReject, Remark FROM QCInProcessDt2 WHERE TransNmbr = " + QuotedStr(ViewState("Reference"))
                ...
                Dim DtPartial As New DataTable("QCInProcessDt2")
        ...
        Try
            ...
            For Each dr In ViewState("Dt").Rows
                If Not dr.RowState = DataRowState.Deleted Then
                    ExistRow = ViewState("Dt2").Select("LHPNo+'|'+WONo = " + QuotedStr(dr("LHPNo") + "|" + dr("WONo")))
                    If ExistRow.Count = 0 Then
                        lbStatus.Text = MessageDlg("Detail QC must have at least 1 record for LHP " + dr("LHPNo") + " WO " + dr("WONo"))
                        Exit Sub
                    End If
                End If
            Next
    ...
    Private Sub ClearHd()
        Try
            ...
            tbReffNmbr.Text = ""
            tbReffType.Text = ""
            tbInspectedBy.Text = ViewState("UserId").ToString
            ddlQCType.SelectedIndex = 0
            ddlWorkCtr.SelectedValue = ""
            ddlShift.SelectedValue = ""
            MultiView1.ActiveViewIndex = 0
            btnReffNmbr.Visible = (ddlQCType.SelectedValue = "Outgoing Inspection" Or ddlQCType.SelectedValue = "Repeated")
            ddlWorkCtr.Enabled = ddlQCType.SelectedValue <> "Outgoing Inspection"
            ddlShift.Enabled = ddlQCType.SelectedValue <> "Outgoing Inspection"
        Catch ex As Exception
    ...
    Private Sub Cleardt()
        Try
            ...
            ddlStatusQC.SelectedValue = "Passed"
        Catch ex As Exception
    ...
    Protected Sub btnAddPartial_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnAddPartial.Click, btnAddDt2Ke2.Click
        ...
        Try
            If CekHd() = False Then
                Exit Sub
            End If
            If lbLHPNoDt2.Text = "" Or lbWONoDt2.Text = "" Then
                Exit Sub
            End If
            Cleardt2()
            'untuk ambil data di Dt
            'dr = ViewState("Dt").Select("LHPNo+'|'+WONo =" + QuotedStr(lbLHPNoDt2.Text + "|" + lbWONoDt2.Text))(0)
            lbItemNo.Text = GetNewItemNo2(ViewState("Dt2"), lbLHPNoDt2.Text + "|" + lbWONoDt2.Text)
    ...
    Function CekHd() As Boolean
        Try
            ...
            '    Return False
            'End If
            If ddlWorkCtr.SelectedValue = "" Then
                lbStatus.Text = MessageDlg("Work Center must have value")
                ddlWorkCtr.Focus()
                Return False
            End If
            If ddlQCType.SelectedValue = "Outgoing Inspection" And tbReffNmbr.Text = "" Then
                lbStatus.Text = MessageDlg("Outgoing QC No must have value")
                btnReffNmbr.Focus()
                Return False
            End If
            If ddlShift.SelectedValue.Trim = "" Then
                lbStatus.Text = MessageDlg("Shift must have value")
                ddlShift.Focus()
                Return False
            End If
            If tbInspectionDate.SelectedDate = Nothing Then
                lbStatus.Text = MessageDlg("Inspection Date must have value")
                tbInspectionDate.Focus()
                Return False
            End If
            If tbInspectedBy.Text = "" Then
                lbStatus.Text = MessageDlg("Inspection by must have value")
                tbInspectedBy.Focus()
                Return False
            End If
            Return True
        ...
        Try
            If Not (Dr Is Nothing) Then
                If Dr.RowState = DataRowState.Deleted Then
                    Return True
                End If
                If Dr("LHPNo").ToString.Trim = "" Then
            ...
            Else
                ...
                If CFloat(tbQtySS.Text) <= 0 Then
                    ...
                    Return False
                End If
                If ddlWrhsQC.SelectedValue = "" Then
    ...
    Protected Sub LbAdvSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles LbAdvSearch.Click
        ...
        Try
            ...
            FilterName = "Reference, Date, QC Type, Work Center, Shift, Inspected Date, Inspected By, Product, Product Name, Remark"
            FilterValue = "TransNmbr, dbo.FormatDate(TransDate), QCType, WorkCtrName, ShiftName, dbo.FormatDate(InspectedDate), InspectedBy, Product, ProductName, Remark"
    ...
    Protected Sub GridDt_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GridDt.RowCommand
        ...
        Try
            ...
            ElseIf e.CommandName = "DetailPartial" Then
                ...
                lbLHPNoDt2.Text = GVR.Cells(2).Text
                lbWONoDt2.Text = GVR.Cells(3).Text
                lbProductDt2.Text = GVR.Cells(4).Text
                lbProductDtName2.Text = " - " + GVR.Cells(5).Text
                lbQtyDt2.Text = GVR.Cells(7).Text + " " + GVR.Cells(8).Text
                lbSamplingSize.Text = GVR.Cells(9).Text

                If TrimStr(lbSamplingSize.Text) = "" Then
                    lbStatus.Text = "Sampling Size must be have value"
                    Exit Sub
                End If
                ...
                If ViewState("Dt2") Is Nothing Then
                    BindDataDtPartial(ViewState("Reference"))
                Else
                    drow = ViewState("Dt2").Select("LHPNo+'|'+WONo =" + QuotedStr(GVR.Cells(2).Text + "|" + GVR.Cells(3).Text))
                    If drow.Length > 0 Then
                        BindGridDt(drow.CopyToDataTable, GridPartial)
                        GridPartial.Columns(0).Visible = Not ViewState("StateHd") = "View"
                    Else
    ...
    Protected Sub GridDt_RowDeleting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewDeleteEventArgs) Handles GridDt.RowDeleting
        ...
        dr = ViewState("Dt").Select("LHPNo+'|'+WONo = " + QuotedStr(GVR.Cells(2).Text + "|" + GVR.Cells(3).Text))
        dr(0).Delete()

        dr2 = ViewState("Dt2").Select("LHPNo+'|'+WONo = " + QuotedStr(GVR.Cells(2).Text + "|" + GVR.Cells(3).Text))
    ...
    Protected Sub GridDt_RowEditing(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewEditEventArgs) Handles GridDt.RowEditing
        ...
        Try
            ...
            ViewState("DtValue") = tbLHPNo.Text + "|" + tbWONo.Text
            MovePanel(pnlDt, pnlEditDt)
            EnableHd(False)
    ...
    Protected Sub FillTextBoxHd(ByVal Nmbr As String)
        ...
        Try
            ...
            BindToDropList(ddlQCType, Dt.Rows(0)("QCType").ToString)
            BindToDate(tbInspectionDate, Dt.Rows(0)("InspectedDate").ToString)
            BindToText(tbInspectedBy, Dt.Rows(0)("InspectedBy").ToString)
    ...
    Protected Sub FillTextBoxDt(ByVal KeyDt As String)
        Dim Dr As DataRow()
        Try
            Dr = ViewState("Dt").select("LHPNo+'|'+WONo = " + QuotedStr(KeyDt))
            If Dr.Length > 0 Then
                BindToText(tbLHPNo, Dr(0)("LHPNo").ToString)
                ...
                BindToDropList(ddlWrhsQC, Dr(0)("Warehouse").ToString)
                BindToDropList(ddlLocation, Dr(0)("Location").ToString)
                'ViewState("Location") = Dr(0)("Location").ToString
                'FillCombo(ddlLocation, "EXEC S_GetWrhsLocation " + QuotedStr(ddlWrhsQC.SelectedValue), True, "Location_Code", "Location_Name", ViewState("DBConnection"))
                ...
                BindToDropList(ddlStatusQC, Dr(0)("QCStatus").ToString)
    ...
    Protected Sub btnProd_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnProd.Click
        ...
        Try
            Session("filter") = "Select LHP_No, WO_No, Product, Product_Name, Specification, Warehouse, Warehouse_Name, Location, Location_Name, Qty, Unit from V_QCInProcessReff WHERE WorkCtr = " + QuotedStr(ddlWorkCtr.SelectedValue)
            ResultField = "LHP_No, WO_No, Product, Product_Name, specification, Warehouse, Location, Qty, Unit"
    ...
    Protected Sub FillTextBoxDt2(ByVal LHPWOno As String, ByVal ItemNo As String)
        Dim Dr As DataRow()
        Try
            Dr = ViewState("Dt2").select("LHPNo+'|'+WONo = " + QuotedStr(LHPWOno) + " AND ItemNo =" + ItemNo)
            If Dr.Length > 0 Then
                ...
                BindToText(tbKoefAQL, Dr(0)("KoefAQL").ToString)
                BindToText(tbAcceptReject, Dr(0)("AcceptReject").ToString)
    ...
    Protected Sub btnSaveDt2_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnSaveDt2.Click
        Try
            ...
            End If
            Dim ExistRow As DataRow()
            ExistRow = ViewState("Dt2").Select("LHPNo+'|'+WONo = " + QuotedStr(lbLHPNoDt2.Text + "|" + lbWONoDt2.Text) + " and ItemNo = " + lbItemNo.Text)
            If ViewState("StateDtPartial") = "Edit" Then
                ...
                '    Exit Sub
                'End If
                Row = ViewState("Dt2").Select("LHPNo+'|'+WONo = " + QuotedStr(lbLHPNoDt2.Text + "|" + lbWONoDt2.Text) + " and ItemNo = " + ViewState("ItemNo").ToString)(0)
                Row.BeginEdit()
                Row("LHPNo") = lbLHPNoDt2.Text
                Row("WONo") = lbWONoDt2.Text
                ...
                Row("KoefAQL") = tbKoefAQL.Text
                Row("AcceptReject") = tbAcceptReject.Text
            ...
            Else
                ...
                dr("LHPNo") = lbLHPNoDt2.Text
                dr("WONo") = lbWONoDt2.Text
                ...
                dr("KoefAQL") = tbKoefAQL.Text
                dr("AcceptReject") = tbAcceptReject.Text
            ...
            Dim drow As DataRow()
            drow = ViewState("Dt2").Select("LHPNo+'|'+WONo =" + QuotedStr(lbLHPNoDt2.Text + "|" + lbWONoDt2.Text))
            If drow.Length > 0 Then
                BindGridDt(drow.CopyToDataTable, GridPartial)
                GridPartial.Columns(0).Visible = Not ViewState("StateHd") = "View"
            Else
    ...
    Protected Sub GridPartial_RowDeleting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewDeleteEventArgs) Handles GridPartial.RowDeleting
        Try
            ...
            dr = ViewState("Dt2").Select("LHPNo+'|'+WONo = " + QuotedStr(lbLHPNoDt2.Text + "|" + lbWONoDt2.Text) + " AND ItemNo = " + GVR.Cells(1).Text)
            dr(0).Delete()
            EnableHd(Not DtExist())
            Dim drow As DataRow()
            drow = ViewState("Dt2").Select("LHPNo+'|'+WONo = " + QuotedStr(lbLHPNoDt2.Text + "|" + lbWONoDt2.Text))
    ...
    Protected Sub GridPartial_RowEditing(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewEditEventArgs) Handles GridPartial.RowEditing
        ...
        Try
            ...
            FillTextBoxDt2(lbLHPNoDt2.Text + "|" + lbWONoDt2.Text, GVR.Cells(1).Text)
    ...
    Protected Sub btnGetData_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnGetData.Click
        ...
        Try
            Session("Result") = Nothing
            If ddlWorkCtr.SelectedValue = "" Then
                lbStatus.Text = "Work Center must have value"
                Exit Sub
            End If
            Session("filter") = "Select LHP_No, Shift, WO_No, Product, Product_Name, Specification, Warehouse, Warehouse_Name, Location, Location_Name, Qty, Unit from V_QCInProcessReff WHERE WorkCtr = " + QuotedStr(ddlWorkCtr.SelectedValue)
            ResultField = "LHP_No, WO_No, Product, Product_Name, Specification, Warehouse, Warehouse_Name, Location, Location_Name, Qty, Unit"
    ...
    Protected Sub btnDefect_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnDefect.Click
        ...
        Try
            ...
            ResultField = "Defect_Code, Defect_Name, Defect_Group_Name, Standard, Acceptable"
    ...
    Public Function GetNewItemNo2(ByVal Dt As DataTable, ByVal LHPWONo As String) As String
        Dim Row As DataRow()
        Dim R As DataRow
        Dim MaxItem As Integer = 0
        Row = Dt.Select("LHPNo+'|'+WONo = " + QuotedStr(LHPWONo))
        For Each R In Row
    ...
    Protected Sub ddlWrhsQC_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlWrhsQC.SelectedIndexChanged
        'Try
        '    FillCombo(ddlLocation, "EXEC S_GetWrhsLocation " + QuotedStr(ddlWrhsQC.SelectedValue), True, "Location_Code", "Location_Name", ViewState("DBConnection"))
        '    ddlLocation.SelectedValue = ""
        'Catch ex As Exception
        '    lbStatus.Text = "ddlWrhsQC_SelectedIndexChanged Error : " + ex.ToString
        'End Try
    End Sub

    Protected Sub ddlQCType_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlQCType.SelectedIndexChanged
        Try
            btnReffNmbr.Visible = (ddlQCType.SelectedValue = "Outgoing Inspection" Or ddlQCType.SelectedValue = "Repeated")
            If ddlQCType.SelectedValue = "Outgoing Inspection" Then
                tbReffType.Text = "OQC"
            ElseIf ddlQCType.SelectedValue = "Repeated" Then
                tbReffType.Text = "LHP"
            Else
                tbReffType.Text = ""
            End If
            ...
            ddlShift.Enabled = ddlQCType.SelectedValue <> "Outgoing Inspection"
        Catch ex As Exception
            lbStatus.Text = "ddlQCType_SelectedIndexChanged Error : " + ex.ToString
        End Try
    End Sub
    ...
    Protected Sub btnReffNmbr_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnReffNmbr.Click
        ...
        Try
            Session("DBConnection") = ViewState("DBConnection")
            If ddlShift.SelectedValue = "" And ddlWorkCtr.SelectedValue = "" Then
                Session("filter") = "Select distinct Reference, Date, dbo.FormatDate(ProductionDate) AS Production_Date, WorkCtr, Shift, Remark from V_QCInProcessReff WHERE Type = " + QuotedStr(tbReffType.Text)
            ElseIf ddlShift.SelectedValue = "" And ddlWorkCtr.SelectedValue <> "" Then
                Session("filter") = "Select distinct Reference, Date, dbo.FormatDate(ProductionDate) AS Production_Date, WorkCtr, Shift, Remark from V_QCInProcessReff WHERE Type = " + QuotedStr(tbReffType.Text) + " AND WorkCtr = " + QuotedStr(ddlWorkCtr.SelectedValue)
            ElseIf ddlShift.SelectedValue <> "" And ddlWorkCtr.SelectedValue = "" Then
                Session("filter") = "Select distinct Reference, Date, dbo.FormatDate(ProductionDate) AS Production_Date, WorkCtr, Shift, Remark from V_QCInProcessReff WHERE Type = " + QuotedStr(tbReffType.Text) + " AND Shift = " + QuotedStr(ddlShift.SelectedValue)
            Else
                Session("filter") = "Select distinct Reference, Date, dbo.FormatDate(ProductionDate) AS Production_Date, WorkCtr, Shift, Remark from V_QCInProcessReff WHERE Type = " + QuotedStr(tbReffType.Text) + " AND Shift = " + QuotedStr(ddlShift.SelectedValue) + " and WorkCtr = " + QuotedStr(ddlWorkCtr.SelectedValue)
            End If
            ResultField = "Reference, WorkCtr, Shift"
            ViewState("Sender") = "btnReffNmbr"
            Session("Column") = ResultField.Split(",")
            AttachScript("OpenSearchDlg();", Page, Me.GetType())
        Catch ex As Exception
            lbStatus.Text = "btnReffNmbr_Click Error : " + ex.ToString
        End Try
    End Sub
End Class
