...
Partial Class TrMTNWOService
    Inherits System.Web.UI.Page
    Protected con As New SqlConnection
    Protected da As New SqlDataAdapter
    Protected GetStringHd As String = "SELECT * FROM V_MTNWOServiceHd"
    Public strTransNmbr As String
    Private Shared PageSize As Integer = 5
    Dim tb As TextBox

    Private Function GetStringDt(ByVal Nmbr As String) As String
        Return "SELECT * FROM V_MTNWOServiceDt WHERE WoServiceNo = " + QuotedStr(Nmbr)
    End Function
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If Not Session("Result") Is Nothing Then
                ...
                End If
                If ViewState("Sender") = "btnServiceReqNo" Then
                    tbServiceReqNo.Text = Session("Result")(0).ToString
                    tbAreaCode.Text = Session("Result")(1).ToString
                    tbAreaName.Text = Session("Result")(2).ToString
                    tbKavlingCode.Text = Session("Result")(3).ToString
                    tbKavlingName.Text = Session("Result")(4).ToString
                    tbRequestBy.Text = Session("Result")(5).ToString
                    tbContactNo.Text = Session("Result")(6).ToString
                    tbEmail.Text = Session("Result")(7).ToString
                    tbRemark.Text = Session("Result")(8).ToString
                End If
    ...
    Private Sub SetInit()
        ...
        Try
            ...
            End If
            lbTitle.Text = "Work Order Service" 'Request.QueryString("MenuName").ToString
    ...
    Protected Sub BtnGo_Click(ByVal sender As Object, ByVal e As EventArgs) Handles BtnGo.Click, btnGo2.Click
        ...
        Try
            ...
            End If
            'If (ActionValue = "PrintBlank") Or (ActionValue = "PrintFull") Then
            If ActionValue = "Print" Then
                ...
                Next
                Result = Result + "'"
                Session("SelectCommand") = "EXEC S_MTFormWOService '" + Result + "'," + QuotedStr(ViewState("UserId"))
                Session("ReportFile") = ".../../../Rpt/FormWOService.frx"
            ...
            Else
                ...
                For j = 0 To (Nmbr.Length - 1)
                    If Nmbr(j) = "" Then
                        Exit For
                    Else
                        Result = ExecSPCommandGo(ActionValue, "S_MTWOService", Nmbr(j), CInt(Session(Request.QueryString("KeyId"))("Year")), CInt(Session(Request.QueryString("KeyId"))("Period")), ViewState("UserId").ToString, ViewState("DBConnection").ToString)
                        If Trim(Result) <> "" Then
    ...
    Protected Sub btnComplete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnComplete.Click
        ...
        Try
            ...
            'cekStatus = SQLExecuteScalar("SELECT Status FROM MTNWOServiceHd WHERE TransNmbr = " + QuotedStr(tbCode.Text), ViewState("DBConnection").ToString)
            'If cekStatus = "P" Then
            '    lbStatus.Text = MessageDlg("Save failed... Data has already posted by another user")
            '    Exit Sub
            'End If
            If SQLExecuteScalar("SELECT WoServiceNo FROM STCBuktiOutMaterialHd WHERE Status = 'P' AND WoServiceNo = " + QuotedStr(tbCode.Text), ViewState("DBConnection").ToString).Length <= 0 Then
                lbStatus.Text = "No. WO Service has not exist in Bukti Material Keluar"
                Exit Sub
            End If
            cekStatus = SQLExecuteScalar("SELECT Status FROM MTNWOServiceHd WHERE TransNmbr = " + QuotedStr(tbCode.Text), ViewState("DBConnection").ToString)
            If cekStatus = "F" Then
                lbStatus.Text = MessageDlg("Complete failed... Data has already Completed by another user")
                Exit Sub
            End If
            If cekStatus <> "P" Then
                lbStatus.Text = MessageDlg("Complete failed... Status Data must be Posting (P)")
                Exit Sub
            End If

            SQLString = "UPDATE MTNWOServiceHd SET TransDate = " + QuotedStr(Format(tbDate.SelectedValue, "yyyy-MM-dd")) + ", ServiceReqNo =" + QuotedStr(tbServiceReqNo.Text) + _
            ", AreaCode =" + QuotedStr(tbAreaCode.Text) + ", KavlingCode =" + QuotedStr(tbKavlingCode.Text) + ", RequestBy =" + QuotedStr(tbRequestBy.Text) + _
            ", ContactNo =" + QuotedStr(tbContactNo.Text) + ", Email =" + QuotedStr(tbEmail.Text) + ", TechInCharge = " + QuotedStr(tbTechInCharge.Text) + _
            ", Remark = " + QuotedStr(tbRemark.Text) + ", Technician1 = " + QuotedStr(tbTechnician1.Text) + ", Manhour1 = " + QuotedStr(tbManhour1.Text.Replace(",", "")) + _
            ", RemarkTech1 = " + QuotedStr(tbRemarkTech1.Text) + ", Technician2 = " + QuotedStr(tbTechnician2.Text) + ", Manhour2 = " + QuotedStr(tbManhour2.Text.Replace(",", "")) + _
            ", RemarkTech2 = " + QuotedStr(tbRemarkTech2.Text) + ", Technician3 = " + QuotedStr(tbTechnician3.Text) + ", Manhour3 = " + QuotedStr(tbManhour3.Text.Replace(",", "")) + _
            ", RemarkTech3 = " + QuotedStr(tbRemarkTech3.Text) + ", RemarkJournal = " + QuotedStr(tbRemarkJournal.Text) + ", NoBMK = " + QuotedStr(tbBMKNo.Text) + _
            ", Kesimpulan = " + QuotedStr(tbKesimpulan.Text) + ", Status = " + QuotedStr("F") + ", UserComplete= " + QuotedStr(ViewState("UserId").ToString) + ", DateComplete = GetDate()" + _
            ...
            lbStsComplete.Text = "Work Order Service Complete"
        Catch ex As Exception
    ...
    Function CekHd() As Boolean
        Try
            ...
            '    Return False
            'End If
            If tbServiceReqNo.Text.Trim = "" Then
                lbStatus.Text = MessageDlg("Service Request No must have value")
                tbServiceReqNo.Focus()
                Return False
            End If
            ...
            If tbEmail.Text.Trim = "" Then
                ...
                Return False
            End If
            If tbTechInCharge.Text.Trim = "" Then
                lbStatus.Text = MessageDlg("Tech In Charge must have value")
                tbTechInCharge.Focus()
                Return False
            End If
    ...
    Protected Sub FillTextBoxHd(ByVal Nmbr As String)
        ...
        Try
            ...
            BindToText(tbServiceReqNo, Dt.Rows(0)("ServiceReqNo").ToString)
            ...
            BindToText(tbTechInCharge, Dt.Rows(0)("TechInCharge").ToString)
            BindToText(tbRemark, Dt.Rows(0)("Remark").ToString)
            BindToText(tbTechnician1, Dt.Rows(0)("Technician1").ToString)
            BindToText(tbManhour1, Dt.Rows(0)("Manhour1").ToString)
            BindToText(tbRemarkTech1, Dt.Rows(0)("RemarkTech1").ToString)
            BindToText(tbTechnician2, Dt.Rows(0)("Technician2").ToString)
            BindToText(tbManhour2, Dt.Rows(0)("Manhour2").ToString)
            BindToText(tbRemarkTech2, Dt.Rows(0)("RemarkTech2").ToString)
            BindToText(tbTechnician3, Dt.Rows(0)("Technician3").ToString)
            BindToText(tbManhour3, Dt.Rows(0)("Manhour3").ToString)
            BindToText(tbRemarkTech3, Dt.Rows(0)("RemarkTech3").ToString)
            BindToText(tbRemarkJournal, Dt.Rows(0)("RemarkJournal").ToString)
            BindToText(tbBMKNo, Dt.Rows(0)("NoBMK").ToString)
            BindToText(tbKesimpulan, Dt.Rows(0)("Kesimpulan").ToString)
    ...
    Protected Sub FillTextBoxNoBMK(ByVal Nmbr As String)
        Dim Dt As DataTable
        Dim NoBMK As String
        Try
            Dt = BindDataTransaction(GetStringDt(Nmbr), "WoServiceNo = " + QuotedStr(Nmbr), ViewState("DBConnection").ToString)
            'tbCode.Text = Nmbr
            If Dt.Rows(0)("TransNmbr").ToString = "" Then
                NoBMK = ""
            Else
                NoBMK = Dt.Rows(0)("TransNmbr").ToString
            End If
            BindToText(tbBMKNo, NoBMK)
        Catch ex As Exception
            Throw New Exception("fill text box header error : " + ex.ToString)
        End Try
    End Sub
    ...
    Private Sub SaveAll()
        ...
        Try
            ...
            If ViewState("StateSave") = "Insert" Then
                ...
                tbCode.Text = GetAutoNmbr("WOS", "Y", Year(tbDate.SelectedValue), Month(tbDate.SelectedValue), "", ViewState("DBConnection").ToString)

                SQLString = "INSERT INTO MTNWOServiceHd(TransNmbr,TransDate,Status,ServiceReqNo,AreaCode,KavlingCode,RequestBy,ContactNo,Email,TechInCharge,Remark,FgReport,FgActive,UserPrep,DatePrep) " + _
                "SELECT " + QuotedStr(tbCode.Text) + ", " + QuotedStr(Format(tbDate.SelectedValue, "yyyy-MM-dd")) + ", 'H', " + QuotedStr(tbServiceReqNo.Text) + ", " + _
                QuotedStr(tbAreaCode.Text) + ", " + QuotedStr(tbKavlingCode.Text) + ", " + QuotedStr(tbRequestBy.Text) + ", " + QuotedStr(tbContactNo.Text) + ", " + _
                QuotedStr(tbEmail.Text) + ", " + QuotedStr(tbTechInCharge.Text) + ", " + QuotedStr(tbRemark.Text) + ", " + QuotedStr("Y") + ", " + QuotedStr("Y") + "," + _
            ...
            Else
                ...
                ", RemarkTech2 = " + QuotedStr(tbRemarkTech2.Text) + ", Technician3 = " + QuotedStr(tbTechnician3.Text) + ", Manhour3 = " + QuotedStr(tbManhour3.Text.Replace(",", "")) + _
                ", RemarkTech3 = " + QuotedStr(tbRemarkTech3.Text) + ", RemarkJournal = " + QuotedStr(tbRemarkJournal.Text) + ", NoBMK = " + QuotedStr(tbBMKNo.Text) + _
                ", Kesimpulan = " + QuotedStr(tbKesimpulan.Text) + ", UserPrep= " + QuotedStr(ViewState("UserId").ToString) + ", DatePrep = GetDate()" + _
                " WHERE TransNmbr = " + QuotedStr(tbCode.Text)
            End If
            SQLString = ChangeQuoteNull(SQLString)
            SQLString = SQLString.Replace("'1900-01-01'", "NULL")
        ...
        Try
            ...
            If e.CommandName = "Go" Then
                ...
                ElseIf DDL.SelectedValue = "Complete" Then
                    If GVR.Cells(3).Text = "H" Or GVR.Cells(3).Text = "G" Then
                        lbStatus.Text = MessageDlg("Status Data must be Posting (P)")
                        Exit Sub
                    End If
                    ...
                    If GVR.Cells(3).Text = "P" Then
                        ...
                        Dim CekNoBMK, CekBmkPosting As String
                        CekNoBMK = SQLExecuteScalar("SELECT TransNmbr FROM STCBuktiOutMaterialHD WHERE WoServiceNo = " + QuotedStr(GVR.Cells(2).Text), ViewState("DBConnection"))
                        If CekNoBMK = "" Then
                            lbStatus.Text = MessageDlg("No BMK (Bukti Material Keluar) not exist")
                            Exit Sub
                        End If
                        CekBmkPosting = SQLExecuteScalar("SELECT TransNmbr FROM STCBuktiOutMaterialHD WHERE Status = 'P' AND WoServiceNo = " + QuotedStr(GVR.Cells(2).Text), ViewState("DBConnection"))
                        'lbStatus.Text = CekNoBMK
                        If CekBmkPosting = "" Then
                            lbStatus.Text = MessageDlg("No BMK (Bukti Material Keluar) in Posting Status not exist")
                            Exit Sub
                        End If
                        'CekStatus = SQLExecuteScalar("SELECT Status FROM MTNWOServiceHd WHERE TransNmbr = " + QuotedStr(tbCode.Text), ViewState("DBConnection").ToString)
                        'If CekStatus() = "P" Then
                        ...
                        BindDataDt(ViewState("TransNmbr"))
                        FillTextBoxHd(ViewState("TransNmbr"))
                        FillTextBoxNoBMK(ViewState("TransNmbr"))
                        ViewState("StateHd") = "Edit"
                        ModifyInput(False, pnlInput)
                        ModifyInput2(False, pnlInput, PnlDt, GridDt)
                        tbBMKNo.Enabled = False
                        'ModifyInput(True, PnlDt)
                        'ModifyInput(True, pnlTechnician)
                        GridDt.Columns(0).Visible = False
                        pnlTechnician.Visible = True
                        btnSaveAll.Visible = False
                        btnSaveTrans.Visible = False
                    ...
                    End If
                ElseIf DDL.SelectedValue = "Print Blank" Then
                    If GVR.Cells(3).Text = "P" Then
                        Try
                            ...
                            If CekMenu <> "" Then
                                ...
                                Exit Sub
                            End If
                            Session("SelectCommand") = "EXEC S_MTFormWOService '" + GVR.Cells(2).Text + "'," + QuotedStr(ViewState("UserId"))
                            Session("ReportFile") = ".../../../Rpt/FormWOServiceBlank.frx"
                            Session("DBConnection") = ViewState("DBConnection")
                            AttachScript("openprintdlg();", Page, Me.GetType)
                            'Session("SelectCommand") = "EXEC S_MTFormWOService '''" + GVR.Cells(2).Text + "'''," + QuotedStr(ViewState("UserId"))
                        Catch ex As Exception
                        ...
                        End Try
                    Else
                        lbStatus.Text = MessageDlg("Printout can't open")
                        Exit Sub
                    End If
                ElseIf DDL.SelectedValue = "Print Complete" Then
                    If GVR.Cells(3).Text = "F" Then
                        Try
                            CekMenu = CheckMenuLevel("Print", ViewState("MenuLevel").Rows(0))
                            If CekMenu <> "" Then
    ...
    Protected Sub btnServiceReqNo_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnServiceReqNo.Click
        Dim ResultField As String
        Try
            Session("filter") = "SELECT TransNmbr,AreaCode,AreaName,KavlingCode,KavlingName,RequestBy,ContactNo,Email,Remark FROM V_MTNGetNoServiceReq "
            ResultField = "TransNmbr,AreaCode,AreaName,KavlingCode,KavlingName,RequestBy,ContactNo,Email,Remark"
            ViewState("Sender") = "btnServiceReqNo"
            Session("Column") = ResultField.Split(",")
            Session("DBConnection") = ViewState("DBConnection")
            AttachScript("OpenPopup();", Page, Me.GetType())
        Catch ex As Exception
            lbStatus.Text = "btnServiceReqNo_Click Error : " + ex.ToString
        End Try
    End Sub
    ...
    '    Dim ResultField As String
    '    Try
    '        Session("filter") = "SELECT IdKavling, KavlingName FROM V_StructureKavling WHERE IdArea = " + QuotedStr(tbAreaCode.Text)
    '        ResultField = "IdKavling, KavlingName"
