...
Partial Class TrDPCustRetur
    Inherits System.Web.UI.Page
    Protected con As New SqlConnection
    Protected da As New SqlDataAdapter
    Protected GetStringHd As String = "Select distinct TransNmbr, Nmbr, TransDate, Status, FgReport, Customer_Code, Customer_Name, Customer, Attn, TotalPayment, TotalDP, TotalCharge, TotalDiffRate, Remark From V_FNDPCustReturHd "

    Private Function GetStringDt(ByVal Nmbr As String) As String
        Return "SELECT * From V_FNDPCustReturDt WHERE TransNmbr = " + QuotedStr(Nmbr)
    End Function

    Private Function GetStringDt2(ByVal Nmbr As String) As String
        Return "SELECT * From V_FNDPCustReturPay WHERE TransNmbr = " + QuotedStr(Nmbr)
    End Function
    ...
    Private Sub SetInit()
        Try
            ...
            lbTotPayment.Text = "Payment (" + ViewState("Currency") + ")"
    ...
    Private Sub BindData(Optional ByVal AdvanceFilter As String = "")
        ...
        Try
            ...
            GridView1.DataBind()
            If DT.Rows.Count > 0 Then
                GridView1.HeaderRow.Cells(8).Text = "Payment (" + ViewState("Currency") + ")"
    ...
    Protected Sub BtnGo_Click(ByVal sender As Object, ByVal e As EventArgs) Handles BtnGo.Click, btnGo2.Click
        ...
        Try
            ...
            If ActionValue = "Print" Then
                ...
                Next
                Result = Result + "'"
                'Session("SelectCommand") = "EXEC S_FNFormBuktiBank " + Result + ",'DPCUSTRETUR'"
                'Session("ReportFile") = ".../../../Rpt/FormBuktiBank.frx"
                Session("SelectCommand") = "EXEC S_FNFormVoucher " + Result + ",'DCR'," + QuotedStr(ViewState("UserId"))
            ...
            Else
                ...
                For j = 0 To (Nmbr.Length - 1)
                    If Nmbr(j) = "" Then
                        Exit For
                    Else
                        Result = ExecSPCommandGo(ActionValue, "S_FNDPCustRetur", Nmbr(j), CInt(Session(Request.QueryString("KeyId"))("Year")), CInt(Session(Request.QueryString("KeyId"))("Period")), ViewState("UserId").ToString, ViewState("DBConnection").ToString)
                        If Trim(Result) <> "" Then
    ...
    Private Sub ClearHd()
        Try
            ...
            tbAttn.Text = ""
            tbRemark.Text = ""
            FillCombo(ddlPayTypeDt2, "EXEC S_GetPayTypeUser " + QuotedStr("PaymentDP" + "Y") + ", " + QuotedStr(ViewState("UserId").ToString), True, "Payment_Code", "Payment_Name", ViewState("DBConnection")) 'ddlReport.SelectedValue
            FillCombo(ddlBankPaymentDt2, "EXEC S_GetBankPayment " + QuotedStr("Y"), True, "Bank_Code", "Bank_Name", ViewState("DBConnection")) 'ddlReport.SelectedValue
        Catch ex As Exception
    ...
    Private Sub Cleardt2()
        Try
            ...
            FillCombo(ddlPayTypeDt2, "EXEC S_GetPayTypeUser " + QuotedStr("PaymentDP" + "Y") + ", " + QuotedStr(ViewState("UserId").ToString), True, "Payment_Code", "Payment_Name", ViewState("DBConnection")) 'ddlReport.SelectedValue
            ChangePaymentType(ddlPayTypeDt2.SelectedValue, tbFgModeDt2, tbPaymentDateDt2, tbDueDateDt2, ddlBankPaymentDt2, ddlCurrDt2, ddlChargeCurrDt2, tbRateDt2, tbChargeRateDt2, tbChargeForexDt2, ViewState("Currency"), ViewState("DigitCurrDt"), ViewState("DBConnection"))
            ChangePaymentType(ddlPayTypeDt2.SelectedValue, tbFgModeDt2, tbPaymentDateDt2, tbGiroDateDt2, ddlBankPaymentDt2, ddlCurrDt2, ddlChargeCurrDt2, tbRateDt2, tbChargeRateDt2, tbChargeForexDt2, ViewState("Currency"), ViewState("DigitCurrDt"), ViewState("DBConnection"))
        Catch ex As Exception
    ...
    Private Sub SaveAll()
        ...
        Try
            ...
            If ViewState("StateSave") = "Insert" Then
                ...
                tbCode.Text = GetAutoNmbr("DCR", "Y", Year(tbDate.SelectedValue), Month(tbDate.SelectedValue), ViewState("PayType").ToString, ViewState("DBConnection").ToString)

                SQLString = "INSERT INTO FINDPCustReturHd (TransNmbr, TransDate, STATUS, FgReport, " + _
                "Customer, Attn, TotalPayment, TotalDP, TotalCharge, TotalDiffRate, Remark, UserPrep, DatePrep) " + _
                ...
                tbTotalPayment.Text.Replace(",", "") + ", " + tbTotalDP.Text.Replace(",", "") + ", " + _
            ...
            Else
                ...
                cekStatus = SQLExecuteScalar("Select Status FROM FINDPCustReturHd WHERE TransNmbr = " + QuotedStr(tbCode.Text), ViewState("DBConnection").ToString)
                If cekStatus = "P" Then
                    lbStatus.Text = MessageDlg("Save failed... Data has already posted by another user")
                    Exit Sub
                End If
                SQLString = "UPDATE FINDPCustReturHd SET Customer=" + QuotedStr(tbCustCode.Text) + _
                ", Attn =" + QuotedStr(tbAttn.Text) + _
                ", TotalPayment = " + tbTotalPayment.Text.Replace(",", "") + _
            ...
            con.Open()
            Dim cmdSql As New SqlCommand("SELECT TransNmbr, DPNo, DPDate, Currency, ForexRate, PPnRate, DPBase, DPPPn, DPTotal, DPPaidBase, DPPaidPPn, DPPaidTotal, BaseForex, PPnForex, TotalForex, TotalHome, DPRate, Remark FROM FINDPCustReturDt WHERE TransNmbr = " + QuotedStr(ViewState("TransNmbr")), con)
            ...
            Dim Dt As New DataTable("FINDPCustReturDt")
            ...
            'save dt2
            cmdSql = New SqlCommand("SELECT TransNmbr, ItemNo, PaymentType, PaymentDate, DocumentNo, Reference, GiroDate, DueDate, BankPayment, FgMode, Currency, ForexRate, PaymentForex, PaymentHome, Remark, ChargeCurrency, ChargeRate, ChargeForex, ChargeHome FROM FINDPCustReturPay WHERE TransNmbr = " + QuotedStr(ViewState("TransNmbr")), con)
            da = New SqlDataAdapter(cmdSql)
            ...
            Dim Dt2 As New DataTable("FINDPCustReturPay")
    ...
    Protected Sub LbAdvSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles LbAdvSearch.Click
        ...
        Try
            ...
            FilterName = "Payment No, Status, Date, Report, Customer, Attn, Payment, DP, Charge, Remark"
            FilterValue = "TransNmbr, Status, dbo.FormatDate(TransDate), FgReport, Customer_Name, Attn, TotalPayment, TotalDP, TotalCharge, Remark"
                ...
                ElseIf DDL.SelectedValue = "Print" Then
                    Try
                        ...
                        If CekMenu <> "" Then
                            ...
                            Exit Sub
                        End If
                        'Session("SelectCommand") = "EXEC S_FNFormBuktiBank '''" + GVR.Cells(2).Text + "''','DPCUSTRETUR'"
                        'Session("ReportFile") = ".../../../Rpt/FormBuktiBank.frx"
                        Session("SelectCommand") = "EXEC S_FNFormVoucher '''" + GVR.Cells(2).Text + "''','DCR'," + QuotedStr(ViewState("UserId"))
                        Session("ReportFile") = ".../../../Rpt/FormVoucher.frx"
                        Session("DBConnection") = ViewState("DBConnection")
                        AttachScript("openprintdlg();", Page, Me.GetType)
                    Catch ex As Exception
    ...
    Protected Sub GridDt_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridDt.RowDataBound
        ...
        Finally
            tbDiffRate.Text = FormatNumber(Convert.ToDecimal(tbTotalPayment.Text) - Convert.ToDecimal(tbTotalDP.Text) - Convert.ToDecimal(tbTotalCharge.Text) - Convert.ToDecimal(tbSelisihKurs.Text), ViewState("DigitHome"))
    ...
    Protected Sub GridDt2_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridDt2.RowDataBound
        Try
            If Not IsDBNull(DataBinder.Eval(e.Row.DataItem, "ItemNo")) Then
                If e.Row.RowType = DataControlRowType.DataRow Then
                    ' add the UnitPrice and QuantityTotal to the running total variables
                    TotalPayment += Convert.ToDecimal(DataBinder.Eval(e.Row.DataItem, "PaymentHome")) '* Convert.ToDecimal(DataBinder.Eval(e.Row.DataItem, "ForexRate"))
                    TotalCharge += Convert.ToDecimal(DataBinder.Eval(e.Row.DataItem, "ChargeHome")) '* Convert.ToDecimal(DataBinder.Eval(e.Row.DataItem, "ChargeRate"))
                ElseIf e.Row.RowType = DataControlRowType.Footer Then
    ...
    Protected Sub GridDt2_RowEditing(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewEditEventArgs) Handles GridDt2.RowEditing
        ...
        Try
            ...
            MovePanel(pnlDt2, pnlEditDt2)
            EnableHd(False)
            'ChangePaymentType(ddlPayTypeDt2.SelectedValue, tbFgModeDt2, tbPaymentDateDt2, tbDueDateDt2, ddlBankPaymentDt2, ddlCurrDt2, ddlChargeCurrDt2, tbRateDt2, tbChargeRateDt2, tbChargeForexDt2, ViewState("Currency"), ViewState("DigitCurrDt"), ViewState("DBConnection"), "Edit")
            'ChangePaymentType(ddlPayTypeDt2.SelectedValue, tbFgModeDt2, tbPaymentDateDt2, tbGiroDateDt2, ddlBankPaymentDt2, ddlCurrDt2, ddlChargeCurrDt2, tbRateDt2, tbChargeRateDt2, tbChargeForexDt2, ViewState("Currency"), ViewState("DigitCurrDt"), ViewState("DBConnection"), "Edit")
    ...
    Protected Sub ddlPayTypeDt2_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlPayTypeDt2.SelectedIndexChanged
        ...
        Try
            ...
            VoucherNo = ""
            'tbPaymentForexDt2.Text = FormatNumber(tbPaymentForexDt2.Text, ViewState("DigitExpenseCurr"))
    ...
    Protected Sub btnDPNo_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnDPNo.Click
        ...
        Try
            ...
            'Session("filter") = "SELECT DP_No, DP_Date, PPn_Rate, Currency, Rate, Base_Forex, PPn_Forex, Total_Forex, Base_Paid, PPn_Paid, Total_Paid FROM V_FNDPCustPending WHERE Customer_Code = " + QuotedStr(tbCustCode.Text)
            'Else
            Session("filter") = "SELECT DP_No, DP_Date, PPn_Rate, Currency, Rate, Base_Forex, PPn_Forex, Total_Forex, Base_Paid, PPn_Paid, Total_Paid FROM V_FNDPCustPending WHERE Report = " + QuotedStr("Y") + " and Customer_Code = " + QuotedStr(tbCustCode.Text) 'ddlReport.SelectedValue
            'End If

            ResultField = "DP_No, DP_Date, PPn_Rate, Currency, Rate, Base_Forex, PPn_Forex, Total_Forex, Base_Paid, PPn_Paid, Total_Paid"
    ...
    Protected Sub tbCustCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbCustCode.TextChanged
        Dim Dr As DataRow
        Try
            Dr = FindMaster("Customer", tbCustCode.Text, ViewState("DBConnection"))
            If Not Dr Is Nothing Then
            ...
            End If
        Catch ex As Exception
            lbStatus.Text = "tb Customer Code Text Changed Error : " + ex.ToString
        End Try
    End Sub
    ...
    '    FillCombo(ddlPayTypeDt2, "EXEC S_GetPayTypeUser " + QuotedStr("PaymentDP" + ddlReport.SelectedValue) + ", " + QuotedStr(ViewState("UserId").ToString), True, "Payment_Code", "Payment_Name", ViewState("DBConnection"))
