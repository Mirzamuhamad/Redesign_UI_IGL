...
Partial Class Transaction_TrSickness_TrSickness
    Inherits System.Web.UI.Page

    Protected con As New SqlConnection
    Protected da As New SqlDataAdapter
    Protected GetStringHd As String = "Select * From V_PlSicknessHd"
    ...
    Private Sub SetInit()
        FillRange(ddlRange)
        FillCombo(ddlWeek, "EXEC S_GetWeek", True, "Week_No", "Week", ViewState("DBConnection"))
        FillCombo(ddlCheckBy, "EXEC S_GetTeamNew", True, "Team_Code", "DivisionName", ViewState("DBConnection"))
    ...
    Private Function GetStringDt(ByVal Nmbr As String) As String
        Return "SELECT TransNmbr, Block, BlockName, Gawang, Remark From V_PlSicknessDt WHERE TransNmbr = " + QuotedStr(Nmbr)
    End Function
            ...
            Else
                ...
                For j = 0 To (Nmbr.Length - 1)
                    If Nmbr(j) = "" Then
                        Exit For
                    Else
                        Result = ExecSPCommandGo(ActionValue, "S_PLSickness", Nmbr(j), CInt(Session(Request.QueryString("KeyId"))("Year")), CInt(Session(Request.QueryString("KeyId"))("Period")), ViewState("UserId").ToString, ViewState("DBConnection").ToString)
                        If Trim(Result) <> "" Then
    ...
    Private Sub SaveAll()
        ...
        Try
            ...
            If ViewState("StateSave") = "Insert" Then
                tbRef.Text = GetAutoNmbr("SCR", "Y", Year(tbDate.SelectedDate), Month(tbDate.SelectedDate), "", ViewState("DBConnection").ToString)
                'lbStatus.Text = tbRef.Text
                'Exit Sub

                SQLString = "INSERT INTO PlSicknessHd (TransNmbr, TransDate, Status, StartSickDate, Year, Week, CheckBy, Remark, UserPrep, DatePrep) " + _
                "SELECT " + QuotedStr(tbRef.Text) + ", '" + Format(tbDate.SelectedDate, "yyyy-MM-dd") + "',  'H', '" + _
                Format(tbStarSickDate.SelectedDate, "yyyy-MM-dd") + "' ," + QuotedStr(tbYear.Text) + ", " + _
                QuotedStr(ddlWeek.SelectedValue) + " , " + _
                QuotedStr(ddlCheckBy.SelectedValue) + ", " + QuotedStr(tbRemark.Text) + "," + _
                QuotedStr(ViewState("UserId").ToString) + ", getDate()"
            Else
                Dim cekStatus As String
                cekStatus = SQLExecuteScalar("Select Status FROM PlsicknessHd WHERE TransNmbr = " + QuotedStr(tbRef.Text), ViewState("DBConnection").ToString)
                If cekStatus = "P" Then
                    lbStatus.Text = MessageDlg("Save failed... Data has already posted by another user")
                    Exit Sub
                End If
                SQLString = "UPDATE PlsicknessHd SET StartSickDate = " + QuotedStr(Format(tbStarSickDate.SelectedDate, "yyyy-MM-dd")) + _
                ", Year = " + QuotedStr(tbYear.Text) + ", Week = " + QuotedStr(ddlWeek.SelectedValue) + _
                ", CheckBy = " + QuotedStr(ddlCheckBy.SelectedValue) + " , Remark = " + QuotedStr(tbRemark.Text) + _
            ...
            con.Open()
            Dim cmdSql As New SqlCommand("SELECT TransNmbr, Block, Gawang, Remark FROM PlsicknessDt WHERE TransNmbr = '" & ViewState("TransNmbr") & "'", con)
            ...
            Dim Update_Command = New SqlCommand( _
                    "UPDATE PlSicknessDt SET Block = @Block, Gawang= @Gawang, Remark = @Remark " + _
            ...
            Update_Command.Parameters.Add("@Gawang", SqlDbType.Float, 18, "Gawang")
            ...
            Dim Delete_Command = New SqlCommand( _
                "DELETE FROM PlSicknessDt WHERE TransNmbr = '" & ViewState("TransNmbr") & "' AND Block = @Block", con)
            ' Add the parameters for the DeleteCommand.
            param = Delete_Command.Parameters.Add("@Block", SqlDbType.VarChar, 5, "Block")
            param.SourceVersion = DataRowVersion.Original
            da.DeleteCommand = Delete_Command

            Dim Dt As New DataTable("PlSicknessDt")
    ...
    Private Sub ClearHd()
        ...
        Try
            tbRef.Text = ""
            tbStarSickDate.SelectedDate = ViewState("ServerDate") 'Today
            tbYear.Text = "2019"
            ddlWeek.Text = ""
            ddlCheckBy.Text = ""
    ...
    Function CekHd() As Boolean
        Try
            ...
            If ddlWeek.SelectedValue = "" Then
                lbStatus.Text = MessageDlg("Week must have value")
                ddlWeek.Focus()
                Return False
            End If
            If ddlCheckBy.SelectedValue = "" Then
    ...
    Function CekDt(Optional ByVal Dr As DataRow = Nothing, Optional ByVal FieldKey As String = "") As Boolean
        Try
            If Not (Dr Is Nothing) Then
                ...
                If Dr("Gawang").ToString.Trim = "" Then
    ...
    Protected Sub LbAdvSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles LbAdvSearch.Click
        ...
        Try
            ...
            FilterName = "TransNmbr, Date, StartSickDate, Remark"
            FilterValue = "TransNmbr, dbo.FormatDate(TransDate), dbo.FormatDate(StartSickDate), Remark"
    ...
    Protected Sub FillTextBoxHd(ByVal Nmbr As String)
        ...
        Try
            ...
            BindToDate(tbStarSickDate, Dt.Rows(0)("StartSickDate").ToString)
            BindToText(tbYear, Dt.Rows(0)("Year").ToString)
            BindToDropList(ddlWeek, Dt.Rows(0)("Week").ToString)
            BindToDropList(ddlCheckBy, Dt.Rows(0)("CheckBy").ToString)
    ...
    Protected Sub FillTextBoxDt(ByVal Product As String)
        ...
        Try
            ...
            If Dr.Length > 0 Then
                ...
                BindToText(tbGawang, Dr(0)("Gawang").ToString)
