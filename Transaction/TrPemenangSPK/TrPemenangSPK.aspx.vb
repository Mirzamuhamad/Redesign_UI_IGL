...
Partial Class PenunjukanPemenang
    ...
    Private Function GetStringDt(ByVal Nmbr As String) As String
        Return "SELECT * From V_PRCPemenangDt WHERE TransNmbr = " + QuotedStr(Nmbr)
    End Function
            ...
            Else
                ...
                For j = 0 To (Nmbr.Length - 1)
                    If Nmbr(j) = "" Then
                        Exit For
                    Else
                        Result = ExecSPCommandGo(ActionValue, "S_PRCPemenang", Nmbr(j), CInt(Session(Request.QueryString("KeyId"))("Year")), CInt(Session(Request.QueryString("KeyId"))("Period")), ViewState("UserId").ToString, ViewState("DBConnection").ToString)
                        If Trim(Result) <> "" Then
    ...
    Private Sub SaveAll()
        ...
        Try
            ...
            If ViewState("StateSave") = "Insert" Then
                ...
                tbRef.Text = GetAutoNmbr("SPK", "Y", Year(tbDate.SelectedValue), Month(tbDate.SelectedValue), "", ViewState("DBConnection").ToString)


                SQLString = "INSERT INTO PRCPemenangHd (TransNmbr,Status, TransDate,No_Penawaran,No_Intruksi,Paket_Pekerjaan,SuppCode,Lokasi, " + _
                "StartDate,EndDate,Durasi,TotalBiaya,Ppn, PpnValue, Pph, PphValue,TotalAmount,TugasCode,TypeSPK, " + _
                ...
                QuotedStr(tbTotalAmount.Text.Replace(",", "")) + ", " + QuotedStr(tbTugasCode.Text) + ",  " + QuotedStr(tbSPK.Text) + " ," + _
                QuotedStr(tbRemark.Text) + ", " + QuotedStr(ViewState("UserId").ToString) + ", getDate()"
            Else
                Dim cekStatus As String
                cekStatus = SQLExecuteScalar("Select Status FROM PRCPemenangHd WHERE TransNmbr = " + QuotedStr(tbRef.Text), ViewState("DBConnection").ToString)
                If cekStatus = "P" Then
                    lbStatus.Text = MessageDlg("Save failed... Data has already posted by another user")
                    Exit Sub
                End If

                SQLString = "UPDATE PRCPemenangHd SET SuppCode = " + QuotedStr(tbSuppCode.Text) + ", No_Penawaran = " + QuotedStr(tbPenawaranNo.Text) + _
            ...
            con.Open()
            Dim cmdSql As New SqlCommand("SELECT TransNmbr, ItemNo, UraianPekerjaan,UnitCode, Luas,BiayaSatuan, Biaya,  Remark FROM PRCPemenangDt WHERE TransNmbr = '" & ViewState("TransNmbr") & "'", con)
            ...
            '"UPDATE PRCPemenangDt SET ItemNo = @ItemNo, InvoiceNo = @InvoiceNo, " + _
            ...
            '    "DELETE FROM PRCPemenangDt WHERE TransNmbr = '" & ViewState("TransNmbr") & "' AND ItemNo = @ItemNo ", con)
            '' Add the parameters for the DeleteCommand.
            'param = Delete_Command.Parameters.Add("@ItemNo", SqlDbType.Int, 4, "ItemNo")
            'param.SourceVersion = DataRowVersion.Original
            'da.DeleteCommand = Delete_Command

            Dim Dt As New DataTable("PRCPemenangDt")
    ...
    Private Sub CountTotalDt()
        ...
        Try
            ...
            For Each Dr In ViewState("Dt").Rows
                ...
                End If
            Next
            tbTotalBiaya.Text = FormatNumber(TotalBiaya, ViewState("DigitHome"))
            ...
            If e.CommandName = "Go" Then
                ...
                ElseIf DDL.SelectedValue = "Print" Then
                    Try
                        ...
                        Form2 = "2"
                        Form3 = "3"
                        Session("SelectCommand") = "EXEC S_FNFormSPK '''" + GVR.Cells(2).Text + "''', " + QuotedStr(ViewState("UserId"))
                        Session("SelectCommand2") = "EXEC S_FNFormSPK '''" + GVR.Cells(2).Text + "''', " + QuotedStr(ViewState("UserId"))
                        Session("SelectCommand3") = "EXEC S_FNFormSPK '''" + GVR.Cells(2).Text + "''', " + QuotedStr(ViewState("UserId"))
                        Session("ReportFile") = ".../../../Rpt/FormSPK.frx"
                        Session("DBConnection") = ViewState("DBConnection")
