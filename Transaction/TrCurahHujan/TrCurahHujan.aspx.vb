...
Partial Class Transaction_TrCurahHujan
    Inherits System.Web.UI.Page

    Protected con As New SqlConnection
    Protected da As New SqlDataAdapter
    Protected GetStringHd As String = "SELECT * FROM V_PLSensusCurahHujan"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            If Not IsPostBack Then
                ...
                tbVolume.Attributes.Add("OnKeyDown", "return PressNumeric();")
            ...
            If Not Session("Result") Is Nothing Then
                If ViewState("Sender") = "btnKoordinat" Then
                    tbKoordinat.Text = Session("Result")(0).ToString
                    BindToText(tbLokasi, Session("Result")(1).ToString)
                End If
    ...
    Private Function GetStringDt(ByVal Nmbr As String) As String
        Return "SELECT TransNmbr, JobPlant, JobName, StatusTanam ,StartDate, EndDate,  Team, TeamName, Qty, Unit,WorkDay, Capacity, Person, DoneClosing, DoneWO, RemarkClose, UserClose, DateClose, RKYear,RKMonth From V_PLPlanBatchDt WHERE TransNmbr = " + QuotedStr(Nmbr)
    End Function
    ...
    Protected Sub BtnGo_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnGo.Click, btnGo2.Click
        ...
        Try
            ...
            If ActionValue = "Print" Then
                ...
                Session("SelectCommand") = "EXEC S_PLFormSensusCurahHujan " + Result + " , " + QuotedStr(ViewState("UserId").ToString)
                lbStatus.Text = Session("SelectCommand")
                Exit Sub
                Session("ReportFile") = ".../../../Rpt/FormSensusCurahHujan.frx"
            ...
            Else
                ...
                For j = 0 To (Nmbr.Length - 1)
                    If Nmbr(j) = "" Then
                        Exit For
                    Else
                        Result = ExecSPCommandGo(ActionValue, "S_PLSensusCurahHujan", Nmbr(j), CInt(Session(Request.QueryString("KeyId"))("Year")), CInt(Session(Request.QueryString("KeyId"))("Period")), ViewState("UserId").ToString, ViewState("DBConnection").ToString)
                        If Trim(Result) <> "" Then
    ...
    Private Sub SaveAll()
        ...
        Try
            ...
            If ViewState("StateSave") = "Insert" Then
                tbRef.Text = GetAutoNmbr("SCJ", "Y", Year(tbDate.SelectedValue), Month(tbDate.SelectedValue), "", ViewState("DBConnection").ToString)
                'lbStatus.Text = tbRef.Text
                'Exit Sub

                SQLString = "INSERT INTO PLSensusCurahHujan (TransNmbr, Transdate, Status, Koordinat, LokasiName, Volume, PIC, Remark, UserPrep, DatePrep, UserAppr, DateAppr) " + _
                "SELECT " + QuotedStr(tbRef.Text) + ", " + QuotedStr(Format(tbDate.SelectedDate, "yyyy-MM-dd")) + ", 'H'," + QuotedStr(tbKoordinat.Text) + ", " + QuotedStr(tbLokasi.Text) + ", " + _
                QuotedStr(tbVolume.Text) + "," + QuotedStr(tbPIC.Text) + "," + QuotedStr(tbRemark.Text) + "," + _
                QuotedStr(ViewState("UserId").ToString) + ", getDate(), " + QuotedStr(ViewState("UserId").ToString) + ", getDate()"
            Else
                Dim cekStatus As String
                cekStatus = SQLExecuteScalar("Select Status FROM PLSensusCurahHujan WHERE TransNmbr = " + QuotedStr(tbRef.Text), ViewState("DBConnection").ToString)
                If cekStatus = "P" Then
                    lbStatus.Text = MessageDlg("Save failed... Data has already posted by another user")
                    Exit Sub
                End If
                SQLString = "UPDATE PLSensusCurahHujan SET Koordinat = " + QuotedStr(tbKoordinat.Text) + _
                ", LokasiName = " + QuotedStr(tbLokasi.Text) + ",Volume = " + QuotedStr(tbVolume.Text) + _
                 ", PIC = " + QuotedStr(tbPIC.Text) + ", Remark = " + QuotedStr(tbRemark.Text) + _
    ...
    Function CekHd() As Boolean
        Try
            ...
            '    Return False
            'End If
            If tbKoordinat.Text = "" Then
                lbStatus.Text = MessageDlg("Koordinat No must have value")
                tbKoordinat.Focus()
                Return False
            End If
            ...
            If e.CommandName = "Go" Then
                ...
                ElseIf DDL.SelectedValue = "Print" Then
                    Try
                        ...
                        End If
                        Session("DBConnection") = ViewState("DBConnection")
                        Session("SelectCommand") = "EXEC S_PLFormSensusCurahHujan " + QuotedStr(GVR.Cells(2).Text) + "," + QuotedStr(ViewState("UserId").ToString)
                        Session("ReportFile") = ".../../../Rpt/FormSensusCurahHujan.frx"
                        AttachScript("openprintdlg();", Page, Me.GetType)
                    Catch ex As Exception
    ...
    Protected Sub FillTextBoxHd(ByVal Nmbr As String)
        ...
        Try
            ...
            'newTrans()
            tbRef.Text = Nmbr
            BindToText(tbKoordinat, Dt.Rows(0)("Koordinat").ToString)
            BindToDate(tbDate, Dt.Rows(0)("TransDate").ToString)
            BindToText(tbLokasi, Dt.Rows(0)("LokasiName").ToString)
    ...
    Protected Sub btnKoordinat_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnKoordinat.Click
        Dim ResultField, CriteriaField As String
        Try

            Session("filter") = "SELECT Koordinat, Lokasi From VMsKoordinat " '" + QuotedStr(ViewState("UserId").ToString) + ", " + QuotedStr(ddlDept.SelectedValue)
            CriteriaField = "Koordinat, Lokasi"
            ResultField = "Koordinat, Lokasi"

            ViewState("Sender") = "btnKoordinat"
