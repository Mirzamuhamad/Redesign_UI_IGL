...
Partial Class TrPETerminateOut
    ...
    Private Function GetStringHd() As String
        Return "Select * From V_PETerminateOutHd"
    End Function
    ...
    Private Function GetStringDt(ByVal Nmbr As String) As String
        Return "SELECT * From V_PETerminateOutdt WHERE TransNmbr = " + QuotedStr(Nmbr)
    End Function
            ...
            Else
                ...
                For j = 0 To (Nmbr.Length - 1)
                    ...
                    ElseIf ActionValue = "Post" Then
                        ...
                        Result = ExecSPCommandGo(ActionValue, "S_PEEmpTerminate", Nmbr(j), CInt(ViewState("GLYear")), CInt(ViewState("GLPeriod")), ViewState("UserId").ToString, ViewState("DBConnection").ToString)
                        If Trim(Result) <> "" Then
    ...
    Protected Sub btnSaveDt_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnSaveDt.Click
        ...
        Try
            If ViewState("StateDt") = "Edit" Then
                ...
                Row("Reason") = ddlReason.SelectedValue
                Row("PHK_Name") = ddlReason.Text
                Row("DonePaid") = tbPaid.Text
            ...
            Else
                ...
                dr("PHK_Name") = ddlReason.Text
                dr("DonePaid") = tbPaid.Text
            ...
            StatusButtonSave(True)
            ddlEmpStatus.Enabled = GetCountRecord(ViewState("Dt")) = 0
        Catch ex As Exception
    ...
    Private Sub SaveAll()
        ...
        Try
            ...
            If ViewState("StateSave") = "Insert" Then
                tbTransNo.Text = GetAutoNmbr("EO", "N", Year(tbDate.SelectedValue), Month(tbDate.SelectedValue), "", ViewState("DBConnection").ToString)
                SQLString = "INSERT INTO PEEmpTerminateHd(TransNmbr, STATUS, Transdate, EffectiveDate, EmpStatus, PHKReason,EmpAppr,JobTitleAppr, Remark, UserPrep, DatePrep) " + _
                "SELECT '" + tbTransNo.Text + "', 'H', '" + Format(tbDate.SelectedValue, "yyyy-MM-dd") + "', '" + Format(tbEffectiveDate.SelectedValue, "yyyy-MM-dd") + "'" + _
                "," + QuotedStr(ddlEmpStatus.SelectedValue) + "," + QuotedStr(ddlReason.SelectedValue) + "," + QuotedStr(tbApproved.Text) + "," + QuotedStr(tbJobTitle.Text) + _
            ...
            Else
                ...
                If cekStatus = "P" Then
                    ...
                    Exit Sub
                End If
                SQLString = "UPDATE PEEmpTerminateHd SET Transdate = '" + Format(tbDate.SelectedValue, "yyyy-MM-dd") + _
                "',EffectiveDate = '" + Format(tbEffectiveDate.SelectedValue, "yyyy-MM-dd") + _
                "',EmpStatus = " + QuotedStr(ddlEmpStatus.SelectedValue) + _
                ",PHKReason = " + QuotedStr(ddlReason.SelectedValue) + _
            ...
            con.Open()
            Dim cmdSql As New SqlCommand(" SELECT TransNmbr, EmpNumb, JobTitle, Reason,DonePaid, " + _
                                         " Remark FROM PEEmpTerminateDt WHERE TransNmbr = '" & ViewState("Reference") & "'", con)
            ...
            Dim Dt As New DataTable("{PEEmpTerminateDt")
    ...
    Private Sub Cleardt()
        Try
            ...
            ddlEmpReason.SelectedIndex = 0
            ddlEmpReason.Enabled = True
            tbPaid.Text = "N"
            tbRemarkDt.Text = ""
        Catch ex As Exception
    ...
    Function CekDt(Optional ByVal Dr As DataRow = Nothing) As Boolean
        Try
            If Not (Dr Is Nothing) Then
                ...
                If Dr("Reason").ToString.Trim = "" Then
                    lbStatus.Text = "Reason Must Have Value"
                    Return False
                End If
            ...
            Else
                ...
                If ddlEmpReason.Text.Trim = "" Then
                    lbStatus.Text = "Reason Must Have Value"
                    Return False
    ...
    Protected Sub LbAdvSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles LbAdvSearch.Click
        ...
        Try
            ...
            FilterName = "Reference, Date, Effective Date, Emp. Status Name,Emp. Approval Name,Job Title, Remark"
            FilterValue = "TransNmbr, dbo.FormatDate(TransDate), EffectiveDate,EmpStatus_Name,EmpAppr_Name, JobTtlAppr_Name, Remark"
    ...
    Protected Sub FillTextBoxHd(ByVal Nmbr As String)
        ...
        Try
            ...
            BindToDropList(ddlReason, Dt.Rows(0)("PHKReason").ToString)
    ...
    Protected Sub FillTextBoxDt(ByVal KeyDt As String)
        ...
        Try
            ...
            If Dr.Length > 0 Then
                ...
                BindToDropList(ddlEmpReason, Dr(0)("Reason").ToString)
                BindToText(tbPaid, Dr(0)("DonePaid").ToString)
    ...
    Protected Sub tbApproved_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbApproved.TextChanged
        ...
        Try
            ...
            SQLString = " SELECT Emp_No, Emp_Name, Job_Level, Job_Level_Name, Job_Title, Job_Title_Name, Emp_Status, Gender " + _
    ...
    Protected Sub btnEmp_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnEmp.Click
        Dim ResultField As String
        Try
            Session("Filter") = " SELECT Emp_No, Emp_Name, Job_Title, Job_Title_Name, Emp_Status, Emp_Status_Name, Gender " + _
                                " FROM V_MsEmployee Where Fg_Active = 'Y' And End_Date Is NULL AND Emp_Status = " + QuotedStr(ddlEmpStatus.SelectedValue)
            ResultField = "Emp_No,Emp_Name,Job_Title,Job_Title_Name,Emp_Status,Gender"
    ...
    Protected Sub tbEmpCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbEmpCode.TextChanged
        ...
        Try
            ...
            SQLString = " SELECT Emp_No, Emp_Name, Job_Title, Job_Title_Name, Emp_Status, Gender " + _
                        " FROM V_MsEmployee Where Fg_Active = 'Y' And End_Date Is NULL AND Emp_Status = " + QuotedStr(ddlEmpStatus.SelectedValue) + " AND Emp_No = " + QuotedStr(tbEmpCode.Text)
    ...
    Protected Sub ddlReason_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlReason.SelectedIndexChanged
        Try
            ...
            Try
                ...
                '    RefreshMaster("S_GetPHKReason", "PHKCode", "PHKName", ddlReason, ViewState("DBConnection"))
                '    ViewState("InputCurrency") = Nothing
                'End If


            Catch ex As Exception
                lbStatus.Text = "ddl Reason selected index Changed : " + ex.ToString
            End Try
    ...
    Protected Sub ddlEmpReason_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlEmpReason.SelectedIndexChanged
        Try

            Try
                'If ViewState("InputCurrency") = "Y" Then
                '    RefreshMaster("S_GetPHKReason", "PHKCode", "PHKName", ddlEmpReason, ViewState("DBConnection"))
                '    ViewState("InputCurrency") = Nothing
                'End If
    ...
    Protected Sub ddlEmpStatus_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlEmpStatus.SelectedIndexChanged
        Try
            ...
            Try
                'If ViewState("InputCurrency") = "Y" Then
                '    RefreshMaster("S_GetEmpStatus", "EmpStatusCode", "EmpStatusName", ddlEmpStatus, ViewState("DBConnection"))
                '    ViewState("InputCurrency") = Nothing
                'End If

                tbEmpCode_TextChanged(Nothing, Nothing)
    ...
    Protected Sub lbEmpStatus_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lbEmpStatus.Click
        ...
        End Try
    End Sub
    Protected Sub lbReason_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lbReason.Click
        Try
            AttachScript("OpenMaster('" + Request.QueryString("KeyId") + "','MsTerminateReason')();", Page, Me.GetType())
        Catch ex As Exception
