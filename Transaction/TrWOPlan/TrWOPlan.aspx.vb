...
Partial Class Transaction_TrWOPLan_TrWOPLan
    ...
    Protected GetStringHd As String = "SELECT DISTINCT TransNmbr, TransDate, Status, Divisi, DivisiName, WorkBy, WorkByName, ReffType, Reference, MonthNo, JobPlant, JobPlantName, Person, QtyBlok, Qty, Unit, UserPrep, DatePrep, UserAppr, DateAppr, " + _
                " Supplier, SupplierName, CIP, CIPName, EstStartWeek, EstEndWeek, QtyWeek, Remark FROM V_PLWoPlanHd "
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            If Not IsPostBack Then
                ...
                'lbCount.Text = SQLExecuteScalar("SELECT COUNT(TransNmbr) FROM V_PLWOPlanGetReff WHERE REFF_Type = 'TBM'", ViewState("DBConnection").ToString)
                'lbCount.Text = SQLExecuteScalar("SELECT COUNT(Reference) FROM V_PLRKBulanan WHERE RKMonth is NOT NULL AND RKYear is NOT NULL", ViewState("DBConnection").ToString)
                lbCount.Text = SQLExecuteScalar("SELECT COUNT(TransNmbr) FROM V_PLWOPlanGetReff WHERE RK_Month is NOT NULL AND RK_Year is NOT NULL", ViewState("DBConnection").ToString)
            ...
            If Not Session("Result") Is Nothing Then
                If ViewState("Sender") = "btnReff" Then
                    ...
                    BindToText(tbPerson, Session("Result")(5).ToString)
                    ...
                    For Each drResult In Session("Result").Rows
                        ...
                        If ExistRow.Count = 0 Then
                            ...
                            If ddlReffType.SelectedValue = "Schedule" Then
                                ...
                                dr("QtyTotal") = (CFloat(drResult("Qty").ToString))
                                dr("NormaHK") = drResult("Capacity").ToString
                            ...
                            Else
                                ...
                                Dim SQLString As String
                                SQLString = "EXEC S_PLWOPlanFindDivBlock " + QuotedStr(ddlReffType.SelectedValue) + " , " + QuotedStr(tbReffNo.Text) + "," + QuotedStr(tbJob.Text) + "," + QuotedStr(drResult("Type").ToString) + ", " + QuotedStr(drResult("DivisiBlokCode").ToString) + ", " + QuotedStr(ddlDivisi.SelectedValue) + "," + tbRotasi.Text
                                'lbStatus.Text = SQLString
                                'Exit Sub
                                dtUnit = SQLExecuteQuery(SQLString, ViewState("DBConnection")).Tables(0)
                                drUnit = dtUnit.Rows(0)

                                dr("Qty") = drUnit("MaxCap").ToString
                                dr("Pokok") = FormatFloat(drUnit("Pkk").ToString, ViewState("DigitQty"))
                                dr("QtyTotal") = (CFloat(drUnit("MaxCap").ToString))
                                dr("NormaHK") = drUnit("NormaHK").ToString
                                If CFloat(drResult("Qty").ToString) = 0 Then
                                    ViewState("SPH") = 0
                                Else
                                    ViewState("SPH") = CFloat(drUnit("Pkk").ToString) / CFloat(drResult("Qty").ToString)
                                End If
                                If (dr("NormaHK") > 0) And (CInt(tbPerson.Text) > 0) Then
                                    dr("WorkDay") = (CFloat(drUnit("MaxCap")) / CFloat(dr("NormaHK")) / CInt(tbPerson.Text))
                                    '   GetEndDate()
                                    dr("EndDate") = Date.FromOADate(dr("StartDate").ToOADate() + CInt(dr("WorkDay")))
                                Else
                                    dr("WorkDay") = 0
                                ...
                                Else : dr("TargetHK") = 0
                                    ...
                                    GetMachine(tbJob.Text, tbReffNo.Text, ddlDivisi.SelectedValue, ddlReffType.SelectedValue)
                                    'GetMaterial(ddlReffType.SelectedValue, tbReffNo.Text, tbJob.Text, drResult("DivisiBlokCode").ToString, tbRotasi.Text, drResult("Type").ToString, drResult("PlantPeriodCode").ToString, dr("Qty").ToString, dr("Pokok").ToString, ddlDivisi.SelectedValue, ddlReffType.SelectedValue)
                ...
                If ViewState("Sender") = "btnOut" Then
                    ...
                    For Each drResult In Session("Result").Rows
                        ...
                        'insert
                        ExistRow = ViewState("Dt").Select("DivisiBlok = " + QuotedStr(Divisi) + " AND Type = " + QuotedStr(Type) + " AND Type = " + QuotedStr(Type))
                        If ExistRow.Count = 0 Then
                            If FirstTime Then
    ...
    Private Sub SetInit()
        Try
            ...
            FillCombo(ddlDivisi, "SELECT Division_Code,Division_name FROM  V_MsDivision", True, "Division_Code", "Division_name", ViewState("DBConnection"))
            ...
            tbPercentage.Attributes.Add("OnKeyDown", "return PressNumeric()")
            ...
            tbQtyDosis.Attributes.Add("OnKeyDown", "return PressNumeric()")
            tbQtyPokok.Attributes.Add("OnKeyDown", "return PressNumeric()")
    ...
    Private Sub BindDataDt2(ByVal Nmbr As String)
        ...
        Try
            ...
            ViewState("Dt2") = dt
            Drow = ViewState("Dt2").Select("Type+'|'+DivisiBlok=" + QuotedStr(LblTypeM.Text + "|" + LblBlockM.Text))
    ...
    Function CekHd() As Boolean
        Try
            ...
            If ddlDivisi.SelectedValue = 0 Then
                lbStatus.Text = MessageDlg("Divisi must have value")
                ddlDivisi.Focus()
                Return False
            End If

            If tbRotasi.Text = "" Or tbRotasi.Text = 0 Then
                lbStatus.Text = MessageDlg("Rotasi must have value")
                tbRotasi.Focus()
                Return False
            End If
        ...
        Try
            If Not Dr Is Nothing Then
                ...
                If TrimStr(Dr("DivisiBlok").ToString) = "" Then
                    lbStatus.Text = MessageDlg("Divisi / Block Must Have Value")
                    Return False
                End If
    ...
    Protected Sub FillTextBoxHd(ByVal Nmbr As String)
        ...
        Try
            ...
            BindToText(tbQtyBlock, Dt.Rows(0)("QtyBlok").ToString)
            BindToText(tbQty, CFloat(Dt.Rows(0)("Qty").ToString))
    ...
    Protected Sub btnSaveDt_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnSaveDt.Click
        Try
            ...
            If ViewState("StateDt") = "Edit" Then
                ...
                Row("QtyTotal") = tbTotal.Text
                Row("Qty") = tbQtyWO.Text
                Row("NormaHK") = tbQtyCapacity.Text
                Row("TargetHK") = tbQtyTarget.Text
                Row("WorkDay") = tbQtyWorkDay.Text
                Row("StartDate") = tbStartDate.SelectedDate
                Row("EndDate") = tbEndDate.SelectedDate
                Row.EndEdit()
            Else
                If CekExistData(ViewState("Dt"), "Type,DivisiBlok", ddlType.Text + "|" + TrimStr(tbBlock.Text)) = True Then
                    lbStatus.Text = "Type " + ddlType.SelectedValue + " Block " + tbBlock.Text + " has been already exist"
                    Exit Sub
                End If
                'Insert
                Dim dr As DataRow
    ...
    Private Sub SaveAll()
        ...
        Try
            ...
            If ViewState("StateSave") = "Insert" Then
                ...
                "SELECT " + QuotedStr(tbCode.Text) + ", " + QuotedStr(Format(tbDate.SelectedValue, "yyyy-MM-dd")) + ", 'H', " + QuotedStr(tbFgBorongan.Text) + ", 'N'," + QuotedStr(ddlDivisi.SelectedValue) + "," + _
                QuotedStr(ddlWorkBy.SelectedValue) + "," + QuotedStr(ddlReffType.SelectedValue) + "," + QuotedStr(tbReffNo.Text) + "," + _
                QuotedStr(tbRotasi.Text) + "," + QuotedStr(tbJob.Text) + "," + tbQtyBlock.Text.Replace(", ", "") + "," + _
            ...
            Else
                ...
                ", WorkBy = " + QuotedStr(ddlWorkBy.SelectedValue) + ", Reference = " + QuotedStr(tbReffNo.Text) + _
                ", CIP = " + QuotedStr(tbCIP.Text) + ",FgBorongan = " + QuotedStr(tbFgBorongan.Text) + ", Supplier = " + QuotedStr(tbKontraktor.Text) + _
                ", Person = " + QuotedStr(tbPerson.Text) + ", ReffType = " + QuotedStr(ddlReffType.SelectedValue) + _
                ", MonthNo = " + QuotedStr(tbRotasi.Text) + ", JobPlant = " + QuotedStr(tbJob.Text) + ", QtyBlok=" + tbQtyBlock.Text.Replace(", ", "") + _
                ...
                ElseIf DDL.SelectedValue = "Print" Then
                    Try
                        ...
                        If CekMenu <> "" Then
                            ...
                            Exit Sub
                        End If
                        'lbStatus.Text = GVR.Cells(2).Text
                        'Exit Sub
    ...
    Protected Sub GridDt_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GridDt.RowCommand
        Try
            ...
            ElseIf e.CommandName = "ViewA" Then
                ...
                LblBlockM.Text = GVR.Cells(3).Text
                LblBlockNameM.Text = GVR.Cells(4).Text
                Dim drow As DataRow()
                If ViewState("Dt2") Is Nothing Then
                    BindDataDt2(ViewState("TransNmbr"))
                Else
            ...
            ElseIf e.CommandName = "ViewE" Then
                ...
                LblBlockE.Text = GVR.Cells(3).Text
                LblBlockNameE.Text = GVR.Cells(4).Text
                Dim drow As DataRow()
            ...
            ElseIf e.CommandName = "ViewM" Then
                ...
                LblBlockMT.Text = GVR.Cells(3).Text
                LblBlockNameMT.Text = GVR.Cells(4).Text
                Dim drow As DataRow()
    ...
    Protected Sub btnSaveDt4_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSaveDt4.Click
        ...
        Try
            ...
            If ViewState("StateDt4") = "Edit" Then
                Dim Row As DataRow
                ' Row = ViewState("Dt4").Select("Material = " + QuotedStr("Dt4Value"))(0)
            ...
            Else
                ...
                'Drow = ViewState("Dt4").Select("Type+'|'+DivisiBlok = " + QuotedStr(LblTypeMT.Text + "|" + LblBlockMT.Text))
                'If drow.Length > 0 Then
                If CekExistData(ViewState("Dt4"), "Material", tbMaterial.Text) = True Then
                    lbStatus.Text = MessageDlg("Material " + tbMaterialName.Text + " has already been exist")
                    Exit Sub
                End If
                dr = ViewState("Dt4").NewRow
                dr("Type") = LblTypeMT.Text
                dr("DivisiBlok") = LblBlockMT.Text
            ...
            ' di sini harus difilter rows masuk ke datatable/dataview, baru di bind ke griddt4
            ' harus dicari source coding lagi. sudah lama ga pegang vb.net

            'BindGridDt(ViewState("Dt4"), GridDt4)

            ' Filtering data gridview ketika Insert(Mirza/ 28 mei 2020)
            Dim drow As DataRow()
            If ViewState("Dt4") Is Nothing Then
                BindDataDt4(ViewState("TransNmbr"))
            Else
    ...
    Protected Sub GridDt4_RowDeleting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewDeleteEventArgs) Handles GridDt4.RowDeleting
        Try
            ...
            dr(0).Delete()
            'BindGridDt(ViewState("Dt4"), GridDt4)
            ' Filtering data gridview ketika Insert
            Dim drow As DataRow()
            If ViewState("Dt4") Is Nothing Then
                BindDataDt4(ViewState("TransNmbr"))
            Else
    ...
    Protected Sub btnReff_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnReff.Click
        Dim ResultField As String
        Try
            Session("filter") = "SELECT  TransNmbr, Team, Person, JobPlant, JobPlant_Name, Rotasi, StartDate, EndDate, Qty, Section, RK_Year, RK_Month, Supplier, SuppName, Unit From V_PLWOPlanGetDivBlock where Division =" + QuotedStr(ddlDivisi.SelectedValue)
            ResultField = "TransNmbr, JobPlant, JobPlant_Name, Rotasi, Team, Person, Unit"
    ...
    Protected Sub btnGetDt_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnGetDt.Click
        ...
        Try
            If ddlReffType.SelectedValue = "Schedule" Then
                Session("filter") = "EXEC S_PLWOPlanReff " + QuotedStr(ddlReffType.SelectedValue) + ", ' A.TransNmbr = '" + QuotedStr(tbReffNo.Text) + "' And A.JobPlant = '" + QuotedStr(tbJob.Text) + "' And A.Division = '" + QuotedStr(ddlDivisi.SelectedValue) + "' And A.Rotasi = " + tbRotasi.Text + "'"
            ...
            Else
                ...
                ResultField = "Type, DivisiBlokCode, DivisiBlokName, PlantPeriodCode, PlantPeriodName,Qty"
            End If
    ...
    Protected Sub tbTotal_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbTotal.TextChanged
        Try
            If tbPercentage.Text > 100 Then tbPercentage.Text = 100
            If tbPercentage.Text < 0 Then tbPercentage.Text = 0
            tbQtyWO.Text = (Val(tbPercentage.Text) / 100) * Val(tbTotal.Text)
        ...
        Try
            ...
            Else
                ...
                For Each GVR In GridDt.Rows
                    ...
                    StartDate = GVR.Cells(13).Text
                    ViewState("Percentage") = 100
                    ViewState("Qty") = GVR.Cells(9).Text
                    ViewState("QtyTotal") = GVR.Cells(8).Text
                    ViewState("Capacity") = GVR.Cells(10).Text
                    ViewState("TargetHK") = GVR.Cells(11).Text
                    ViewState("WorkDay") = GVR.Cells(12).Text

                    If ViewState("Percentage") > 100 Then ViewState("Percentage") = 100
                    If ViewState("Percentage") < 0 Then ViewState("Percentage") = 0
                    GVR.Cells(7).Text = (Val(ViewState("Percentage") / 100) * Val(ViewState("QtyTotal")))

                    If (ViewState("Capacity") > 0) And (tbPerson.Text > 0) Then
                        GVR.Cells(12).Text = FormatNumber((GVR.Cells(7).Text) / CFloat(ViewState("Capacity")) / CInt(tbPerson.Text), 2)
                        GVR.Cells(14).Text = Date.FromOADate(StartDate.ToOADate() + CInt(GVR.Cells(12).Text))
                    ...
                    Row.BeginEdit()
                    Row("NormaHK") = GVR.Cells(10).Text
                    Row("TargetHK") = GVR.Cells(11).Text
                    Row("WorkDay") = GVR.Cells(12).Text
                    Row("EndDate") = FormatDateTime(GVR.Cells(14).Text)
                    Row.EndEdit()
    ...
    Protected Sub tbQtyCapacity_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbQtyCapacity.TextChanged
        Try
            If (tbQtyCapacity.Text > 0) And (tbPerson.Text > 0) Then
                tbQtyWorkDay.Text = FormatNumber((tbQtyWO.Text / tbQtyCapacity.Text) / CInt(tbPerson.Text), 2)

                tbEndDate.SelectedDate = Date.FromOADate(tbStartDate.SelectedDate.ToOADate() + CInt(tbQtyWorkDay.Text))
                'lbStatus.Text = tbEndDate.SelectedDate
                'Exit Sub
            Else : tbQtyWorkDay.Text = 0
            End If

            If (tbQtyCapacity.Text > 0) And (tbQtyWO.Text > 0) Then
                tbQtyTarget.Text = FormatNumber((tbQtyWO.Text) / (tbQtyCapacity.Text), 2)
            Else : tbQtyTarget.Text = 0
            End If
        Catch ex As Exception
            Throw New Exception("tbQtyCapacity_TextChanged Error : " + ex.ToString)
    ...
    Protected Sub btnBackDt2_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnBackDt2.Click, btnBackDt2ke2.Click, BtnBackDt3.Click, BtnBackDt3ke2.Click, BtnBackDt4.Click, btnbackDt4ke2.Click
        ...
        '   btnBackDt2ke2
    End Sub
    Private Sub GetMachine(ByVal job As String, ByVal reff As String, ByVal Divisi As String, ByVal Type As String)
        Dim drResult As DataRow
        Dim ExistRow As DataRow()
        Dim dtUnit As DataTable
        Dim drUnit As DataRow
        ...
        Try
            For Each drResult In Session("Result").Rows
                ExistRow = ViewState("Dt2").Select("DivisiBlok = " + QuotedStr(Divisi) + " AND Type = " + QuotedStr(Type))
                If ExistRow.Count = 0 Then
                    SQLString = "EXEC S_PLWOPlanGetMachineAll " + QuotedStr(job) + " , " + QuotedStr(reff)
                    lbStatus.Text = SQLString
                    Exit For
                    dtUnit = SQLExecuteQuery(SQLString, ViewState("DBConnection")).Tables(0)
                    drUnit = dtUnit.Rows(0)
    ...
    Private Sub GetMaterial(ByVal type As String, ByVal reffNo As String, ByVal job As String, ByVal block As String, ByVal rotasi As Integer, ByVal divBlockType As String, ByVal statusTanam As String, ByVal qty As Double, ByVal pkk As Double, ByVal Divisi As String, ByVal ReffType As String)
        Dim drResult As DataRow
        Dim ExistRow As DataRow()
        Dim dtUnit As DataTable
        Dim drUnit As DataRow
        Try
            For Each drResult In Session("Result").Rows
                ExistRow = ViewState("Dt4").Select("DivisiBlok = " + QuotedStr(Divisi) + " AND Type = " + QuotedStr(type))
                If ExistRow.Count = 0 Then
                    dtUnit = SQLExecuteQuery("EXEC S_PLWOPlanFindRMReff " + QuotedStr(ReffType) + " , " + QuotedStr(reffNo) + " , " + QuotedStr(job) + " , " + QuotedStr(block) + " , " + rotasi + " , " + QuotedStr(divBlockType) + " , " + QuotedStr(statusTanam) + " , " + qty.ToString.Replace(",", "") + " , " + pkk.ToString.Replace(",", ""), ViewState("DBConnection")).Tables(0)
                    drUnit = dtUnit.Rows(0)
                    Dim dr As DataRow
                    dr = ViewState("Dt4").NewRow
                    dr("Type") = type
                ...
                End If
            Next
            BindGridDt(ViewState("Dt4"), GridDt2)
            EnableHd(GetCountRecord(ViewState("Dt4")) = 0)
            GridDt4.Columns(1).Visible = GetCountRecord(ViewState("Dt4")) > 0
        Catch ex As Exception
    ...
    Protected Sub lbCount_Click(ByVal sender As Object, ByVal e As EventArgs) Handles lbCount.Click
        ...
        Try
            'sqlstring = "SELECT * FROM V_PLWOPlanGetReff WHERE Reff_Type = 'TBM' "
            sqlstring = "SELECT * FROM V_PLWOPlanGetReff WHERE RK_Month is NOT NULL AND RK_Year is NOT NULL "
            ...
            ' Untuk proteksi apply data, jika data tak sesuai field di resultsame
            ...
            For Each GVR In GridDt.Rows
                ...
                For Each drResult2 In dt2.Rows
                    If CekExistData(ViewState("Dt4"), "Material, Type", drResult2("Material_Code") + "|" + LblTypeMT.Text) = False Then
                        ...
                        dr2("DivisiBlok") = LblBlockMT.Text
                        ...
                        If GVR.Cells(7).Text.Replace(",", "") > 0 Then
                            dr2("QtyDosis") = dr2("Qty") / GVR.Cells(7).Text.Replace(",", "")
                        Else
                            dr2("QtyDosis") = 0
                        End If

                        If GVR.Cells(9).Text.Replace(",", "") > 0 Then
                            dr2("QtyPokok") = dr2("Qty") / GVR.Cells(9).Text.Replace(",", "")
                        Else : dr2("QtyPokok") = 0
