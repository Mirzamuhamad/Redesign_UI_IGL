...
Partial Class Transaction_TrPayTrade_TrPayTrade
    ...
    Protected GetStringHd As String = "Select distinct TransNmbr, Nmbr, TransDate, Status, FgReport, SupplierCode, SupplierName, Supplier, Attn, Currency, TotalPayForex, TotalPayment, TotalOthers, TotalInvoice, TotalPPh, TotalCharge, TotalKurs, TotalSelisih, Remark From V_FNPayTradeHd "
        ...
        Try
            ...
            If Not Session("Result") Is Nothing Then
                ...
                ElseIf ViewState("Sender") = "btnDocNo" Then
                    ...
                    tbRate.Text = FormatFloat(Session("Result")(2).ToString, ViewState("DigitRate"))
    ...
    Private Sub SetInit()
        Try
            FillRange(ddlRange)
            lbTitle.Text = "Payment Trade" 'Request.QueryString("MenuName").ToString
            ...
            tbPPhToPaidDt2.Attributes.Add("OnKeyDown", "return PressNumeric();")

            tbPaymentForex.Attributes.Add("OnKeyDown", "return PressNumeric();")
    ...
    Private Sub Cleardt()
        Try
            ...
            ChangePaymentType2(ddlPaymentType.SelectedValue, tbFgMode, tbPaymentDate, tbDueDate, ddlBankPayment, ddlCurr, tbRate, ViewState("Currency"), ViewState("DBConnection"))
            FillCombo(ddlBankPayment, "EXEC S_GetBankPaymentTrade " + QuotedStr(ddlCurr.SelectedValue), True, "Bank_Code", "Bank_Name", ViewState("DBConnection")) 'ddlReport.SelectedValue
            ...
            If CFloat(tbTotalInvoice.Text) > (CFloat(tbTotalPayment.Text) - CFloat(tbTotalOther.Text)) Then
                tbPaymentForex.Text = FormatFloat((CFloat(tbTotalInvoice.Text) - (CFloat(tbTotalPayment.Text) - CFloat(tbTotalOther.Text))) / CFloat(tbRate.Text), ViewState("DigitCurr"))
            Else
    ...
    Protected Sub btnSaveDt_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnSaveDt.Click
        Try
            ...
            If ViewState("StateDt") = "Edit" Then
                ...
                Row("PaymentDate") = Format(tbPaymentDate.SelectedDate, "dd MMM yyyy")
            ...
            Else
                ...
                dr("PaymentDate") = Format(tbPaymentDate.SelectedDate, "dd MMM yyyy")
    ...
    Private Sub CountTotalInvoiceHome(ByVal Item As String)
        Dim HasilKurs, HasilInv, HasilPPh, Base, PPN, PPh, RateInv, RatePPn, RatePay, PayRate, PayRatePPn, FgValue As Double
        Dim Dr As DataRow
        Dim drow As DataRow()
        ...
        Try
            ...
            '    If Not Dr.RowState = DataRowState.Deleted Then
            '        HasilInv = HasilInv + CFloat(Dr("InvoiceHome").ToString)
            '        HasilPPh = HasilPPh + CFloat(Dr("PPhHome").ToString)
            '        'HasilSR = HasilSR + CFloat(Dr("SelisihHome").ToString)
            '    End If
            'Next
            tbTotalInvoice.Text = FormatNumber(HasilInv, ViewState("DigitHome"))
            tbTotalPPh.Text = FormatNumber(HasilPPh, ViewState("DigitHome"))
            tbTotalKurs.Text = FormatNumber(HasilKurs, ViewState("DigitHome"))
            tbTotalSelisih.Text = FormatNumber(CFloat(tbTotalPayment.Text) + CFloat(tbTotalOther.Text) + CFloat(tbTotalPPh.Text) - CFloat(tbTotalCharge.Text) - CFloat(tbTotalInvoice.Text) - CFloat(tbTotalKurs.Text), ViewState("DigitHome"))
        Catch ex As Exception
    ...
    Private Sub CountTotalDt()
        Dim infois, peimen, PPh, cas, other, payforex As Double
        Dim Dr As DataRow
        Try
            infois = 0
            peimen = 0
            ...
            For Each Dr In ViewState("Dt").Rows
                ...
                End If
            Next
            tbTotalPaymentForex.Text = FormatNumber(payforex, ViewState("DigitHome"))
    ...
    Private Sub SaveAll()
        ...
        Try
            ...
            If ViewState("StateSave") = "Insert" Then
                ...
                "Supplier, Attn, TotalPayForex, TotalPayment, TotalOthers, TotalInvoice, TotalPPh, TotalCharge, TotalKurs, TotalSelisih, Remark, UserPrep, " + _
                "DatePrep) " + _
                ...
                ", " + tbTotalPaymentForex.Text.Replace(",", "") + _
            ...
            Dim cmdSql As New SqlCommand("SELECT TransNmbr, ItemNo, PaymentType, PaymentDate, DocumentNo, Reference, GiroDate, DueDate, BankPayment, Currency, ForexRate, PaymentForex, PaymentHome, InvoiceHome, PPhHome, Remark, FgMode, FgValue FROM FinPayTradeDt WHERE TransNmbr = " + QuotedStr(ViewState("TransNmbr")), con)
    ...
    Protected Sub btnGetInv_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnGetInv.Click
        ...
        Try
            ...
            Session("DBConnection") = ViewState("DBConnection")
            Session("filter") = sqlstring
            ResultField = "Invoice_No, Currency, Forex_Rate, Supplier_Invoice, Base_Forex, PPN_Forex, Total_Forex, Base_Paid, PPN_Paid, Total_Paid, Amount_Saldo, PPN_Rate, Due_Date, FgValue, PPh_Forex, PPh_Paid, PPh_Saldo, Supplier, Supplier_Name"
            CriteriaField = "Invoice_No, Supplier_Invoice, Supplier, Supplier_Name, Currency"
