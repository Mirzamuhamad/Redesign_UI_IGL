...
Partial Class Assign_AssUserProduct_AssUserProduct
    ...
    Protected Sub btnCopyTo_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnCopyTo.Click
        Try
            ...
            ddlGroupByCopy.SelectedValue = ddlGroupByCopy.SelectedValue
    ...
    Private Sub LoadDataGrid()
        Try
            ...
            If tbCode.Text.Trim <> "" Then
                dsAssigned.SelectCommand = "EXEC S_SAUserProductAssign " + QuotedStr(tbCode.Text) + ", " + ddlGroupBy.SelectedIndex.ToString + ""
                dsAvailable.SelectCommand = "EXEC S_SAUserProductAvailable " + QuotedStr(tbCode.Text) + ", " + ddlGroupBy.SelectedIndex.ToString + ""
            Else
    ...
    Private Sub ExecSPAddRemove(ByVal GroupValue As String, ByVal Page As Integer, ByVal StrSelected As String, ByVal Flag As Integer)
        ...
        Try
            SQLString = "EXEC S_SAUserProductAddRemove '" + GroupValue + "', " + Page.ToString + ", '" + StrSelected + "', '" + ViewState("UserId").ToString + "', " + Flag.ToString
    ...
    Protected Sub btnAdd_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnAdd.Click
        ...
        Try
            ...
            If ddlGroupBy.SelectedValue = "User" Then
                'StrSelected = SelectedGrid(AvailableGrid, "cbAvailable", "lbAvailableCode", "ProductTypeCode")
                StrSelected = SelectedGridDev(AvailableGrid, "ProductCode")
            ElseIf ddlGroupBy.SelectedValue = "Product Type" Then
                'StrSelected = SelectedGrid(AvailableGrid, "cbAvailable", "lbAvailableCode", "userId")
    ...
    Protected Sub btnRemove_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnRemove.Click
        ...
        Try
            ...
            StrSelected = ""
            If ddlGroupBy.SelectedValue = "User" Then
                'StrSelected = SelectedGrid(AssignedGrid, "cbAssign", "lbAssignCode", "ProductType")
                StrSelected = SelectedGridDev(AssignedGrid, "ProductType")
            ElseIf ddlGroupBy.SelectedValue = "Product Type" Then
                'StrSelected = SelectedGrid(AssignedGrid, "cbAssign", "lbAssignCode", "UserId")
    ...
    Protected Sub btnCopy_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnCopy.Click
        ...
        Try
            ...
            SqlString = ""
            ' 0 = User   1 = User
            SqlString = "EXEC S_SAUserProductCopyFrom " + QuotedStr(tbFromCode.Text) + ", " + QuotedStr(tbToCode.Text) + ", " + ddlGroupByCopy.SelectedIndex.ToString + ", " + QuotedStr(ViewState("UserId"))

            SQLExecuteNonQuery(SqlString, ViewState("DBConnection").ToString)
            lbstatus.Text = MessageDlg("Data Copied Successfully")
    ...
    Protected Sub btnSearch_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnSearch.Click
        ...
        Try
            If ddlGroupBy.SelectedValue = "User" Then
                Session("filter") = "select * From VSAUsers where FgAdmin <> 'Y'"
                ResultField = "User_Id, User_Name"
            Else
                Session("filter") = "SELECT * FROM VMsProductMateri"
                ResultField = "Product_Materi_Code, Product_Materi_Name"
            End If
    ...
    Protected Sub tbCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbCode.TextChanged
        ...
        Try
            If ddlGroupBy.SelectedValue = "User" Then
                ds = SQLExecuteQuery("select User_Id, User_Name From VSAUsers Where FgAdmin <> 'Y' AND User_Id = " + QuotedStr(tbCode.Text), ViewState("DBConnection").ToString)
            Else 'If ddlGroupBy.SelectedValue = "ProductType" Then
                ds = SQLExecuteQuery("SELECT Product_Materi_Code, Product_Materi_Name FROM VMsProductMateri Where Product_Materi_Code = " + QuotedStr(tbCode.Text), ViewState("DBConnection").ToString)
            End If
    ...
    Protected Sub tbFromCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbFromCode.TextChanged
        ...
        Try
            If ddlGroupByCopy.SelectedValue = "User" Then
                ds = SQLExecuteQuery("select User_Id, User_Name From VSAUsers Where FgAdmin <> 'Y' AND User_Id = " + QuotedStr(tbFromCode.Text), ViewState("DBConnection").ToString)
            Else 'If ddlGroupBy.SelectedValue = "Account" Then
                ds = SQLExecuteQuery("SELECT Product_Materi_Code, Product_Materi_Name FROM VMsProductMateri Where Product_Materi_Code= " + QuotedStr(tbFromCode.Text), ViewState("DBConnection").ToString)
            End If
    ...
    Protected Sub tbToCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbToCode.TextChanged
        ...
        Try
            If ddlGroupByCopy.SelectedValue = "User" Then
                ds = SQLExecuteQuery("select User_Id, User_Name From VSAUsers Where FgAdmin <> 'Y' AND User_Id = " + QuotedStr(tbToCode.Text), ViewState("DBConnection").ToString)
            Else 'If ddlGroupBy.SelectedValue = "Account" Then
                ds = SQLExecuteQuery("SELECT Product_Materi_Code, Product_Materi_Name FROM VMsProductMateri Where Product_Materi_Code= " + QuotedStr(tbToCode.Text), ViewState("DBConnection").ToString)
            End If
    ...
    Protected Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        Dim SQLString As String
        Try
            If ddlGroupBy.SelectedValue = "Product Type" Then
                SQLString = "EXEC S_SAUserProductPrint 0, 'Product Type - User', 'User Code', 'User Name' , " + QuotedStr(ViewState("UserId"))
            Else
                SQLString = "EXEC S_SAUserProductPrint 1, 'User - Product Type', 'Product Type Code', 'Product Type Name' , " + QuotedStr(ViewState("UserId"))
            End If
