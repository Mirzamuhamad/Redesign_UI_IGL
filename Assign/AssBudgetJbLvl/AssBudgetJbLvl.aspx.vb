...
Partial Class Assign_AssBudgetJbLvl_AssBudgetJbLvl
    ...
    Private Sub LoadDataGrid()
        Try
            ...
            If tbCode.Text.Trim <> "" Then
                dsAssigned.SelectCommand = "EXEC S_MsBudgetJobLvlAssign " + QuotedStr(tbCode.Text) + ", " + ddlGroupBy.SelectedIndex.ToString '+ ", ''" '+ tbPercentage.Text.Replace(", ", "")
                dsAvailable.SelectCommand = "EXEC S_MsBudgetJobLvlAvailable " + QuotedStr(tbCode.Text) + ", " + ddlGroupBy.SelectedIndex.ToString '+ ", ''" '+ tbPercentage.Text.Replace(", ", "")
            Else
                dsAssigned.SelectCommand = "SELECT '' AS CODE, '' AS NAME, 0 AS Plafond"
    ...
    Private Sub ExecSPAddRemove(ByVal GroupValue As String, ByVal Page As Integer, ByVal StrSelected As String, ByVal Flag As Integer, ByVal Percentage As Integer)
        Dim SQLString As String
        Try
            SQLString = "EXEC S_MsBudgetJobLvlAddRemove '" + GroupValue + "', " + Page.ToString + ", '" + StrSelected + "', '" + ViewState("UserId").ToString + "', " + Flag.ToString + ", " + Percentage.ToString
    ...
    Protected Sub btnAddAll_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnAddAll.Click
        ...
        Try
            If Not IsNumeric(tbPercentage.Text) Then
                lbstatus.Text = "Plafond must be numeric type."
                tbPercentage.Focus()
                Exit Sub
            End If
            ...
            'Exit Sub
            ExecSPAddRemove(tbCode.Text, ddlGroupBy.SelectedIndex, StrFilter, 0, tbPercentage.Text.Replace(", ", ""))
            AvailableGrid.Selection.UnselectAll()
        Catch ex As Exception
    ...
    Protected Sub btnAdd_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnAdd.Click
        ...
        Try
            ...
            If tbCode.Text = "" Then Exit Sub
            StrSelected = ""
            If ddlGroupBy.SelectedValue = "Budget" Then
                StrSelected = SelectedGridDev(AvailableGrid, "JobLvlCode")
            ElseIf ddlGroupBy.SelectedValue = "Job Level" Then
                StrSelected = SelectedGridDev(AvailableGrid, "BudgetCode")
            End If
            If StrSelected.Trim <> "" Then
                ' 0 = Warehouse   1 = User
                ExecSPAddRemove(tbCode.Text, ddlGroupBy.SelectedIndex, StrSelected, 1, tbPercentage.Text.Replace(", ", ""))
            End If
            AvailableGrid.Selection.UnselectAll()
        Catch ex As Exception
    ...
    Protected Sub btnRemove_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnRemove.Click
        ...
        Try
            ...
            StrSelected = ""
            If ddlGroupBy.SelectedValue = "Budget" Then
                StrSelected = SelectedGridDev(AssignedGrid, "JobLvlCode")
            ElseIf ddlGroupBy.SelectedValue = "Job Level" Then
                StrSelected = SelectedGridDev(AssignedGrid, "BudgetReason")
            End If
            If StrSelected.Trim <> "" Then
                ' 0 = Warehouse   1 = User
                ExecSPAddRemove(tbCode.Text, ddlGroupBy.SelectedIndex, StrSelected, 2, tbPercentage.Text.Replace(", ", ""))
            End If
            AssignedGrid.Selection.UnselectAll()
        Catch ex As Exception
    ...
    Protected Sub btnRemoveAll_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnRemoveAll.Click
        ...
        Try
            ...
            Else
                StrFilter = ""
            End If
            ExecSPAddRemove(tbCode.Text, ddlGroupBy.SelectedIndex, StrFilter, 3, tbPercentage.Text.Replace(", ", ""))
            AssignedGrid.Selection.UnselectAll()
        Catch ex As Exception
    ...
    Protected Sub btnCopy_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnCopy.Click
        ...
        Try
            ...
            SqlString = ""
            ' 0 = Warehouse   1 = User
            SqlString = "EXEC S_MsBudgetJobLvlCopyFrom " + QuotedStr(tbFromCode.Text) + ", " + QuotedStr(tbToCode.Text) + ", " + ddlGroupByCopy.SelectedIndex.ToString + ", " + QuotedStr(ViewState("UserId"))
    ...
    Protected Sub btnSearch_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnSearch.Click
        ...
        Try
            If ddlGroupBy.SelectedValue = "Budget" Then
                Session("filter") = "SELECT * FROM VMsBudget"
                ResultField = "Budget_Code, Budget_Name"
            Else
                Session("filter") = "SELECT * FROM VMsJobLevel"
                ResultField = "Job_Level, Job_Level_Name"
            End If
    ...
    Protected Sub btnSearchFrom_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearchFrom.Click
        Dim ResultField As String
        Try
            If ddlGroupByCopy.SelectedValue = "Budget" Then
                Session("filter") = "SELECT * FROM VMsBudget"
                ResultField = "Budget_Code, Budget_Name"
            Else
                Session("filter") = "SELECT * FROM VMsJobLevel"
    ...
    Protected Sub tbCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbCode.TextChanged
        ...
        Try
            If ddlGroupBy.SelectedValue = "Budget" Then
                ds = SQLExecuteQuery("SELECT Budget_Code, Budget_Name FROM VMsBudget WHERE Budget_Code = " + QuotedStr(tbCode.Text), ViewState("DBConnection").ToString)
            Else 'If ddlGroupBy.SelectedValue = "User" Then
                ds = SQLExecuteQuery("SELECT Job_Level, Job_Level_Name FROM VMsJobLevel WHERE Job_Level = " + QuotedStr(tbCode.Text), ViewState("DBConnection").ToString)
            End If
    ...
    Protected Sub tbFromCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbFromCode.TextChanged
        ...
        Try
            If ddlGroupByCopy.SelectedValue = "Budget" Then
                ds = SQLExecuteQuery("SELECT Budget_Code, Budget_Name FROM VMsBudget WHERE Budget_Code = " + QuotedStr(tbFromCode.Text), ViewState("DBConnection").ToString)
            Else 'If ddlGroupBy.SelectedValue = "Account" Then
                ds = SQLExecuteQuery("SELECT Job_Level, Job_Level_Name FROM VMsJobLevel WHERE Job_Level = " + QuotedStr(tbFromCode.Text), ViewState("DBConnection").ToString)
            End If
    ...
    Protected Sub tbToCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbToCode.TextChanged
        ...
        Try
            If ddlGroupByCopy.SelectedValue = "Budget" Then
                ds = SQLExecuteQuery("SELECT Budget_Code, Budget_Name FROM VMsBudget WHERE Budget_Code = " + QuotedStr(tbToCode.Text), ViewState("DBConnection").ToString)
            Else 'If ddlGroupBy.SelectedValue = "Account" Then
                ds = SQLExecuteQuery("SELECT Job_Level, Job_Level_Name FROM VMsJobLevel WHERE Job_Level = " + QuotedStr(tbToCode.Text), ViewState("DBConnection").ToString)
            End If
    ...
    Protected Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        ...
        Try
            If ddlGroupBy.SelectedValue = "Budget" Then
                SQLString = "EXEC S_MsBudgetJobLvlPrint 0,'Budget - Job Level','Budget Code','Budget Name','Plafond','admin'"
                'SQLString = "EXEC S_MsBudgetJobLvlPrint 0, 'Budget - Job Level', 'Job Level Code', 'Job Level Name' "
            Else
                SQLString = "EXEC S_MsBudgetJobLvlPrint 1,'Budget - Job Level','Budget Code','Budget Name','Plafond','admin'"
                'SQLString = "EXEC S_MsBudgetJobLvlPrint 1, 'Job Level - Budget', 'Budget Code', 'Budget Name' "
            End If
            Session("PrintType") = "Print"
            Session("DBConnection") = ViewState("DBConnection")
            Session("SelectCommand") = SQLString
            Session("ReportFile") = ".../../../Rpt/RptPrintAssign3.frx"
    ...
    Private Sub LoadDataAvailable()
        Try
            If tbCode.Text.Trim <> "" Then
                dsAvailable.SelectCommand = "EXEC S_MsBudgetJobLvlAvailable " + QuotedStr(tbCode.Text) + ", " + ddlGroupBy.SelectedIndex.ToString
            Else
                dsAvailable.SelectCommand = "SELECT '' AS CODE, '' AS NAME"
            End If
            AvailableGrid.DataBind()
