...
Partial Class Assign_AssPayrollTerminate_AssPayrollTerminate
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            If Not IsPostBack Then
                ...
                tbPercentage.Text = CFloat(100.0)
                Dim dt As DataTable
    ...
    Private Sub LoadDataGrid()
        Try
            ...
            If tbCode.Text.Trim <> "" Then
                dsAssigned.SelectCommand = "EXEC S_MsPayrollPHKReasonAssign " + QuotedStr(tbCode.Text) + ", " + ddlGroupBy.SelectedIndex.ToString '+ ", ''" '+ tbPercentage.Text.Replace(", ", "")
                dsAvailable.SelectCommand = "EXEC S_MsPayrollPHKReasonAvailable " + QuotedStr(tbCode.Text) + ", " + ddlGroupBy.SelectedIndex.ToString '+ ", ''" '+ tbPercentage.Text.Replace(", ", "")
            Else
                dsAssigned.SelectCommand = "SELECT '' AS CODE, '' AS NAME, 0 AS Percentage"
    ...
    Private Sub ExecSPAddRemove(ByVal GroupValue As String, ByVal Page As Integer, ByVal StrSelected As String, ByVal Flag As Integer, ByVal Percentage As Integer)
        ...
        Try
            SQLString = "EXEC S_MsPayrollPHKReasonAddRemove '" + GroupValue + "', " + Page.ToString + ", '" + StrSelected + "', '" + ViewState("UserId").ToString + "', " + Flag.ToString + ", " + Percentage.ToString
    ...
    Protected Sub btnAddAll_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnAddAll.Click
        ...
        Try
            If Not IsNumeric(tbPercentage.Text) Then
                lbstatus.Text = "Percentage (%) must be numeric type."
                tbPercentage.Focus()
                Exit Sub
            End If
    ...
    Protected Sub btnRemove_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnRemove.Click
        ...
        Try
            ...
            ElseIf ddlGroupBy.SelectedValue = "Payroll" Then
                StrSelected = SelectedGridDev(AssignedGrid, "PHKReason")
            End If
            If StrSelected.Trim <> "" Then
                ' 0 = Warehouse   1 = User
    ...
    Protected Sub btnCopy_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnCopy.Click
        ...
        Try
            ...
            SqlString = ""
            ' 0 = Warehouse   1 = User
            SqlString = "EXEC S_MsPayrollPHKReasonCopyFrom " + QuotedStr(tbFromCode.Text) + ", " + QuotedStr(tbToCode.Text) + ", " + ddlGroupByCopy.SelectedIndex.ToString + ", " + QuotedStr(ViewState("UserId"))
    ...
    Protected Sub btnSearch_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnSearch.Click
        ...
        Try
            If ddlGroupBy.SelectedValue = "PHK Reason" Then
                Session("filter") = "SELECT * FROM VMsPHKReason"
                ResultField = "PHKCode, PHKName"
            Else
                Session("filter") = "SELECT * FROM VMsPayroll"
                ResultField = "PayrollCode, PayrollName"
            End If
    ...
    Protected Sub btnSearchFrom_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearchFrom.Click
        Dim ResultField As String
        Try
            If ddlGroupByCopy.SelectedValue = "PHK Reason" Then
                Session("filter") = "SELECT * FROM VMsPHKReason"
                ResultField = "PHKCode, PHKName"
            Else
                Session("filter") = "SELECT * FROM VMsPayroll"
    ...
    Protected Sub tbCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbCode.TextChanged
        ...
        Try
            If ddlGroupBy.SelectedValue = "PHK Reason" Then
                ds = SQLExecuteQuery("SELECT PHKCode, PHKName FROM MsPHKReason WHERE PHKCode = " + QuotedStr(tbCode.Text), ViewState("DBConnection").ToString)
            Else 'If ddlGroupBy.SelectedValue = "User" Then
                ds = SQLExecuteQuery("SELECT PayrollCode, PayrollName FROM MsPayroll WHERE PayrollCode = " + QuotedStr(tbCode.Text), ViewState("DBConnection").ToString)
            End If
    ...
    Protected Sub tbFromCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbFromCode.TextChanged
        ...
        Try
            If ddlGroupByCopy.SelectedValue = "PHK Reason" Then
                ds = SQLExecuteQuery("SELECT PHKCode, PHKName FROM MsPHKReason WHERE PHKCode = " + QuotedStr(tbFromCode.Text), ViewState("DBConnection").ToString)
            Else 'If ddlGroupBy.SelectedValue = "Account" Then
                ds = SQLExecuteQuery("SELECT PayrollCode, PayrollName FROM MsPayroll WHERE PayrollCode = " + QuotedStr(tbFromCode.Text), ViewState("DBConnection").ToString)
            End If
    ...
    Protected Sub tbToCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbToCode.TextChanged
        ...
        Try
            If ddlGroupByCopy.SelectedValue = "PHK Reason" Then
                ds = SQLExecuteQuery("SELECT PHKCode, PHKName FROM MsPHKReason WHERE PHKCode = " + QuotedStr(tbToCode.Text), ViewState("DBConnection").ToString)
            Else 'If ddlGroupBy.SelectedValue = "Account" Then
                ds = SQLExecuteQuery("SELECT PayrollCode, PayrollName FROM MsPayroll WHERE PayrollCode = " + QuotedStr(tbToCode.Text), ViewState("DBConnection").ToString)
            End If
    ...
    Protected Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        ...
        Try
            If ddlGroupBy.SelectedValue = "PHK Reason" Then
                SQLString = "EXEC S_MsPayrollPHKReasonPrint 0, 'PHKReason - Payroll', 'Payroll Code', 'Payroll Name','Percentage'," + QuotedStr(ViewState("UserId"))
            Else
                SQLString = "EXEC S_MsPayrollPHKReasonPrint 1, 'Payroll - PHKReason', 'PHKReason Code', 'PHKReason Name','Percentage'," + QuotedStr(ViewState("UserId"))
            End If
    ...
    Private Sub LoadDataAvailable()
        Try
            If tbCode.Text.Trim <> "" Then
                dsAvailable.SelectCommand = "EXEC S_MsPayrollPHKReasonAvailable " + QuotedStr(tbCode.Text) + ", " + ddlGroupBy.SelectedIndex.ToString
            Else
                dsAvailable.SelectCommand = "SELECT '' AS CODE, '' AS NAME"
            End If
            AvailableGrid.DataBind()
