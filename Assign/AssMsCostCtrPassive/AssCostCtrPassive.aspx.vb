...
Imports System.Data
Partial Class Assign_AssCostCtrPassive_AssCostCtrPassive
    ...
    Private Sub LoadDataGrid()
        Try
            ...
            If tbCode.Text.Trim <> "" Then
                dsAssigned.SelectCommand = "EXEC S_MSCostCtrPassiveAssign " + QuotedStr(tbCode.Text) + ", " + ddlGroupBy.SelectedIndex.ToString
                dsAvailable.SelectCommand = "EXEC S_MSCostCtrPassiveAvailable " + QuotedStr(tbCode.Text) + ", " + ddlGroupBy.SelectedIndex.ToString
            Else
    ...
    Private Sub ExecSPAddRemove(ByVal GroupValue As String, ByVal Page As Integer, ByVal StrSelected As String, ByVal Flag As Integer)
        ...
        Try
            SQLString = "EXEC S_MsCostCtrPassiveAddRemove '" + GroupValue + "', " + Page.ToString + ", '" + StrSelected + "', '" + ViewState("UserId").ToString + "', " + Flag.ToString
    ...
    Protected Sub btnAdd_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnAdd.Click
        ...
        Try
            If tbCode.Text = "" Then Exit Sub
            StrSelected = ""
            If ddlGroupBy.SelectedValue = "Cost Center Passive" Then
                StrSelected = SelectedGridDev(AvailableGrid, "CostCtrCode")
            ElseIf ddlGroupBy.SelectedValue = "Cost Center Active" Then
                StrSelected = SelectedGridDev(AvailableGrid, "CostCtrCode")
            End If
    ...
    Protected Sub btnRemove_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnRemove.Click
        ...
        Try
            ...
            StrSelected = ""
            If ddlGroupBy.SelectedValue = "Cost Center Passive" Then
                StrSelected = SelectedGridDev(AssignedGrid, "CostCtrCode")
            ElseIf ddlGroupBy.SelectedValue = "Cost Center Active" Then
                StrSelected = SelectedGridDev(AssignedGrid, "CostCtrCode")
            End If
            If StrSelected.Trim <> "" Then
    ...
    Protected Sub btnCopy_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnCopy.Click
        ...
        Try
            ...
            SqlString = ""
            ' 0 = Warehouse   1 = User
            SqlString = "EXEC S_MsCostCtrPassiveCopyFrom " + QuotedStr(tbFromCode.Text) + ", " + QuotedStr(tbToCode.Text) + ", " + ddlGroupByCopy.SelectedIndex.ToString + ", " + QuotedStr(ViewState("UserId"))
    ...
    Protected Sub btnSearch_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnSearch.Click
        ...
        Try
            If ddlGroupBy.SelectedValue = "Cost Center Active" Then
                Session("filter") = "SELECT CostCtrCode, CostCtrName FROM MsCostCtr WHERE FgType = 'Active'"
                ResultField = "CostCtrCode, CostCtrName"
            Else
                Session("filter") = "SELECT CostCtrCode, CostCtrName FROM MsCostCtr WHERE FgType = 'Passive'"
                ResultField = "CostCtrCode, CostCtrName"
            End If
    ...
    Protected Sub btnSearchFrom_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearchFrom.Click
        Dim ResultField As String
        Try
            If ddlGroupByCopy.SelectedValue = "Cost Center Active" Then
                Session("filter") = "SELECT CostCtrCode, CostCtrName FROM MsCostCtr WHERE FgType = 'Active'"
                ResultField = "CostCtrCode, CostCtrName"
            Else
                Session("filter") = "SELECT CostCtrCode, CostCtrName FROM MsCostCtr WHERE FgType = 'Passive'"
    ...
    Protected Sub tbCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbCode.TextChanged
        ...
        Try
            If ddlGroupBy.SelectedValue = "Cost Center Active" Then
                ds = SQLExecuteQuery("SELECT CostCtrCode, CostCtrName FROM MsCostCtr WHERE FgType = 'Active' AND CostCtrCode = " + QuotedStr(tbCode.Text), ViewState("DBConnection").ToString)
            Else 'If ddlGroupBy.SelectedValue = "User" Then
                ds = SQLExecuteQuery("SELECT CostCtrCode, CostCtrName FROM MsCostCtr WHERE FgType = 'Passive' AND CostCtrCode = " + QuotedStr(tbCode.Text), ViewState("DBConnection").ToString)
            End If
    ...
    Protected Sub tbFromCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbFromCode.TextChanged
        ...
        Try
            If ddlGroupByCopy.SelectedValue = "Cost Center Active" Then
                ds = SQLExecuteQuery("SELECT CostCtrCode, CostCtrName FROM MsCostCtr WHERE FgType = 'Active' AND CostCtrCode = " + QuotedStr(tbFromCode.Text), ViewState("DBConnection").ToString)
            Else 'If ddlGroupBy.SelectedValue = "Account" Then
                ds = SQLExecuteQuery("SELECT CostCtrCode, CostCtrName FROM MsCostCtr WHERE FgType = 'Passive' AND CostCtrCode = " + QuotedStr(tbFromCode.Text), ViewState("DBConnection").ToString)
            End If
    ...
    Protected Sub tbToCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbToCode.TextChanged
        ...
        Try
            If ddlGroupByCopy.SelectedValue = "Cost Center Active" Then
                ds = SQLExecuteQuery("SELECT CostCtrCode, CostCtrName FROM MsCostCtr WHERE FgType = 'Active' AND CostCtrCode = " + QuotedStr(tbToCode.Text), ViewState("DBConnection").ToString)
            Else 'If ddlGroupBy.SelectedValue = "Account" Then
                ds = SQLExecuteQuery("SELECT CostCtrCode, CostCtrName FROM MsCostCtr WHERE FgType = 'Passive' AND CostCtrCode = " + QuotedStr(tbToCode.Text), ViewState("DBConnection").ToString)
            End If
    ...
    Protected Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        ...
        Try
            If ddlGroupBy.SelectedValue = "Cost Center Active" Then
                SQLString = "EXEC S_MSCostCtrPassivePrint 0, 'Cost Center Active - Cost Center Passive', 'Cost Center Passive', 'Cost Center Pasive Name', " + QuotedStr(ViewState("UserId"))
            Else
                SQLString = "EXEC S_MSCostCtrPassivePrint 1, 'Cost Center Passive - Cost Center Active', 'Cost Center Active', 'Cost Center Active Name',  " + QuotedStr(ViewState("UserId"))
            End If
