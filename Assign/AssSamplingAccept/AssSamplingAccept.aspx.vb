...
Partial Class Assign_AssSamplingAccept_AssSamplingAccept
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            If Not IsPostBack Then
                ...
                tbAcceptable.Attributes.Add("OnKeyDown", "return PressNumeric();")
                tbMinReject.Attributes.Add("OnKeyDown", "return PressNumeric();")
                tbAcceptable.Text = CFloat(0)
                tbMinReject.Text = CFloat(0)
            End If
    ...
    Private Sub LoadDataGrid()
        Try
            ...
            If tbCode.Text.Trim <> "" Then
                dsAssigned.SelectCommand = "EXEC S_MsSamplingAcceptAssign " + QuotedStr(tbCode.Text) + ", " + ddlGroupBy.SelectedIndex.ToString '+ ", ''" '+ tbPercentage.Text.Replace(", ", "")
                dsAvailable.SelectCommand = "EXEC S_MsSamplingAcceptAvailable " + QuotedStr(tbCode.Text) + ", " + ddlGroupBy.SelectedIndex.ToString '+ ", ''" '+ tbPercentage.Text.Replace(", ", "")
            Else
                dsAssigned.SelectCommand = "SELECT '' AS CODE, '' AS NAME, 0 AS Acceptable, 0 AS MinReject"
    ...
    Private Sub ExecSPAddRemove(ByVal GroupValue As String, ByVal Page As Integer, ByVal StrSelected As String, ByVal Flag As Integer, ByVal Acceptable As Double, ByVal MinReject As Double)
        Dim SQLString As String
        Try
            SQLString = "EXEC S_MsSamplingAcceptAddRemove '" + GroupValue + "', " + Page.ToString + ", '" + StrSelected + "', '" + ViewState("UserId").ToString + "', " + Flag.ToString + ", " + Acceptable.ToString.Replace(",", "") + ", " + MinReject.ToString.Replace(",", "")
    ...
    Protected Sub btnAddAll_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnAddAll.Click
        ...
        Dim P As Integer
        Try
            If Not IsNumeric(tbAcceptable.Text) Then
                lbstatus.Text = "Acceptable must be numeric type."
                tbAcceptable.Focus()
                Exit Sub
            End If
            If Not IsNumeric(tbMinReject.Text) Then
                lbstatus.Text = "Min Reject must be numeric type."
                tbMinReject.Focus()
                Exit Sub
            End If
            ...
            'Exit Sub
            ExecSPAddRemove(tbCode.Text, ddlGroupBy.SelectedIndex, StrFilter, 0, tbAcceptable.Text.Replace(", ", ""), tbMinReject.Text.Replace(", ", ""))
            AvailableGrid.Selection.UnselectAll()
        Catch ex As Exception
    ...
    Protected Sub btnAdd_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnAdd.Click
        ...
        Try
            ...
            If tbCode.Text = "" Then Exit Sub
            StrSelected = ""
            If ddlGroupBy.SelectedValue = "Sampling" Then
                StrSelected = SelectedGridDev(AvailableGrid, "DefectGroupCode")
            ElseIf ddlGroupBy.SelectedValue = "Defect Group" Then
                StrSelected = SelectedGridDev(AvailableGrid, "SamplingCode")
            End If
            If StrSelected.Trim <> "" Then
                ' 0 = Warehouse   1 = User
                ExecSPAddRemove(tbCode.Text, ddlGroupBy.SelectedIndex, StrSelected, 1, tbAcceptable.Text.Replace(", ", ""), tbMinReject.Text.Replace(", ", ""))
            End If
            AvailableGrid.Selection.UnselectAll()
        Catch ex As Exception
    ...
    Protected Sub btnRemove_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnRemove.Click
        ...
        Try
            ...
            StrSelected = ""
            If ddlGroupBy.SelectedValue = "Sampling" Then
                StrSelected = SelectedGridDev(AssignedGrid, "DefectGroupCode")
            ElseIf ddlGroupBy.SelectedValue = "Defect Group" Then
                StrSelected = SelectedGridDev(AssignedGrid, "SamplingCode")
            End If
            If StrSelected.Trim <> "" Then
                ' 0 = Warehouse   1 = User
                ExecSPAddRemove(tbCode.Text, ddlGroupBy.SelectedIndex, StrSelected, 2, tbAcceptable.Text.Replace(", ", ""), tbMinReject.Text.Replace(", ", ""))
            End If
            AssignedGrid.Selection.UnselectAll()
        Catch ex As Exception
    ...
    Protected Sub btnRemoveAll_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnRemoveAll.Click
        ...
        Try
            ...
            Else
                StrFilter = ""
            End If
            ExecSPAddRemove(tbCode.Text, ddlGroupBy.SelectedIndex, StrFilter, 3, tbAcceptable.Text.Replace(", ", ""), tbMinReject.Text.Replace(", ", ""))
            AssignedGrid.Selection.UnselectAll()
        Catch ex As Exception
    ...
    Protected Sub btnCopy_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnCopy.Click
        ...
        Try
            ...
            SqlString = ""
            ' 0 = Warehouse   1 = User
            SqlString = "EXEC S_MsSamplingAcceptCopyFrom " + QuotedStr(tbFromCode.Text) + ", " + QuotedStr(tbToCode.Text) + ", " + ddlGroupByCopy.SelectedIndex.ToString + ", " + QuotedStr(ViewState("UserId"))
    ...
    Protected Sub btnSearch_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnSearch.Click
        ...
        Try
            If ddlGroupBy.SelectedValue = "Sampling" Then
                Session("filter") = "SELECT SamplingCode, SamplingType, RangeStart, RangeEnd, Qty, Percentage FROM MsSampling"
                ResultField = "SamplingCode, SamplingType"
            Else
                Session("filter") = "SELECT * FROM VMsDefectGroup"
                ResultField = "Defect_Group_Code, Defect_Group_Name"
            End If
    ...
    Protected Sub btnSearchFrom_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearchFrom.Click
        Dim ResultField As String
        Try
            If ddlGroupByCopy.SelectedValue = "Sampling" Then
                Session("filter") = "SELECT SamplingCode, SamplingType, RangeStart, RangeEnd, Qty, Percentage FROM MsSampling"
                ResultField = "SamplingCode, SamplingType"
            Else
                Session("filter") = "SELECT * FROM VMsDefectGroup"
    ...
    Protected Sub tbCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbCode.TextChanged
        ...
        Try
            If ddlGroupBy.SelectedValue = "Sampling" Then
                ds = SQLExecuteQuery("SELECT SamplingCode, SamplingType FROM MsSampling WHERE SamplingCode = " + QuotedStr(tbCode.Text), ViewState("DBConnection").ToString)
            Else 'If ddlGroupBy.SelectedValue = "User" Then
                ds = SQLExecuteQuery("SELECT Defect_Group_Code, Defect_Group_Name FROM VMsDefectGroup WHERE Defect_Group_Code = " + QuotedStr(tbCode.Text), ViewState("DBConnection").ToString)
            End If
    ...
    Protected Sub tbFromCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbFromCode.TextChanged
        ...
        Try
            If ddlGroupByCopy.SelectedValue = "Sampling" Then
                ds = SQLExecuteQuery("SELECT SamplingCode, SamplingType FROM MsSampling WHERE SamplingCode = " + QuotedStr(tbFromCode.Text), ViewState("DBConnection").ToString)
            Else 'If ddlGroupBy.SelectedValue = "User" Then
                ds = SQLExecuteQuery("SELECT Defect_Group_Code, Defect_Group_Name FROM VMsDefectGroup WHERE Defect_Group_Code = " + QuotedStr(tbFromCode.Text), ViewState("DBConnection").ToString)
            End If
            If IsNothing(ds) Or ds.Tables(0).Rows.Count <= 0 Or IsNothing(ds.Tables(0)) Then
                tbFromCode.Text = ""
                tbFromName.Text = ""
    ...
    Protected Sub tbToCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbToCode.TextChanged
        ...
        Try
            ...
            If ddlGroupByCopy.SelectedValue = "Sampling" Then
                ds = SQLExecuteQuery("SELECT SamplingCode, SamplingType FROM MsSampling WHERE SamplingCode = " + QuotedStr(tbToCode.Text), ViewState("DBConnection").ToString)
            Else 'If ddlGroupBy.SelectedValue = "Account" Then
                ds = SQLExecuteQuery("SELECT Defect_Group_Code, Defect_Group_Name FROM VMsDefectGroup WHERE Defect_Group_Code = " + QuotedStr(tbToCode.Text), ViewState("DBConnection").ToString)
            End If
    ...
    Protected Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        ...
        Try
            If ddlGroupBy.SelectedValue = "Sampling" Then
                SQLString = "EXEC S_MsSamplingAcceptPrint 0, 'Sampling Accept', 'Defect Code', 'Defect Group Name','Acceptable','Min Reject','Admin' "
            Else
                SQLString = "EXEC S_MsSamplingAcceptPrint 1, 'Sampling Accept', 'Defect Code', 'Defect Group Name','Acceptable','Min Reject','Admin'"
            End If
    ...
    Private Sub LoadDataAvailable()
        Try
            If tbCode.Text.Trim <> "" Then
                dsAvailable.SelectCommand = "EXEC S_MsSamplingAcceptAvailable " + QuotedStr(tbCode.Text) + ", " + ddlGroupBy.SelectedIndex.ToString
            Else
                dsAvailable.SelectCommand = "SELECT '' AS CODE, '' AS NAME"
            End If
            AvailableGrid.DataBind()
