...
Partial Class Assign_AssPayrollGroupDt_AssPayrollGroupDt
    ...
    Private Sub LoadDataGrid()
        Try
            ...
            If tbCode.Text.Trim <> "" Then
                dsAssigned.SelectCommand = "EXEC S_MsPayrollGroupDtAssign " + QuotedStr(tbCode.Text) + ", " + ddlGroupBy.SelectedIndex.ToString '+ ", ''" '+ tbPercentage.Text.Replace(", ", "")
                dsAvailable.SelectCommand = "EXEC S_MsPayrollGroupDtAvailable " + QuotedStr(tbCode.Text) + ", " + ddlGroupBy.SelectedIndex.ToString '+ ", ''" '+ tbPercentage.Text.Replace(", ", "")
            Else
    ...
    Private Sub ExecSPAddRemove(ByVal GroupValue As String, ByVal Page As Integer, ByVal StrSelected As String, ByVal Flag As Integer)
        ...
        Try
            SQLString = "EXEC S_MsPayrollGroupDtAddRemove '" + GroupValue + "', " + Page.ToString + ", '" + StrSelected + "', '" + ViewState("UserId").ToString + "', " + Flag.ToString
    ...
    Protected Sub btnAdd_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnAdd.Click
        ...
        Try
            ...
            If tbCode.Text = "" Then Exit Sub
            StrSelected = ""
            If ddlGroupBy.SelectedValue = "Group Payroll" Then
                StrSelected = SelectedGridDev(AvailableGrid, "PayrollCode")
            ElseIf ddlGroupBy.SelectedValue = "Payroll" Then
                StrSelected = SelectedGridDev(AvailableGrid, "PayrollGrpCode")
            End If
    ...
    Protected Sub btnRemove_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnRemove.Click
        ...
        Try
            ...
            ElseIf ddlGroupBy.SelectedValue = "Payroll" Then
                StrSelected = SelectedGridDev(AssignedGrid, "PayrollGroup")
            End If
            If StrSelected.Trim <> "" Then
                ' 0 = Warehouse   1 = User
    ...
    Protected Sub btnCopy_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnCopy.Click
        ...
        Try
            ...
            SqlString = ""
            ' 0 = Warehouse   1 = User
            SqlString = "EXEC S_MsPayrollGroupDtCopyFrom " + QuotedStr(tbFromCode.Text) + ", " + QuotedStr(tbToCode.Text) + ", " + ddlGroupByCopy.SelectedIndex.ToString + ", " + QuotedStr(ViewState("UserId"))
    ...
    Protected Sub btnSearch_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnSearch.Click
        ...
        Try
            If ddlGroupBy.SelectedValue = "Group Payroll" Then
                Session("filter") = "SELECT * FROM VMsPayrollGroup"
                ResultField = "PayrollGrpCode, PayrollGrpName"
            Else
                Session("filter") = "SELECT * FROM VMsPayroll WHERE PayrollPref = 'P' and PayrollType IN ('GP','TT','TTT','POT') "
    ...
    Protected Sub btnSearchFrom_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearchFrom.Click
        Dim ResultField As String
        Try
            If ddlGroupByCopy.SelectedValue = "Group Payroll" Then
                Session("filter") = "SELECT * FROM VMsPayrollGroup"
                ResultField = "PayrollGrpCode, PayrollGrpName"
            Else
                Session("filter") = "SELECT * FROM VMsPayroll WHERE PayrollPref = 'P' and PayrollType IN ('GP','TT','TTT','POT') "
    ...
    Protected Sub tbCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbCode.TextChanged
        ...
        Try
            If ddlGroupBy.SelectedValue = "Group Payroll" Then
                ds = SQLExecuteQuery("SELECT PayrollGrpCode, PayrollGrpName FROM MsPayrollGroup WHERE PayrollGrpCode = " + QuotedStr(tbCode.Text), ViewState("DBConnection").ToString)
            Else 'If ddlGroupBy.SelectedValue = "User" Then
                ds = SQLExecuteQuery("SELECT PayrollCode, PayrollName FROM MsPayroll WHERE PayrollPref = 'P' and PayrollType IN ('GP','TT','TTT','POT') AND PayrollCode = " + QuotedStr(tbCode.Text), ViewState("DBConnection").ToString)
            End If
    ...
    Protected Sub tbFromCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbFromCode.TextChanged
        ...
        Try
            If ddlGroupByCopy.SelectedValue = "Group Payroll" Then
                ds = SQLExecuteQuery("SELECT PayrollGrpCode, PayrollGrpName FROM MsPayrollGroup WHERE PayrollGrpCode = " + QuotedStr(tbFromCode.Text), ViewState("DBConnection").ToString)
            Else 'If ddlGroupBy.SelectedValue = "Account" Then
                ds = SQLExecuteQuery("SELECT PayrollCode, PayrollName FROM MsPayroll WHERE PayrollPref = 'P' and PayrollType IN ('GP','TT','TTT','POT') AND PayrollCode = " + QuotedStr(tbFromCode.Text), ViewState("DBConnection").ToString)
            End If
    ...
    Protected Sub tbToCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbToCode.TextChanged
        ...
        Try
            If ddlGroupByCopy.SelectedValue = "Group Payroll" Then
                ds = SQLExecuteQuery("SELECT PayrollGrpCode, PayrollGrpName FROM MsPayrollGroup WHERE PayrollGrpCode = " + QuotedStr(tbToCode.Text), ViewState("DBConnection").ToString)
            Else 'If ddlGroupBy.SelectedValue = "Account" Then
                ds = SQLExecuteQuery("SELECT PayrollCode, PayrollName FROM MsPayroll WHERE PayrollPref = 'P' and PayrollType IN ('GP','TT','TTT','POT') AND PayrollCode = " + QuotedStr(tbToCode.Text), ViewState("DBConnection").ToString)
            End If
    ...
    Protected Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        ...
        Try
            If ddlGroupBy.SelectedValue = "Group Payroll" Then
                SQLString = "EXEC S_MsPayrollGroupDtPrint 0, 'Payroll Group - Payroll', 'Payroll Code', 'Payroll Name' , " + QuotedStr(ViewState("UserId"))
            Else
                SQLString = "EXEC S_MsPayrollGroupDtPrint 1, 'Payroll - Payroll Group', 'Payroll Group Code', 'Payroll Group Name' , " + QuotedStr(ViewState("UserId"))
            End If
    ...
    Private Sub LoadDataAvailable()
        Try
            If tbCode.Text.Trim <> "" Then
                dsAvailable.SelectCommand = "EXEC S_MsPayrollGroupDtAvailable " + QuotedStr(tbCode.Text) + ", " + ddlGroupBy.SelectedIndex.ToString
            Else
                dsAvailable.SelectCommand = "SELECT '' AS CODE, '' AS NAME"
            End If
            AvailableGrid.DataBind()
