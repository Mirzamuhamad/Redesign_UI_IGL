...
Partial Class Assign_MsPayrollJobLvl_MsPayrollJobLvl
    ...
    Private Sub ExecSPAddRemove(ByVal StrSelected As String, ByVal Flag As Integer)
        ...
        Try
            SQLString = "EXEC S_MsPayrollJbLvlAddRemove '" + tbCode.Text.Trim + "', '" + Format(tbEffDate.SelectedDate, "yyyy-MM-dd") + "', '" + ddlCurr.SelectedValue + "'," + tbAmount.Text.ToString.Replace(",", "") + ",'" + ddlFormula.SelectedValue + "','" + tbRemark.Text + "', '" + StrSelected + "', '" + ViewState("UserId").ToString + "', " + Flag.ToString
    ...
    Private Sub ExecSPAddRemoveAdjust(ByVal StrSelected As String, ByVal Flag As Integer)
        Dim SQLString As String
        Try
            SQLString = "EXEC S_MsPayrollJbLvlAddAdjust '" + tbCode.Text.Trim + "', '" + Format(tbEffAdjust.SelectedDate, "yyyy-MM-dd") + "', '" + ddlAdjust.SelectedValue + "'," + tbAmountAdjust.Text.ToString.Replace(",", "") + ",'" + tbRemarkAdjust.Text + "', '" + StrSelected + "', '" + ViewState("UserId").ToString + "', " + Flag.ToString
            SQLExecuteQuery(SQLString, ViewState("DBConnection").ToString)
        Catch ex As Exception
    ...
    Protected Sub btnSearch_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnSearch.Click
        ...
        Try
            Session("filter") = "SELECT * FROM V_MsPayroll Where Payroll_Pref = 'P' AND Payroll_Type IN ('GP','TT','TTT','POT') AND GroupBy = 'Job Level'"
    ...
    Protected Sub tbCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbCode.TextChanged
        ...
        Dim ds As DataSet
        Try
            ds = SQLExecuteQuery("SELECT * FROM V_MsPayroll WHERE Payroll_Pref = 'P' AND Payroll_Type IN ('GP','TT','TTT','POT') AND Payroll_Code = " + QuotedStr(tbCode.Text) + " AND GroupBy = 'Job Level'", ViewState("DBConnection").ToString)
    ...
    Protected Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        ...
        Try
            ...
            'SQLString = "EXEC S_MsPayrollJbLvlPrint 0, 'Employee - Payroll Slip', 'Payroll Code', 'Payroll Name', 'Start Date', 'Formula', " + QuotedStr(ViewState("UserId").ToString)
            'Else
            SQLString = "EXEC S_MsPayrollJbLvlPrint " + QuotedStr(ViewState("UserId").ToString)
            'End If
    ...
    Private Sub bindDataGrid()
        Dim SqlString As String
        Try
            SqlString = "EXEC S_MsPayrollJbLvlView " + QuotedStr(tbCode.Text)
            If ViewState("SortExpression") = Nothing Then
                ViewState("SortExpression") = "Job_Level ASC"
            End If
    ...
    Private Sub LoadDataAvailable()
        Try
            ...
            dt = SQLExecuteQuery("EXEC S_MsPayrollJbLvlAvailable " + QuotedStr(tbCode.Text) + ", " + QuotedStr(Format(tbEffDate.SelectedDate, "yyyy-MM-dd")), ViewState("DBConnection").ToString).Tables(0)
            ViewState("Dt") = dt
            'If tbCode.Text.Trim <> "" Then
            'AvailableGrid.Selection.UnselectAll()
            dsAvailable.SelectCommand = "EXEC S_MsPayrollJbLvlAvailable " + QuotedStr(tbCode.Text) + ", " + QuotedStr(Format(tbEffDate.SelectedDate, "yyyy-MM-dd"))
            'Else
    ...
    Private Sub LoadDataAdjust()
        Try
            ...
            dt = SQLExecuteQuery("EXEC S_MsPayrollJbLvlAdjust " + QuotedStr(tbCode.Text) + ", " + QuotedStr(Format(tbEffAdjust.SelectedDate, "yyyy-MM-dd")), ViewState("DBConnection").ToString).Tables(0)
            ViewState("DtAdjust") = dt
            'GridAdjust.Selection.UnselectAll()
            dsAdjust.SelectCommand = "EXEC S_MsPayrollJbLvlAdjust " + QuotedStr(tbCode.Text) + ", " + QuotedStr(Format(tbEffAdjust.SelectedDate, "yyyy-MM-dd"))
            GridAdjust.DataBind()
        Catch ex As Exception
    ...
    Protected Sub DataGrid_RowDeleting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewDeleteEventArgs) Handles DataGrid.RowDeleting
        ...
        Try
            If CheckMenuLevel("Delete") = False Then
                Exit Sub
            End If
            EmpCode = DataGrid.Rows(e.RowIndex).FindControl("JobLevel")
            EffDate = DataGrid.Rows(e.RowIndex).FindControl("EffDate")
            SQLExecuteNonQuery("Delete from MsPayrollJbLvl where Payroll = " + QuotedStr(tbCode.Text) + " AND JobLevel = " + QuotedStr(EmpCode.Text) + " AND StartDate = " + QuotedStr(EffDate.Text), ViewState("DBConnection").ToString)
            bindDataGrid()
    ...
    Protected Sub DataGrid_RowUpdating(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewUpdateEventArgs) Handles DataGrid.RowUpdating
        ...
        Try
            ...
            lbCode = GVR.FindControl("JobLevelEdit")
            ...
            If CFloat(tbAmount.Text.Trim) <= 0 Then
                ...
                Exit Sub
            End If
            IfExist = SQLExecuteScalar("SELECT dbo.FormatDate(StartDate) FROM MsPayrollJbLvl WHERE Payroll = " + QuotedStr(tbCode.Text) + " AND JobLevel = " + QuotedStr(lbCode.Text) + " AND StartDate <> '" + Format(ViewState("StartDate"), "yyyy-MM-dd") + "' AND StartDate >= " + QuotedStr(Format(tbDate.SelectedDate, "yyyy-MM-dd")), ViewState("DBConnection").ToString)

            If IfExist.Trim.Length > 0 Then
                lbstatus.Text = MessageDlg("Payroll " + QuotedStr(tbCode.Text) + " AND JobLevel " + QuotedStr(lbCode.Text) + " has already have New Eff. Date " + QuotedStr(IfExist))
                tbDate.Focus()
                Exit Sub
            End If
            SQLString = "Update MsPayrollJbLvl set StartDate = " + QuotedStr(tbDate.Text) + _
            ...
            "  WHERE Payroll = " + QuotedStr(tbCode.Text) + " AND JobLevel = " + QuotedStr(lbCode.Text) + " AND StartDate = '" + Format(ViewState("StartDate"), "yyyy-MM-dd") + "'"
    ...
    Function SelectedGridPay(ByRef GV As ASPxGridView, ByVal ColumnName As String) As String
        ...
        Try
            ...
            Result = GV.GetSelectedFieldValues("Job_Level")
    ...
    Protected Sub btnOKSlip_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnOKSlip.Click
        ...
        Try
            ...
            Else
                StrSelected = SelectedGridPay(AvailableGrid, "Job_Level")

                If StrSelected.Trim = "" Then
                    lbstatus.Text = "Please select the Job Level!"
                    Exit Sub
                End If

            End If

            'IfExist = SQLExecuteScalar("SELECT Payroll FROM MsPayrollJobLvl WHERE Payroll = " + QuotedStr(tbCode.Text) + " AND StartDate = " + QuotedStr(tbEffDate.SelectedDate) + StrExist, ViewState("DBConnection").ToString)
            'If IfExist.Trim.Length > 0 Then
    ...
    Protected Sub btnOKAdjust_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnOKAdjust.Click
        ...
        Try
            ...
            Else
                StrSelected = SelectedGridPay(GridAdjust, "Job_Level")
    ...
    Protected Sub btnAdjustSlip_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAdjustSlip.Click
        Try
            ...
            LoadDataAdjust()
        Catch ex As Exception
            lbstatus.Text = "btnAdjustSlip_Click error " + ex.ToString
        End Try
    End Sub
End Class
