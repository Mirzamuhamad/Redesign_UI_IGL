...
Partial Class Assign_MsPayrollEmpSt_MsPayrollEmpSt
    ...
    Private Sub ExecSPAddRemove(ByVal StrSelected As String, ByVal Flag As Integer)
        ...
        Try
            SQLString = "EXEC S_MsPayrollEmpStAddRemove '" + tbCode.Text.Trim + "', '" + Format(tbEffDate.SelectedDate, "yyyy-MM-dd") + "', '" + ddlCurr.SelectedValue + "'," + tbAmount.Text.ToString.Replace(",", "") + ",'" + ddlFormula.SelectedValue + "','" + tbRemark.Text + "', '" + StrSelected + "', '" + ViewState("UserId").ToString + "', " + Flag.ToString
    ...
    Private Sub ExecSPAddRemoveAdjust(ByVal StrSelected As String, ByVal Flag As Integer)
        Dim SQLString As String
        Try
            SQLString = "EXEC S_MsPayrollEmpStAddAdjust '" + tbCode.Text.Trim + "', '" + Format(tbEffAdjust.SelectedDate, "yyyy-MM-dd") + "', '" + ddlAdjust.SelectedValue + "'," + tbAmountAdjust.Text.ToString.Replace(",", "") + ",'" + tbRemarkAdjust.Text + "', '" + StrSelected + "', '" + ViewState("UserId").ToString + "', " + Flag.ToString
            SQLExecuteQuery(SQLString, ViewState("DBConnection").ToString)
        Catch ex As Exception
    ...
    Protected Sub btnSearch_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnSearch.Click
        ...
        Try
            Session("filter") = "SELECT * FROM V_MsPayroll Where Payroll_Pref = 'P' AND Payroll_Type IN ('GP','TT','TTT','POT') AND GroupBy = 'Status Emp'"
    ...
    Protected Sub tbCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbCode.TextChanged
        ...
        Dim ds As DataSet
        Try
            ds = SQLExecuteQuery("SELECT * FROM V_MsPayroll WHERE Payroll_Pref = 'P' AND Payroll_Type IN ('GP','TT','TTT','POT') AND Payroll_Code = " + QuotedStr(tbCode.Text) + " AND GroupBy = 'Status Emp'", ViewState("DBConnection").ToString)
    ...
    Protected Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        ...
        Try
            ...
            'SQLString = "EXEC S_MsPayrollEmpStPrint 0, 'Employee - Payroll Slip', 'Payroll Code', 'Payroll Name', 'Start Date', 'Formula', " + QuotedStr(ViewState("UserId").ToString)
            'Else
            SQLString = "EXEC S_MsPayrollEmpStPrint " + QuotedStr(ViewState("UserId").ToString)
            'End If
            Session("PrintType") = "Print"
            Session("DBConnection") = ViewState("DBConnection")
            Session("SelectCommand") = SQLString
            Session("ReportFile") = ".../../../Rpt/FormPayrollEmpSt.frx"
    ...
    Private Sub bindDataGrid()
        Dim SqlString As String
        Try
            SqlString = "EXEC S_MsPayrollEmpStView " + QuotedStr(tbCode.Text)
            If ViewState("SortExpression") = Nothing Then
                ViewState("SortExpression") = "EmpStatus ASC"
            End If
    ...
    Private Sub LoadDataAvailable()
        Try
            ...
            dt = SQLExecuteQuery("EXEC S_MsPayrollEmpStAvailable " + QuotedStr(tbCode.Text) + ", " + QuotedStr(Format(tbEffDate.SelectedDate, "yyyy-MM-dd")), ViewState("DBConnection").ToString).Tables(0)
            ViewState("Dt") = dt
            'If tbCode.Text.Trim <> "" Then
            'AvailableGrid.Selection.UnselectAll()
            dsAvailable.SelectCommand = "EXEC S_MsPayrollEmpStAvailable " + QuotedStr(tbCode.Text) + ", " + QuotedStr(Format(tbEffDate.SelectedDate, "yyyy-MM-dd"))
            'Else
    ...
    Private Sub LoadDataAdjust()
        Try
            ...
            dt = SQLExecuteQuery("EXEC S_MsPayrollEmpStAdjust " + QuotedStr(tbCode.Text) + ", " + QuotedStr(Format(tbEffAdjust.SelectedDate, "yyyy-MM-dd")), ViewState("DBConnection").ToString).Tables(0)
            ViewState("DtAdjust") = dt
            'GridAdjust.Selection.UnselectAll()
            dsAdjust.SelectCommand = "EXEC S_MsPayrollEmpStAdjust " + QuotedStr(tbCode.Text) + ", " + QuotedStr(Format(tbEffAdjust.SelectedDate, "yyyy-MM-dd"))
            GridAdjust.DataBind()
        Catch ex As Exception
    ...
    Protected Sub DataGrid_RowDeleting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewDeleteEventArgs) Handles DataGrid.RowDeleting
        ...
        Try
            If CheckMenuLevel("Delete") = False Then
                Exit Sub
            End If
            EmpCode = DataGrid.Rows(e.RowIndex).FindControl("EmpStatus")
            EffDate = DataGrid.Rows(e.RowIndex).FindControl("EffDate")
            'lbstatus.Text = "Delete from MsPayrollEmpSt where Payroll = " + QuotedStr(tbCode.Text) + " AND EmpStatus = " + QuotedStr(EmpCode.Text) + " AND StartDate = " + QuotedStr(EffDate.Text)
            'Exit Sub
            SQLExecuteNonQuery("Delete from MsPayrollEmpSt where Payroll = " + QuotedStr(tbCode.Text) + " AND EmpStatus = " + QuotedStr(EmpCode.Text) + " AND StartDate = " + QuotedStr(EffDate.Text), ViewState("DBConnection").ToString)
            bindDataGrid()
    ...
    Protected Sub DataGrid_RowUpdating(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewUpdateEventArgs) Handles DataGrid.RowUpdating
        ...
        Try
            ...
            lbCode = GVR.FindControl("EmpStatusEdit")
            ...
            If CFloat(tbAmount.Text.Trim) <= 0 Then
                ...
                Exit Sub
            End If
            IfExist = SQLExecuteScalar("SELECT dbo.FormatDate(StartDate) FROM MsPayrollEmpSt WHERE Payroll = " + QuotedStr(tbCode.Text) + " AND EmpStatus = " + QuotedStr(lbCode.Text) + " AND StartDate <> '" + Format(ViewState("StartDate"), "yyyy-MM-dd") + "' AND StartDate >= " + QuotedStr(Format(tbDate.SelectedDate, "yyyy-MM-dd")), ViewState("DBConnection").ToString)

            If IfExist.Trim.Length > 0 Then
                lbstatus.Text = MessageDlg("Payroll " + QuotedStr(tbCode.Text) + " AND EmpStatus " + QuotedStr(lbCode.Text) + " has already have New Eff. Date " + QuotedStr(IfExist))
                tbDate.Focus()
                Exit Sub
            End If
            SQLString = "Update MsPayrollEmpSt set StartDate = " + QuotedStr(tbDate.Text) + _
            ...
            "  WHERE Payroll = " + QuotedStr(tbCode.Text) + " AND EmpStatus = " + QuotedStr(lbCode.Text) + " AND StartDate = '" + Format(ViewState("StartDate"), "yyyy-MM-dd") + "'"
    ...
    Function SelectedGridPay(ByRef GV As ASPxGridView, ByVal ColumnName As String) As String
        ...
        Try
            ...
            Result = GV.GetSelectedFieldValues("EmpStatus_Code")
    ...
    Protected Sub btnOKSlip_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnOKSlip.Click
        ...
        Try
            ...
            Else
                StrSelected = SelectedGridPay(AvailableGrid, "EmpStatus_Code")
                'StrSelected = StrSelected.Replace("''", "'")
                'StrSelected = StrSelected.Replace("EmpStatus_Code", "EmpStatus")

                If StrSelected.Trim = "" Then
                    lbstatus.Text = "Please select the Employee Status!"
                    Exit Sub
                End If

            End If

            'IfExist = SQLExecuteScalar("SELECT Payroll FROM MsPayrollEmpSt WHERE Payroll = " + QuotedStr(tbCode.Text) + " AND StartDate = " + QuotedStr(tbEffDate.SelectedDate) + StrExist, ViewState("DBConnection").ToString)
            'If IfExist.Trim.Length > 0 Then
    ...
    Protected Sub btnOKAdjust_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnOKAdjust.Click
        ...
        Try
            ...
            Else
                StrSelected = SelectedGridPay(GridAdjust, "EmpStatus_Code")
                'StrSelected = StrSelected.Replace("''", "'")
                'StrSelected = StrSelected.Replace("EmpStatus_Code", "EmpStatus")
