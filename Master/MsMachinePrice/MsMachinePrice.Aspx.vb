...
Partial Class Master_MsMachinePrice_MsMachinePrice
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            If Not IsPostBack Then
                InitProperty()
                FillCombo(ddlCategory, "Select MachineGrpCode, MachineGrpName from MsMachineGroup", False, "MachineGrpCode", "MachineGrpName", ViewState("DBConnection"))
    ...
    Sub BindData()
        ...
        Try
            ...
            End If
            tempDS = SQLExecuteQuery("EXEC S_MsMachinePriceGetDt " + QuotedStr(Format(tbDate.SelectedDate, "yyyyMMdd")) + ", " + QuotedStr(ddlCategory.SelectedValue) + ", " + QuotedStr(StrFilter), ViewState("DBConnection").ToString)
            DataGrid.DataSource = tempDS.Tables(0)
            DataGrid.DataBind()

            For Each GVR In DataGrid.Rows
                Price = GVR.FindControl("Price")
                Premi = GVR.FindControl("Premi")
                Price.Attributes.Add("OnKeyDown", "return PressNumeric();")
                Premi.Attributes.Add("OnKeyDown", "return PressNumeric();")
            Next
        Catch ex As Exception
    ...
    Protected Sub BtnApply_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnApply.Click
        ...
        Dim lbCode, lbLastPrice, lbLastPremi As Label
        Dim tbPrice, tbPremi As TextBox
        ...
        Try
            ...
            For i = 0 To DataGrid.Rows.Count - 1
                GVR = DataGrid.Rows(i)
                lbCode = GVR.FindControl("Machine")
                tbPrice = GVR.FindControl("Price")
                tbPremi = GVR.FindControl("Premi")
                tbPrice.Text = tbPrice.Text.Replace(",", "")
                tbPremi.Text = tbPremi.Text.Replace(",", "")
                lbLastPrice = GVR.FindControl("LastPrice")
                lbLastPremi = GVR.FindControl("LastPremi")
                lbLastPrice.Text = lbLastPrice.Text.Replace(",", "")
                lbLastPremi.Text = lbLastPremi.Text.Replace(",", "")

                If lbLastPrice.Text.Trim = "" Then
                    lbLastPrice.Text = "0"
                End If
                If lbLastPremi.Text.Trim = "" Then
                    lbLastPremi.Text = "0"
                End If
                If tbPrice.Text.Trim = "" Then
                    tbPrice.Text = "0"
                End If
                If tbPremi.Text.Trim = "" Then
                    tbPremi.Text = "0"
                End If
                If Not IsNumeric(tbPrice.Text) Then
                    ...
                    Exit For
                End If
                If Not IsNumeric(tbPremi.Text) Then
                    lbstatus.Text = "Premi for " + lbCode.Text + " must in numeric format"
                    exe = False
                    tbPremi.Focus()
                    Exit For
                End If
            ...
            If exe Then
                For i = 0 To DataGrid.Rows.Count - 1
                    ...
                    If Not ((tbPrice.Text = "" Or tbPrice.Text = "0")) And ((tbPrice.Text <> lbLastPrice.Text) Or (tbPremi.Text <> lbLastPremi.Text)) Then
                        SQLString = "EXEC S_MsMachinePriceUpdate " + QuotedStr(Format(tbDate.SelectedDate, "yyyy-MM-dd")) + ", " + _
                            QuotedStr(lbCode.Text) + ", " + tbPrice.Text + ", " + tbPremi.Text + ", " + QuotedStr(ViewState("UserId").ToString)
        ...
        Try
            ...
            Try
                ...
                If StrFilter.Length > 5 Then
                    ...
                    StrFilter = " And " + StrFilter
                End If
                tempDS = SQLExecuteQuery("EXEC S_MsMachinePriceGetDt " + QuotedStr(Format(tbDate.SelectedDate, "yyyyMMdd")) + ", " + QuotedStr(StrFilter), ViewState("DBConnection").ToString)
                GridExport.DataSource = tempDS.Tables(0)
                GridExport.DataBind()
            Catch ex As Exception
                Throw New Exception("Bind Data Error : " + ex.ToString)
            End Try
            ExportGridToExcel("Machine Price")
        Catch ex As Exception
