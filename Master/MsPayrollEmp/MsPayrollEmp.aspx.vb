...
Partial Class Assign_MsPayrollEmp_MsPayrollEmp
    ...
    Private Sub ExecSPAddRemove(ByVal StrSelected As String, ByVal Flag As Integer)
        ...
        Try
            SQLString = "EXEC S_MsPayrollEmpAddRemove '" + tbCode.Text.Trim + "', '" + Format(tbEffDate.SelectedDate, "yyyy-MM-dd") + "', '" + ddlCurr.SelectedValue + "'," + tbAmount.Text.ToString.Replace(",", "") + ",'" + ddlFormula.SelectedValue + "','" + tbRemark.Text + "', '" + StrSelected + "', '" + ViewState("UserId").ToString + "', " + Flag.ToString
    ...
    Private Sub ExecSPAddRemoveAdjust(ByVal StrSelected As String, ByVal Flag As Integer)
        Dim SQLString As String
        Try
            SQLString = "EXEC S_MsPayrollEmpAddAdjust '" + tbCode.Text.Trim + "', '" + Format(tbEffAdjust.SelectedDate, "yyyy-MM-dd") + "', '" + ddlAdjust.SelectedValue + "'," + tbAmountAdjust.Text.ToString.Replace(",", "") + ",'" + tbRemarkAdjust.Text + "', '" + StrSelected + "', '" + ViewState("UserId").ToString + "', " + Flag.ToString
    ...
    Protected Sub btnSearch_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnSearch.Click
        ...
        Try
            Session("filter") = "SELECT * FROM V_MsPayroll Where Payroll_Pref = 'P' AND Payroll_Type IN ('GP','TT','TTT','POT') AND GroupBy = 'Employee'"
    ...
    Protected Sub tbCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbCode.TextChanged
        ...
        Dim ds As DataSet
        Try
            ds = SQLExecuteQuery("SELECT * FROM V_MsPayroll WHERE Payroll_Pref = 'P' AND Payroll_Type IN ('GP','TT','TTT','POT') AND Payroll_Code = " + QuotedStr(tbCode.Text) + " AND GroupBy = 'Employee'", ViewState("DBConnection").ToString)
    ...
    Protected Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        ...
        Try
            ...
            'Else
            SQLString = "EXEC S_MsPayrollEmpPrint " + QuotedStr(ViewState("UserId").ToString)
            'End If
            Session("PrintType") = "Print"
            Session("DBConnection") = ViewState("DBConnection")
            Session("SelectCommand") = SQLString
            Session("ReportFile") = ".../../../Rpt/FormPayrollEmp.frx"
    ...
    Private Sub bindDataGrid()
        ...
        Try
            ...
            End If
            SqlString = "EXEC S_MsPayrollEmpView " + QuotedStr(tbCode.Text) + "," + QuotedStr(ViewState("UserId").ToString) + "," + QuotedStr(StrFilter)
    ...
    Private Sub LoadDataAvailable()
        Try
            ...
            dt = SQLExecuteQuery("EXEC S_MsPayrollEmpAvailable " + QuotedStr(tbCode.Text) + ", " + QuotedStr(Format(tbEffDate.SelectedDate, "yyyy-MM-dd")) + ", " + QuotedStr(ViewState("UserId").ToString), ViewState("DBConnection").ToString).Tables(0)
            ViewState("Dt") = dt
            'If tbCode.Text.Trim <> "" Then
            'AvailableGrid.Selection.UnselectAll()
            dsAvailable.SelectCommand = "EXEC S_MsPayrollEmpAvailable " + QuotedStr(tbCode.Text) + ", " + QuotedStr(Format(tbEffDate.SelectedDate, "yyyy-MM-dd")) + ", " + QuotedStr(ViewState("UserId").ToString)
            'Else
            'dsAvailable.SelectCommand = "Select '' as Emp_No, '' as Emp_Name, '' AS Job_Title_Name, '' AS Section_Name"
            'End If
    ...
    Private Sub LoadDataAdjust()
        Try
            ...
            dt = SQLExecuteQuery("EXEC S_MsPayrollEmpAdjust " + QuotedStr(tbCode.Text) + ", " + QuotedStr(Format(tbEffAdjust.SelectedDate, "yyyy-MM-dd"))+ ", " + QuotedStr(ViewState("UserId").ToString), ViewState("DBConnection").ToString).Tables(0)
            ViewState("DtAdjust") = dt

            'GridAdjust.Selection.UnselectAll()
            dsAdjust.SelectCommand = "EXEC S_MsPayrollEmpAdjust " + QuotedStr(tbCode.Text) + ", " + QuotedStr(Format(tbEffAdjust.SelectedDate, "yyyy-MM-dd"))+ ", " + QuotedStr(ViewState("UserId").ToString)
            GridAdjust.DataBind()
        Catch ex As Exception
    ...
    Protected Sub DataGrid_RowDeleting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewDeleteEventArgs) Handles DataGrid.RowDeleting
        Dim EmpCode, EffDate As Label
        Try
            If CheckMenuLevel("Delete") = False Then
                Exit Sub
            End If
            EmpCode = DataGrid.Rows(e.RowIndex).FindControl("EmpNumb")
            EffDate = DataGrid.Rows(e.RowIndex).FindControl("EffDate")
            'lbstatus.Text = "Delete from MsPayrollEmp where Payroll = " + QuotedStr(tbCode.Text) + " AND EmpNumb = " + QuotedStr(EmpCode.Text) + " AND StartDate = " + QuotedStr(EffDate.Text)
            'Exit Sub
            SQLExecuteNonQuery("Delete from MsPayrollEmp where Payroll = " + QuotedStr(tbCode.Text) + " AND EmpNumb = " + QuotedStr(EmpCode.Text) + " AND StartDate = " + QuotedStr(EffDate.Text), ViewState("DBConnection").ToString)
            bindDataGrid()
    ...
    Protected Sub DataGrid_RowUpdating(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewUpdateEventArgs) Handles DataGrid.RowUpdating
        ...
        Try
            ...
            lbCode = GVR.FindControl("EmpNumbEdit")
            ...
            If CFloat(tbAmount.Text.Trim) <= 0 Then
                ...
                Exit Sub
            End If
            IfExist = SQLExecuteScalar("SELECT dbo.FormatDate(StartDate) FROM MsPayrollEmp WHERE Payroll = " + QuotedStr(tbCode.Text) + " AND EmpNumb = " + QuotedStr(lbCode.Text) + " AND StartDate <> '" + Format(ViewState("StartDate"), "yyyy-MM-dd") + "' AND StartDate >= " + QuotedStr(Format(tbDate.SelectedDate, "yyyy-MM-dd")), ViewState("DBConnection").ToString)

            If IfExist.Trim.Length > 0 Then
                lbstatus.Text = MessageDlg("Payroll " + QuotedStr(tbCode.Text) + " AND Employee " + QuotedStr(lbCode.Text) + " has already have New Eff. Date " + QuotedStr(IfExist))
                tbDate.Focus()
                Exit Sub
            End If
            SQLString = "Update MsPayrollEmp set StartDate = " + QuotedStr(tbDate.Text) + _
            ...
            "  WHERE Payroll = " + QuotedStr(tbCode.Text) + " AND EmpNumb = " + QuotedStr(lbCode.Text) + " AND StartDate = '" + Format(ViewState("StartDate"), "yyyy-MM-dd") + "'"
            SQLExecuteNonQuery(SQLString, ViewState("DBConnection").ToString)
    ...
    Function SelectedGridPay(ByRef GV As ASPxGridView, ByVal ColumnName As String) As String
        ...
        Try
            ...
            Result = GV.GetSelectedFieldValues("Emp_No")
    ...
    Protected Sub btnOKSlip_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnOKSlip.Click
        ...
        Try
            ...
            Else
                StrSelected = SelectedGridPay(AvailableGrid, "Emp_No")
                'StrSelected = StrSelected.Replace("''", "'")
                'StrSelected = StrSelected.Replace("Emp_No", "EmpNumb")

                If StrSelected.Trim = "" Then
                    lbstatus.Text = "Please select the Employee!"
                    Exit Sub
                End If
            ...
            'IfExist = SQLExecuteScalar("SELECT Payroll FROM MsPayrollEmp WHERE Payroll = " + QuotedStr(tbCode.Text) + " AND StartDate = " + QuotedStr(tbEffDate.SelectedDate) + StrExist, ViewState("DBConnection").ToString)
            'If IfExist.Trim.Length > 0 Then
    ...
    Protected Sub btnOKAdjust_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnOKAdjust.Click
        ...
        Try
            ...
            Else
                StrSelected = SelectedGridPay(GridAdjust, "Emp_No")
                'StrSelected = StrSelected.Replace("''", "'")
                'StrSelected = StrSelected.Replace("Emp_No", "EmpNumb")
    ...
    Protected Sub btnSearchFilter_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearchFilter.Click
        Try
            bindDataGrid()
        Catch ex As Exception
            lbstatus.Text = "btnSearchFilter_Click Error : " + ex.ToString
        End Try
    End Sub
End Class
