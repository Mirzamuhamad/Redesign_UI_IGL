Imports System.Data
Partial Class MsModerator_MsModerator
    ...
    Protected Sub tbCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbCode.TextChanged
        ...
        Try
            ...
            If Left(tbCode.Text, 4) <> "M/" Then
                tbCode.Text = "M/" + tbCode.Text
            Else
    ...
    Protected Sub FillTextBox(ByVal ModCode As String)
        Dim SqlString As String
        Dim DT As DataTable
        Try
            SqlString = "SELECT * FROM V_MsModerator  WHERE ModCode = " + QuotedStr(ModCode)
            DT = BindDataTransaction(SqlString, "", ViewState("DBConnection").ToString)
            BindToText(tbCode, DT.Rows(0)("ModCode").ToString)
            BindToText(tbName, DT.Rows(0)("ModName").ToString)
            BindToDropList(ddlGender, DT.Rows(0)("Gender").ToString)
            BindToDropList(ddlTypeID, DT.Rows(0)("TypeId").ToString)
            BindToText(tbModeratorID, DT.Rows(0)("ModID").ToString)
    ...
    Private Sub bindDataGrid()
        ...
        Try
            ...
            SqlString = "SELECT * From V_MsModerator " + StrFilter
            If ViewState("SortExpression") = Nothing Then
                ViewState("SortExpression") = "ModCode DESC"
    ...
    Protected Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        ...
        Try
            ...
            Session("SelectCommand") = "S_FormPrintMaster6 'V_MsModerator','ModCode','ModName','Gender','TypeID','ModID','Address1+'', ''+Address2+'', ''+Desa +'', ''+ Kec+'', ''+ Kab +'', ''+ City +'', Pos: ''+ ZipCode+'', Telp : ''+ Phone','Moderator File','Code','Description','Gender','TypeID','ModeratorID','Alamat'," + QuotedStr(StrFilter) + ", " + QuotedStr(ViewState("UserId"))
        ...
        Try
            ...
            If e.CommandName = "Go" Then
                ...
                ElseIf DDL.SelectedValue = "Edit" Then
                    ...
                    '            Exit Sub
                    '        End If
                    '        SQLExecuteNonQuery("UPDATE MsModerator SET Fgactive = 'N' WHERE JobCode = '" & GVR.Cells(1).Text & "' ", ViewState("DBConnection").ToString)
                    '        bindDataGrid()
                    '    Catch ex As Exception
                ...
                ElseIf DDL.SelectedValue = "Delete" Then
                    Try
                        If CheckMenuLevel("Delete") = False Then
                            Exit Sub
                        End If
                        SQLExecuteNonQuery("DELETE MsModerator WHERE ModCode = '" & GVR.Cells(1).Text & "' ", ViewState("DBConnection").ToString)
                        bindDataGrid()
                    Catch ex As Exception
                        lstatus.Text = "DataGrid_RowCommand Delete Error = " + ex.ToString
                    End Try
    ...
    Protected Sub BtnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnSave.Click
        ...
        Try
            ...
            If ViewState("State") = "Insert" Then
                If SQLExecuteScalar("SELECT ModCode FROM V_MsModerator WHERE ModCode = " + QuotedStr(tbCode.Text), ViewState("DBConnection").ToString).Length > 0 Then
                    lstatus.Text = "Moderator Code " + QuotedStr(tbCode.Text) + " has already been exist"
                    Exit Sub
                End If

                SqlString = "INSERT INTO MsModerator (ModCode, ModName, Gender, TypeID, ModID, NoKK, Address1,Address2,Desa, Kec, Kab, City, ZipCode, Phone, Email, NPWP, FgPT, UserId, UserDate ) " + _
                ...
                QuotedStr(tbModeratorID.Text) + ", " + _
            ...
            Else
                SqlString = "UPDATE MsModerator SET ModName= " + QuotedStr(tbName.Text) & _
                            ", Gender = " + QuotedStr(ddlGender.Text) & _
                            ", TypeID = " + QuotedStr(ddlTypeID.Text) & _
                            ", ModID = " + QuotedStr(tbModeratorID.Text) & _
                            ...
                            " WHERE ModCode = " + QuotedStr(tbCode.Text)
            End If
