...
Partial Class MsPPH
    ...
    Private Sub bindDataGrid()
        ...
        Try
            ...
            SqlString = "SELECT * FROM V_MsPPH " + StrFilter
            If ViewState("SortExpression") = Nothing Then
                ViewState("SortExpression") = "PPHCode ASC"
            End If
    ...
    Protected Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        ...
        Try
            ...
            Session("SelectCommand") = "S_FormPrintMaster3 'V_MsPPH', 'PPHCode +''-''+ PPHName ', 'Type', 'Accountname ' , 'PPH File', 'PPH Code - Name', 'Type', 'Account', '" + StrFilter + "', " + QuotedStr(ViewState("UserId"))
    ...
    Protected Sub DataGrid_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles DataGrid.RowCommand
        ...
        Try
            If e.CommandName = "Insert" Then
                dbCode = DataGrid.FooterRow.FindControl("PPHCodeAdd")
                dbName = DataGrid.FooterRow.FindControl("PPHNameAdd")
                Type = DataGrid.FooterRow.FindControl("TypeAdd")
                tbAcc = DataGrid.FooterRow.FindControl("AccountAdd")

                If dbCode.Text.Trim.Length = 0 Then
                    lstatus.Text = "PPH Code must be filled."
                    dbCode.Focus()
                    Exit Sub
                End If
                If dbName.Text.Trim.Length = 0 Then
                    lstatus.Text = "PPH Name must be filled."
                    dbName.Focus()
                    Exit Sub
                End If
               

                If SQLExecuteScalar("SELECT PPHCode From MsPPH WHERE PPHCode  = " + QuotedStr(dbCode.Text), ViewState("DBConnection").ToString).Length > 0 Then
                    lstatus.Text = "PPH " + QuotedStr(dbCode.Text) + " has already been exist"
                    Exit Sub
                End If

                'insert the new entry
                SQLString = "Insert into MsPPH (PPHCode, PPHName,Type,Account,UserId, UserDate ) " + _
                "SELECT " + QuotedStr(dbCode.Text) + ", " + QuotedStr(dbName.Text) + _
                ", " + QuotedStr(Type.SelectedValue) + _
    ...
    Protected Sub DataGrid_RowDeleting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewDeleteEventArgs) Handles DataGrid.RowDeleting
        ...
        Try
            If CheckMenuLevel("Delete") = False Then
                Exit Sub
            End If
            txtID = DataGrid.Rows(e.RowIndex).FindControl("PPHCode")

            SQLExecuteNonQuery("Delete from MsPPH where PPHCode = '" & txtID.Text & "'", ViewState("DBConnection"))
            bindDataGrid()
    ...
    Protected Sub DataGrid_RowEditing(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewEditEventArgs) Handles DataGrid.RowEditing
        ...
        Try
            ...
            txt = obj.FindControl("PPHNameEdit")
            txt.Focus()
        Catch ex As Exception
    ...
    Protected Sub DataGrid_RowUpdating(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewUpdateEventArgs) Handles DataGrid.RowUpdating
        ...
        Try
            lbCode = DataGrid.Rows(e.RowIndex).FindControl("PPHCodeEdit")
            dbName = DataGrid.Rows(e.RowIndex).FindControl("PPHNameEdit")
            Type = DataGrid.Rows(e.RowIndex).FindControl("TypeEdit")
            ...
            SQLString = "Update MsPPH set PPHName= " + QuotedStr(dbName.Text) + _
            ",Type = " + QuotedStr(Format(Type.SelectedValue)) + _
                ",Account= " + QuotedStr(tbAcc.Text) + _
                     " where PPHCode = '" & lbCode.Text + "'"
