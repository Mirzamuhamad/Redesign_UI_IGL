...
Partial Class Assign_MsPayrollEmp_MsPayrollEmp
    ...
    Private Sub bindDataGrid()
        Dim SqlString As String
        Try            
            SqlString = "EXEC S_MsPayrollEmpView " + QuotedStr(tbCode.Text)
    ...
    Private Sub LoadDataAvailable()
        Try
            ...
            dt = SQLExecuteQuery("EXEC S_MsPayrollEmpAvailable " + QuotedStr(tbCode.Text) + ", " + QuotedStr(Format(tbEffDate.SelectedDate, "yyyy-MM-dd")), ViewState("DBConnection").ToString).Tables(0)
            ViewState("Dt") = dt
            'If tbCode.Text.Trim <> "" Then
            AvailableGrid.Selection.UnselectAll()
            dsAvailable.SelectCommand = "EXEC S_MsPayrollEmpAvailable " + QuotedStr(tbCode.Text) + ", " + QuotedStr(Format(tbEffDate.SelectedDate, "yyyy-MM-dd"))
            'Else
    ...
    Private Sub LoadDataAdjust()
        Try
            ...
            dt = SQLExecuteQuery("EXEC S_MsPayrollEmpAdjust " + QuotedStr(tbCode.Text) + ", " + QuotedStr(Format(tbEffDate.SelectedDate, "yyyy-MM-dd")), ViewState("DBConnection").ToString).Tables(0)
            ViewState("DtAdjust") = dt
            GridAdjust.Selection.UnselectAll()
            dsAdjust.SelectCommand = "EXEC S_MsPayrollEmpAdjust " + QuotedStr(tbCode.Text) + ", " + QuotedStr(Format(tbEffDate.SelectedDate, "yyyy-MM-dd"))
            GridAdjust.DataBind()
        Catch ex As Exception
    ...
    Protected Sub btnOKSlip_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnOKSlip.Click
        Dim StrSelected, StrExist As String
        Dim P As Integer
        Try
            ...
            Else
                ...
                StrExist = StrExist.Replace("Emp_No", "EmpNumb")
