...
Partial Class MsForm
    ...
    Protected Sub FillTextBox(ByVal SellCode As String)
        ...
        Dim DT As DataTable
        Try
            SqlString = "SELECT * FROM V_MsSeller  WHERE SellCode = " + QuotedStr(SellCode)
            DT = BindDataTransaction(SqlString, "", ViewState("DBConnection").ToString)
            BindToText(tbCode, DT.Rows(0)("SellCode").ToString)
            BindToText(tbName, DT.Rows(0)("SellName").ToString)
            BindToDropList(ddlGender, DT.Rows(0)("Gender").ToString)
            BindToDropList(ddlTypeID, DT.Rows(0)("TypeId").ToString)
            BindToText(tbSellerID, DT.Rows(0)("SellID").ToString)
    ...
    Private Sub bindDataGrid()
        ...
        Try
            ...
            SqlString = "SELECT * From V_MsSeller " + StrFilter
            If ViewState("SortExpression") = Nothing Then
                ViewState("SortExpression") = "SellCode DESC"
    ...
    Protected Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        ...
        Try
            ...
            Session("SelectCommand") = "S_FormPrintMaster6 'V_MsSeller','SellCode','SellName','Gender','TypeID','SellID','Address1+'', ''+Address2+'', ''+ Desa+'', ''+ Kec+'', ''+ Kab +'', ''+ City +'', Pos: ''+ ZipCode+'', Telp : ''+ Phone','Seller File','Code','Description','Gender','TypeID','SellerID','Alamat'," + QuotedStr(StrFilter) + ", " + QuotedStr(ViewState("UserId"))
        ...
        Try
            ...
            If e.CommandName = "Go" Then
                ...
                ElseIf DDL.SelectedValue = "Edit" Then
                    ...
                    '            Exit Sub
                    '        End If
                    '        SQLExecuteNonQuery("UPDATE MsSeller SET Fgactive = 'N' WHERE JobCode = '" & GVR.Cells(1).Text & "' ", ViewState("DBConnection").ToString)
                    '        bindDataGrid()
                    '    Catch ex As Exception
                ...
                ElseIf DDL.SelectedValue = "Delete" Then
                    Try
                        If CheckMenuLevel("Delete") = False Then
                            Exit Sub
                        End If
                        SQLExecuteNonQuery("DELETE MsSeller WHERE SellCode = '" & GVR.Cells(1).Text & "' ", ViewState("DBConnection").ToString)
                        bindDataGrid()
                    Catch ex As Exception
                        lstatus.Text = "DataGrid_RowCommand Delete Error = " + ex.ToString
                    End Try
    ...
    Protected Sub BtnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnSave.Click
        ...
        ' Exit sub
            ...
            If ViewState("State") = "Insert" Then
                If SQLExecuteScalar("SELECT SellCode FROM V_MsSeller WHERE SellCode = " + QuotedStr(tbCode.Text), ViewState("DBConnection").ToString).Length > 0 Then
                    lstatus.Text = "Seller Code " + QuotedStr(tbCode.Text) + " has already been exist"
                    Exit Sub
                End If

                SqlString = "INSERT INTO MsSeller (SellCode, SellName, Gender, TypeID, SellID, NoKK, Address1,Address2,Desa, Kec, Kab, City, ZipCode, Phone, Email, NPWP, FgPT, UserId, UserDate ) " + _
            ...
            Else
                SqlString = "UPDATE MsSeller SET SellName= " + QuotedStr(tbName.Text) & _
                            ", Gender = " + QuotedStr(ddlGender.Text) & _
                            ", TypeID = " + QuotedStr(ddlTypeID.Text) & _
                            ", SellID = " + QuotedStr(tbSellerID.Text) & _
                            ...
                            " WHERE SellCode = " + QuotedStr(tbCode.Text)
            End If
    ...
    DataGrid.PageSize = Convert.ToInt32(ddlPageSize.SelectedValue)
    DataGrid.PageIndex = 0
    bindDataGrid
End Sub
