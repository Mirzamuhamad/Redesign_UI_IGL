...
Partial Class Tugas
    ...
    Protected Sub FillTextBox(ByVal TugasCode As String)
        Dim SqlString As String
        Dim DT As DataTable
        Try
            SqlString = "SELECT * FROM V_MsPemberiTugas WHERE TugasCode = " + QuotedStr(TugasCode)
            DT = BindDataTransaction(SqlString, "", ViewState("DBConnection").ToString)
            BindToText(tbCode, DT.Rows(0)("TugasCode").ToString)
            BindToText(tbName, DT.Rows(0)("TugasName").ToString)
            BindToText(tbNoKTP, DT.Rows(0)("NoKtp").ToString)
    ...
    Private Sub bindDataGrid()
        ...
        Try
            ...
            SqlString = "SELECT * From V_MsPemberiTugas" + StrFilter
            If ViewState("SortExpression") = Nothing Then
                ViewState("SortExpression") = "TugasCode DESC"
    ...
    Protected Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        ...
        Try
            ...
            Session("SelectCommand") = "S_FormPrintMaster6 'V_MsNotaris','TugasCode','TugasName','Gender','TypeID','NotID','Address1+'', ''+Address2+'', ''+Desa +'', ''+ Kec+'', ''+ Kab +'', ''+ City +'', Pos: ''+ ZipCode+'', Telp : ''+ Phone','Notaris File','Code','Description','Gender','TypeID','NotarisID','Alamat'," + QuotedStr(StrFilter) + ", " + QuotedStr(ViewState("UserId"))
        ...
        Try
            ...
            If e.CommandName = "Go" Then
                ...
                ElseIf DDL.SelectedValue = "Edit" Then
                    ...
                    '            Exit Sub
                    '        End If
                    '        SQLExecuteNonQuery("UPDATE MsPemberiTugasSET Fgactive = 'N' WHERE JobCode = '" & GVR.Cells(1).Text & "' ", ViewState("DBConnection").ToString)
                    '        bindDataGrid()
                    '    Catch ex As Exception
                ...
                ElseIf DDL.SelectedValue = "Delete" Then
                    Try
                        If CheckMenuLevel("Delete") = False Then
                            Exit Sub
                        End If
                        SQLExecuteNonQuery("DELETE MsPemberiTugas WHERE TugasCode = '" & GVR.Cells(1).Text & "' ", ViewState("DBConnection").ToString)
                        bindDataGrid()
                    Catch ex As Exception
                        lstatus.Text = "DataGrid_RowCommand Delete Error = " + ex.ToString
                    End Try
    ...
    Private Function cekInput() As Boolean
        Try
            ...
            If tbNoKTP.Text.Trim.Length = 0 Then
    ...
    Protected Sub BtnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnSave.Click
        ...
        Try
            ...
            If ViewState("State") = "Insert" Then
                Code = SQLExecuteScalar("SELECT TugasCode FROM V_MsPemberiTugas WHERE TugasCode = " + QuotedStr(tbCode.Text), ViewState("DBConnection").ToString)
                If Code = tbCode.Text Then
                    lstatus.Text = MessageDlg("Pemberi Tugas Code " + QuotedStr(tbCode.Text) + " has already been exist")
                    Exit Sub
                End If

                'ID = SQLExecuteScalar("SELECT TypeID+'|'+NotID FROM V_MsPemberiTugas WHERE TypeID = " + QuotedStr(ddlTypeID.SelectedValue) + " AND NotID = " + QuotedStr(tbNotarisID.Text), ViewState("DBConnection").ToString)
                ...
                SqlString = "INSERT INTO MsPemberiTugas(TugasCode, TugasName, NoKTP, Title, Address1,Address2,City,Phone, Email, NPWP, UserId, UserDate ) " + _
                "SELECT " + QuotedStr(tbCode.Text) + ", " + QuotedStr(tbName.Text) + ", " & _
                QuotedStr(tbNoKTP.Text) + ", " + _
                QuotedStr(tbTitle.Text) + ", " + _
                QuotedStr(tbAddress.Text) + ", " + _
                QuotedStr(tbAddress2.Text) + ", " + _
                QuotedStr(ddlCity.SelectedValue) + ", " + _
                QuotedStr(tbPhone.Text) + ", " + _
                QuotedStr(tbEmail.Text) + ", " + _
                QuotedStr(tbNpwp.Text) + ", " + _
               QuotedStr(ViewState("UserId").ToString) + ", GetDate()"
            Else
                SqlString = "UPDATE MsPemberiTugas SET TugasName = " + QuotedStr(tbName.Text) & _
                            ", City = " + QuotedStr(ddlCity.SelectedValue) & _
                            ", NoKtp = " + QuotedStr(tbNoKTP.Text) & _
                            ", Title = " + QuotedStr(tbTitle.Text) & _
                            ...
                            " WHERE TugasCode = " + QuotedStr(tbCode.Text)
            End If
