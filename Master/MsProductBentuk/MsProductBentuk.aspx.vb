...
Partial Class Master_MsProductBentuk_MsProductBentuk
    ...
    '        SqlString = "Select * from MsProductBentuk " + StrFilter
    '        If ViewState("SortExpression") = Nothing Then
    '            ViewState("SortExpression") = "ProductJenis ASC"
    '        End If
    ...
    Private Sub bindDataGrid()
        ...
        Try
            StrFilter = GenerateFilterMs(ddlField.SelectedValue, ddlField2.SelectedValue, tbFilter.Text, tbfilter2.Text, ddlNotasi.SelectedValue)
            SqlString = "SELECT A.ProductJenis, A.ProductBentuk, A.BentukName, B.JenisName As Jenis_name FROM MsProductBentuk A INNER JOIN MsProductJenis B ON A.ProductJenis = B.JenisCode " + StrFilter + " Order By A.ProductJenis "
            If ViewState("SortExpression") = Nothing Then
                ViewState("SortExpression") = "ProductJenis ASC"
    ...
    Private Sub bindDataGridDt()
        ...
        Try
            ...
            "R.AccountName AS AccExpLossName  FROM MsProductBentukDt A LEFT OUTER JOIN " + _
            ...
            "MsAccount R ON A.AccExpLoss = R.Account WHERE A.ProductJenis =" + QuotedStr(ViewState("Nmbr")), ViewState("DBConnection").ToString)
    ...
    Public Sub DataGrid_RowEditing(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewEditEventArgs) Handles DataGrid.RowEditing
        ...
        Dim txt As TextBox
        'Dim ddlFgStock, dbProductBentuk As DropDownList
        ...
        Try
            ...
            txt = obj.FindControl("BentukNameEdit")
            'dbProductBentuk = obj.FindControl("ProductBentukEdit")
            'ddlFgStock = obj.FindControl("FgStockEdit")

            'If dbProductBentuk.SelectedIndex = 4 Then
        ...
        '    DataGrid.ShowFooter = False
        '    bindDataGrid()
        '    tbName = DataGrid.Rows(e.NewEditIndex).FindControl("BentukNameEdit")
        '    'dbProductBentuk = DataGrid.Rows(e.NewEditIndex).FindControl("ProductBentuk")
        '    'lstatus.Text = dbProductBentuk.Text
        '    'Exit Sub
        '    tbName.Focus()
        'Catch ex As Exception
        '    lstatus.Text = "DataGrid_Edit exception : " + ex.ToString
    ...
    Protected Sub DataGrid_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles DataGrid.RowCommand
        ...
        Dim dbName, dbBentuk As TextBox
        ...
        Try
            If e.CommandName = "Insert" Then
                ...
                dbCode = GVR.FindControl("ProductJenisAdd")
                dbName = GVR.FindControl("BentukNameAdd")
                dbBentuk = GVR.FindControl("ProductBentukAdd")
                'ddlFgStock = GVR.FindControl("FgStockAdd")

                If dbCode.SelectedValue.Trim = "" Then
                    lstatus.Text = "<script language='javascript'>alert('Product Jenis Code must be filled');</script>"
                    dbCode.Focus()
                    Exit Sub
                End If

                If dbBentuk.Text.Trim = "" Then
                    lstatus.Text = "<script language='javascript'>alert('Product Bentuk must be filled');</script>"
                    dbBentuk.Focus()
                    Exit Sub
                End If

                If dbName.Text.Trim.Length = 0 Then
                    lstatus.Text = "<script language='javascript'>alert('Product Bentuk Name must be filled');</script>"
                    dbName.Focus()
                    Exit Sub
                End If

                If SQLExecuteScalar("SELECT Product_Materi_Code From VMsProductBentuk WHERE Product_Materi_Code = " + QuotedStr(dbCode.Text), ViewState("DBConnection").ToString).Length > 0 Then
                    lstatus.Text = "Product Materi " + QuotedStr(dbCode.Text) + " has already been exist"
                    Exit Sub
                End If

                If SQLExecuteScalar("SELECT ProductBentuk From MsProductBentuk WHERE ProductBentuk = " + QuotedStr(dbBentuk.Text), ViewState("DBConnection").ToString).Length > 0 Then
                    lstatus.Text = "Product Bentuk " + QuotedStr(dbBentuk.Text) + " has already been exist"
                    Exit Sub
                End If

                'insert the new entry
                SQLString = "Insert into MsProductBentuk (ProductJenis, BentukName, ProductBentuk, UserID, UserDate) " + _
                "SELECT " + QuotedStr(dbCode.SelectedValue) + "," + QuotedStr(dbName.Text) + ", " + QuotedStr(dbBentuk.Text) + "," + _
                QuotedStr(ViewState("UserId").ToString) + ", GetDate()"
                SQLExecuteNonQuery(SQLString, ViewState("DBConnection").ToString)
                bindDataGrid()
            ElseIf e.CommandName = "View" Then
                Dim lbCode, lbName, lbBentuk As Label
                GVR = DataGrid.Rows(CInt(e.CommandArgument))
                lbCode = GVR.FindControl("ProductJenis")
                lbBentuk = GVR.FindControl("ProductBentuk")
                lbName = GVR.FindControl("BentukName")
                ViewState("Nmbr") = lbCode.Text
                ViewState("ProductBentuk") = lbBentuk.Text
                lbProductBentuk.Text = lbBentuk.Text
                '+ " - " + lbName.Text
                pnlHd.Visible = False
                pnlDt.Visible = True
            ...
            If e.CommandName = "Go" Then
                ...
                If DDL.SelectedValue = "Delete" Then
                    Try
                        ...
                        SQLExecuteNonQuery("Delete from MsProductBentukDt where ProductJenis = " + QuotedStr(ViewState("Nmbr")) + " AND WrhsType =" + QuotedStr(GVR.Cells(1).Text), ViewState("DBConnection").ToString)
                        bindDataGridDt()
    ...
    Protected Sub DataGrid_RowUpdating(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewUpdateEventArgs) Handles DataGrid.RowUpdating
        Dim SQLString As String
        Dim dbName, dbProductBentuk As TextBox
        Dim lbCode As Label
        'Dim ddlFgStock As DropDownList
        Dim GVR As GridViewRow
        Try
            GVR = DataGrid.Rows(e.RowIndex)
            lbCode = GVR.FindControl("ProductJenisEdit")
            dbName = GVR.FindControl("BentukNameEdit")
            'dbProductBentuk = GVR.FindControl("ProductBentukEdit")
            ...
            SQLString = "Update MsProductBentuk set BentukName =" + QuotedStr(dbName.Text) + _
            "  where ProductJenis = '" & lbCode.Text + "'"
    ...
    Protected Sub DataGridDt_RowUpdating(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewUpdateEventArgs) Handles DataGridDt.RowUpdating
        ...
        Try
            ...
            SQLString = "UPDATE MsProductBentukDt SET ACCInvent = " + QuotedStr(tbAccInvent.Text) + _
            ", ProductBentuk= " + QuotedStr(lbProductBentuk.Text) + _
    ...
    Protected Sub DataGrid_RowDeleting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewDeleteEventArgs) Handles DataGrid.RowDeleting
        ...
        Try
            If CheckMenuLevel("Delete") = False Then
                Exit Sub
            End If
            txtID = DataGrid.Rows(e.RowIndex).FindControl("ProductJenis")

            SQLExecuteNonQuery("Delete from MsProductBentukDt where ProductJenis = '" & txtID.Text & "'", ViewState("DBConnection").ToString)
            SQLExecuteNonQuery("Delete from MsProductBentuk where ProductJenis = '" & txtID.Text & "'", ViewState("DBConnection").ToString)
            bindDataGrid()
    ...
    Protected Sub DataGridDt_RowDeleting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewDeleteEventArgs) Handles DataGridDt.RowDeleting
        ...
        '    SQLExecuteNonQuery("Delete from MsProductBentukDt where ProductType = " + QuotedStr(ViewState("Nmbr")) + " AND WrhsType =" + QuotedStr(txtID.Text))
        '    bindDataGridDt()
    ...
    Protected Sub ProductBentukEdit_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim dgi As GridViewRow
        Dim dbProductBentuk, ddlFgStock As DropDownList
        Try
            dgi = DataGrid.Rows(DataGrid.EditIndex)
            dbProductBentuk = dgi.FindControl("ProductBentukEdit")
            'ddlFgStock = dgi.FindControl("FgStockEdit")

            'If dbProductBentuk.SelectedValue = "Service" Or dbProductBentuk.SelectedValue = "Fixed Asset" Or dbProductBentuk.SelectedValue = "Expendable Asset" Then
    ...
    Protected Sub ProductBentukAdd_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim dgi As GridViewRow
        Dim dbProductBentuk, ddlFgStock As DropDownList
        Try
            dgi = DataGrid.FooterRow
            dbProductBentuk = dgi.FindControl("ProductBentukAdd")
    ...
    Protected Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        ...
        Try
            ...
            Session("SelectCommand") = "EXEC S_FormPrintMaster4 'VMsProductBentukNew',' Jenis_name','ProductJenis',' BentukName','ProductBentuk '," + QuotedStr(lblTitle.Text) + ",'Product Jenis Name','Product Jenis Code','Product Bentuk Name','Product Bentuk'," + QuotedStr(StrFilter) + "," + QuotedStr(ViewState("UserId"))
    ...
    Protected Sub FillTextBoxDt(ByVal Nmbr As String, ByVal Code As String)
        Try
            ...
            "MsAccount K ON A.AccSales = K.Account LEFT OUTER JOIN " + _
            "MsAccount L ON A.AccSRetur = L.Account LEFT OUTER JOIN " + _
            "MsAccount R ON A.AccExpLoss = R.Account WHERE A.ProductJenis =" + QuotedStr(Nmbr) + " AND WrhsType = " + QuotedStr(Code), ViewState("DBConnection").ToString).Tables(0).Rows(0)
    ...
    Protected Sub btnAccTransitReject_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAccTransitReject.Click
        ...
        Try
            Session("DBConnection") = ViewState("DBConnection")
            Session("filter") = "SELECT * FROM VMsAccount Where Currency In (" + QuotedStr(ViewState("Currency")) + ") And FgSubled IN ('N','P') and FgType = 'PL' and FgNormal = 'C'"
            FieldResult = "Account, Description"
            Session("Column") = FieldResult.Split(",")
            AttachScript("OpenPopup();", Page, Me.GetType())
            ViewState("Sender") = "btnAccTransitReject"
        Catch ex As Exception
            lstatus.Text = "btnAccTransitReject Error : " + ex.ToString
        End Try
    ...
    Protected Sub tbAccTransitReject_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbAccTransitReject.TextChanged
        ...
        Dim ds As DataSet
        Try
            ds = SQLExecuteQuery("SELECT * FROM VMsAccount Where Currency In (" + QuotedStr(ViewState("Currency")) + ") And FgSubled IN ('N','P') and FgType = 'BS' and FgNormal = 'D' Account = " + QuotedStr(tbAccTransitReject.Text), ViewState("DBConnection").ToString)
            If IsNothing(ds) Or ds.Tables(0).Rows.Count <= 0 Or IsNothing(ds.Tables(0)) Then
                tbAccTransitReject.Text = ""
                tbAccTransitRejectName.Text = ""
            Else
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        Try
            ...
            If ViewState("State") = "Insert" Then
                If SQLExecuteScalar("SELECT WrhsType From MsProductBentukDt WHERE ProductJenis = " + QuotedStr(ViewState("Nmbr")) + " AND WrhsType =" + QuotedStr(ddlWrhsType.SelectedValue), ViewState("DBConnection").ToString).Length > 0 Then
                    lstatus.Text = "Product Jenis" + QuotedStr(ViewState("Nmbr")) + " Warehouse Type " + QuotedStr(ddlWrhsType.Text) + " has already been exist"
                    Exit Sub
                End If



                SQLString = "Insert Into MsProductBentukDt (ProductJenis, ProductBentuk, WrhsType, AccInvent, AccSales, AccCOGS, AccTransitSJ, AccTransitWrhs, AccTransitReject, AccTransitRetur, AccSRetur, AccExpLoss, UserId, UserDate) " + _
                "SELECT " + QuotedStr(ViewState("Nmbr")) + "," + QuotedStr(lbProductBentuk.Text) + ", " + QuotedStr(ddlWrhsType.SelectedValue) + "," + _
            ...
            ElseIf ViewState("State") = "Edit" Then
                ...
                " AND ProductJenis =" + QuotedStr(ViewState("Nmbr"))
            End If
